    ...
    for location in world.get_locations():
        if world.settings.shuffle_song_items == "song":
            if location.type == "Song":
                ...
                add_item_rule(
                    location,
                    lambda location, item: (
                        (
                            location.world.distribution.song_as_items
                            or any(
        ...
        if location.type == "Shop":
            if location.name in world.shop_prices:
                ...
                if (
                    location.name in world.distribution.locations
                    and world.distribution.locations[location.name].price is not None
                ):
                ...
                add_item_rule(
                    location,
                    lambda location, item: item.type == "Shop"
                    and item.world.id == location.world.id,
                )
        ...
        if world.skip_child_zelda and location.name == "Song from Impa":
            if (
                world.settings.triforce_hunt
                and world.total_starting_triforce_count
                >= world.triforce_goal - world.settings.world_count
            ):
...
def create_shop_rule(location: Location) -> AccessRule:
    ...
    return location.world.parser.parse_rule(
        "(Progressive_Wallet, %d)" % required_wallets(location.price)
    )

...
def get_hint_group(group: str, world: World) -> list[Hint]:
    ...
    for name in hintTable:
        ...
        if (
            group == "dual_always"
            and hint.name in conditional_dual_always
            and conditional_dual_always[hint.name](world)
        ):
            hint.type = "dual_always"

        if (
            group == "entrance_always"
            and hint.name in conditional_entrance_always
            and conditional_entrance_always[hint.name](world)
        ):
        ...
        type_append = False
        if (
            group in ["overworld", "dungeon", "sometimes", "dual", "entrance"]
            and hint.name in conditional_sometimes.keys()
        ):
...
def get_required_hints(world: World) -> list[Hint]:
    ...
    for name in hintTable:
        ...
        if (
            "always" in hint.type
            or hint.name in conditional_always
            and conditional_always[hint.name](world)
        ):
            ret.append(hint)
    return ret
...
def get_upgrade_hint_list(world: World, locations: list[str]) -> list[Hint]:
    ...
    for name in multiTable:
        if name not in hint_exclusions(world):
            ...
            if (
                len(locations) < len(multi.locations)
                and all(location in multi.locations for location in locations)
                and (
                    hint.name not in conditional_sometimes.keys()
                    or conditional_sometimes[hint.name](world)
                )
            ):
                accepted_type = False
...
def stones_required_by_settings(world: World) -> int:
    ...
    if (
        world.settings.shuffle_ganon_bosskey == "on_lacs"
        and world.settings.lacs_condition == "stones"
    ):
    ...
    if (
        world.settings.shuffle_ganon_bosskey == "on_lacs"
        and world.settings.lacs_condition == "dungeons"
    ):
...
def medallions_required_by_settings(world: World) -> int:
    ...
    if (
        world.settings.shuffle_ganon_bosskey == "on_lacs"
        and world.settings.lacs_condition == "medallions"
    ):
...
conditional_always: dict[str, Callable[[World], bool]] = {
    ...
    and "Mask of Truth" not in world.settings.shuffle_child_trade,
    ...
    "DMT Biggoron": lambda world: (
        "Claim Check" not in world.settings.adult_trade_start
        or len(world.settings.adult_trade_start) != 1
    )
    and not world.settings.adult_trade_shuffle,
    "Kak 30 Gold Skulltula Reward": lambda world: tokens_required_by_settings(world) < 30
    and "30_skulltulas" not in world.settings.misc_hints,
    "Kak 40 Gold Skulltula Reward": lambda world: tokens_required_by_settings(world) < 40
    and "40_skulltulas" not in world.settings.misc_hints,
    "Kak 50 Gold Skulltula Reward": lambda world: tokens_required_by_settings(world) < 50
    and "50_skulltulas" not in world.settings.misc_hints,
...
conditional_entrance_always: dict[str, Callable[[World], bool]] = {
    "Ganons Castle Grounds -> Ganons Castle Lobby": lambda world: (
        world.settings.bridge != "open"
        ...
        and (
            ...
            or world.settings.bridge_hearts > world.settings.starting_hearts + 1
        )
    ),
}
...
conditional_dual_always: dict[str, Callable[[World], bool]] = {
    ...
    and "frogs2" not in world.settings.misc_hints,
}
...
conditional_sometimes: dict[str, Callable[[World], bool]] = {
    ...
    or "Eyeball Frog" not in world.settings.adult_trade_start,
    # Conditional dual hints
    "GV Pieces of Heart Ledges": lambda world: not world.settings.shuffle_cows
    and world.settings.tokensanity not in ("overworld", "all"),
    "LH Adult Bean Destination Checks": lambda world: world.settings.shuffle_interior_entrances
    == "off",
    ...
    and "Eyeball Frog" in world.settings.adult_trade_start,
    "Fire Temple Lower Loop": lambda world: world.settings.tokensanity not in ("dungeons", "all"),
    "Water Temple River Loop Chests": lambda world: world.settings.tokensanity
    not in ("dungeons", "all"),
    "Water Temple MQ Lower Checks": lambda world: world.settings.tokensanity
    not in ("dungeons", "all"),
    "Spirit Temple Child Lower": lambda world: world.settings.tokensanity
    not in ("dungeons", "all"),
    "Spirit Temple Adult Lower": lambda world: world.settings.tokensanity
    not in ("dungeons", "all"),
    "Shadow Temple Invisible Blades Chests": lambda world: world.settings.tokensanity
    not in ("dungeons", "all"),
    # We don't want to hint unshuffled dungeon rewards even if compasses give info and are removed
    "Queen Gohma Rewards": lambda world: world.settings.shuffle_dungeon_rewards
    not in ("vanilla", "reward"),
    "King Dodongo Rewards": lambda world: world.settings.shuffle_dungeon_rewards
    not in ("vanilla", "reward"),
    "Barinade Rewards": lambda world: world.settings.shuffle_dungeon_rewards
    not in ("vanilla", "reward"),
    "Phantom Ganon Rewards": lambda world: world.settings.shuffle_dungeon_rewards
    not in ("vanilla", "reward"),
    "Volvagia Rewards": lambda world: world.settings.shuffle_dungeon_rewards
    not in ("vanilla", "reward"),
    "Morpha Rewards": lambda world: world.settings.shuffle_dungeon_rewards
    not in ("vanilla", "reward"),
    "Bongo Bongo Rewards": lambda world: world.settings.shuffle_dungeon_rewards
    not in ("vanilla", "reward"),
    "Twinrova Rewards": lambda world: world.settings.shuffle_dungeon_rewards
    not in ("vanilla", "reward"),
    ...
    "Ganons Castle Grounds -> Ganons Castle Lobby": lambda world: (
        ...
        and (
            world.settings.bridge != "hearts"
            or world.settings.bridge_hearts > world.settings.starting_hearts
        )
    ),
}
...
hintTable: dict[str, tuple[list[str] | str, Optional[str], str | list[str]]] = {
    "Kokiri Emerald": (
        ["a tree's farewell", "the Spiritual Stone of the Forest"],
        "the Kokiri Emerald",
        "item",
    ),
    "Goron Ruby": (
        ["the Gorons' hidden treasure", "the Spiritual Stone of Fire"],
        "the Goron Ruby",
        "item",
    ),
    "Zora Sapphire": (
        ["an engagement ring", "the Spiritual Stone of Water"],
        "the Zora Sapphire",
        "item",
    ),
    "Light Medallion": (
        ["a sagely power frozen in time", "an old man's sagely power", "a yellow disc"],
        "the Light Medallion",
        "item",
    ),
    "Forest Medallion": (
        ["a sagely power given by a childhood friend", "a Kokiri's sagely power", "a green disc"],
        "the Forest Medallion",
        "item",
    ),
    "Fire Medallion": (
        ["a sagely power forged in lava", "a Goron's sagely power", "a red disc"],
        "the Fire Medallion",
        "item",
    ),
    "Water Medallion": (
        ["a sagely power given by your fiancée", "a Zora's sagely power", "a blue disc"],
        "the Water Medallion",
        "item",
    ),
    "Shadow Medallion": (
        ["a sagely power forged in blood", "a Sheikah's sagely power", "a purple disc"],
        "the Shadow Medallion",
        "item",
    ),
    "Spirit Medallion": (
        ["a sagely power tainted by mind control", "a Gerudo's sagely power", "an orange disc"],
        "the Spirit Medallion",
        "item",
    ),
    "Triforce Piece": (
        ["a triumph fork", "cheese", "a gold fragment"],
        "a Piece of the Triforce",
        "item",
    ),
    "Magic Meter": (
        ["mystic training", "pixie dust", "a green rectangle"],
        "a Magic Meter",
        "item",
    ),
    "Double Defense": (
        ["a white outline", "damage decrease", "strengthened love"],
        "Double Defense",
        "item",
    ),
    ...
    "Progressive Hookshot": (
        ["Dampé's keepsake", "the Grapple Beam", "the BOING! chain"],
        "a Hookshot",
        "item",
    ),
    "Progressive Strength Upgrade": (
        ["power gloves", "metal mittens", "the heavy lifty"],
        "a Strength Upgrade",
        "item",
    ),
    "Progressive Scale": (["a deeper dive", "a piece of Zora"], "a Zora Scale", "item"),
    "Megaton Hammer": (
        ["the dragon smasher", "the metal mallet", "the heavy hitter"],
        "the Megaton Hammer",
        "item",
    ),
    ...
    "Kokiri Sword": (
        ["a butter knife", "a starter slasher", "a switchblade"],
        "the Kokiri Sword",
        "item",
    ),
    ...
    "Mirror Shield": (
        ["the reflective rampart", "Medusa's weakness", "a silvered surface"],
        "the Mirror Shield",
        "item",
    ),
    "Farores Wind": (
        ["teleportation", "a relocation rune", "a green ball", "a green gust"],
        "Farore's Wind",
        "item",
    ),
    "Nayrus Love": (
        ["a safe space", "an impregnable aura", "a blue barrier", "a blue crystal"],
        "Nayru's Love",
        "item",
    ),
    "Dins Fire": (["an inferno", "a heat wave", "a red ball"], "Din's Fire", "item"),
    "Fire Arrows": (
        ["the furnace firearm", "the burning bolts", "a magma missile"],
        "the Fire Arrows",
        "item",
    ),
    "Ice Arrows": (
        ["the refrigerator rocket", "the frostbite bolts", "an iceberg maker"],
        "the Ice Arrows",
        "item",
    ),
    "Blue Fire Arrows": (
        ["the icy hot rocket", "the blue bolts", "an iceberg destroyer"],
        "the Blue Fire Arrows",
        "item",
    ),
    "Light Arrows": (
        ["the shining shot", "the luminous launcher", "Ganondorf's bane", "the lighting bolts"],
        "the Light Arrows",
        "item",
    ),
    "Lens of Truth": (
        ["a lie detector", "a ghost tracker", "true sight", "a detective's tool"],
        "the Lens of Truth",
        "item",
    ),
    ...
    "Eponas Song": (
        ["an equestrian etude", "Malon's melody", "a ranch song"],
        "Epona's Song",
        "item",
    ),
    ...
    "Song of Storms": (
        ["Rain Dance", "a thunderstorm tune", "windmill acceleration"],
        "the Song of Storms",
        "item",
    ),
    "Minuet of Forest": (
        ["the song of tall trees", "an arboreal anthem", "a green spark trail"],
        "the Minuet of Forest",
        "item",
    ),
    "Bolero of Fire": (
        ["a song of lethal lava", "a red spark trail", "a volcanic verse"],
        "the Bolero of Fire",
        "item",
    ),
    "Serenade of Water": (
        ["a song of a damp ditch", "a blue spark trail", "the lake's lyric"],
        "the Serenade of Water",
        "item",
    ),
    "Requiem of Spirit": (
        ["a song of sandy statues", "an orange spark trail", "the desert ditty"],
    ...
    "Prelude of Light": (
        ["a luminous prologue melody", "a yellow spark trail", "the temple traveler"],
        "the Prelude of Light",
        "item",
    ),
    "Bottle": (["a glass container", "an empty jar", "encased air"], "a Bottle", "item"),
    "Rutos Letter": (
        ["a call for help", "the note that Mweeps", "an SOS call", "a fishy stationery"],
        "Ruto's Letter",
        "item",
    ),
    "Bottle with Milk": (
        ["cow juice", "a white liquid", "a baby's breakfast"],
        "a Milk Bottle",
        "item",
    ),
    "Bottle with Red Potion": (["a vitality vial", "a red liquid"], "a Red Potion Bottle", "item"),
    "Bottle with Green Potion": (
        ["a magic mixture", "a green liquid"],
        "a Green Potion Bottle",
        "item",
    ),
    "Bottle with Blue Potion": (
        ["an ailment antidote", "a blue liquid"],
    ...
    "Bottle with Blue Fire": (
        ["a conflagration canteen", "an icemelt jar"],
        "a Blue Fire Bottle",
        "item",
    ),
    ...
    "Bottle with Big Poe": (
        ["the spookiest ghost", "a sidequest spirit"],
        "a Big Poe Bottle",
        "item",
    ),
    "Stone of Agony": (["the shake stone", "the Rumble Pak (TM)"], "the Stone of Agony", "item"),
    "Gerudo Membership Card": (
        ["a girl club membership", "a desert tribe's pass"],
        "the Gerudo Card",
        "item",
    ),
    "Progressive Wallet": (
        ["a mo' money holder", "a gem purse", "a portable bank"],
        "a Wallet",
        "item",
    ),
    "Deku Stick Capacity": (
        ["a lumber rack", "more flammable twigs"],
        "Deku Stick Capacity",
        "item",
    ),
    "Deku Nut Capacity": (["more nuts", "flashbang storage"], "Deku Nut Capacity", "item"),
    "Heart Container": (
        ["a lot of love", "a Valentine's gift", "a boss's organ"],
        "a Heart Container",
        "item",
    ),
    ...
    "Rupees (200)": (
        ["a juicy jackpot", "a yellow gem", "a giant gem", "great wealth"],
        "a Huge Rupee",
        "item",
    ),
    ...
    "Zeldas Letter": (
        ["an autograph", "royal stationery", "royal snail mail"],
        "Zelda's Letter",
        "item",
    ),
    ...
    "Pocket Egg": (
        ["a Cucco container", "a Cucco, eventually", "a fowl youth"],
        "the Pocket Egg",
        "item",
    ),
    ...
    "Map (Deku Tree)": (
        ["an atlas of an ancient tree", "blueprints of an ancient tree"],
        "a Map of the Deku Tree",
        "item",
    ),
    "Map (Dodongos Cavern)": (
        ["an atlas of an immense cavern", "blueprints of an immense cavern"],
        "a Map of Dodongo's Cavern",
        "item",
    ),
    "Map (Jabu Jabus Belly)": (
        ["an atlas of the belly of a deity", "blueprints of the belly of a deity"],
        "a Map of Jabu Jabu's Belly",
        "item",
    ),
    "Map (Forest Temple)": (
        ["an atlas of a deep forest", "blueprints of a deep forest"],
        "a Map of the Forest Temple",
        "item",
    ),
    "Map (Fire Temple)": (
        ["an atlas of a high mountain", "blueprints of a high mountain"],
        "a Map of the Fire Temple",
        "item",
    ),
    "Map (Water Temple)": (
        ["an atlas of a vast lake", "blueprints of a vast lake"],
        "a Map of the Water Temple",
        "item",
    ),
    "Map (Shadow Temple)": (
        ["an atlas of the house of the dead", "blueprints of the house of the dead"],
        "a Map of the Shadow Temple",
        "item",
    ),
    "Map (Spirit Temple)": (
        ["an atlas of the goddess of the sand", "blueprints of the goddess of the sand"],
        "a Map of the Spirit Temple",
        "item",
    ),
    "Map (Bottom of the Well)": (
        ["an atlas of a shadow's prison", "blueprints of a shadow's prison"],
        "a Map of the Bottom of the Well",
        "item",
    ),
    "Map (Ice Cavern)": (
        ["an atlas of a frozen maze", "blueprints of a frozen maze"],
        "a Map of the Ice Cavern",
        "item",
    ),
    "Compass": (["a treasure tracker", "a magnetic needle"], "a Compass", "item"),
    "Compass (Deku Tree)": (
        ["a treasure tracker for an ancient tree", "a magnetic needle for an ancient tree"],
        "a Deku Tree Compass",
        "item",
    ),
    "Compass (Dodongos Cavern)": (
        ["a treasure tracker for an immense cavern", "a magnetic needle for an immense cavern"],
    ...
    "Compass (Forest Temple)": (
        ["a treasure tracker for a deep forest", "a magnetic needle for a deep forest"],
        "a Forest Temple Compass",
        "item",
    ),
    "Compass (Fire Temple)": (
        ["a treasure tracker for a high mountain", "a magnetic needle for a high mountain"],
        "a Fire Temple Compass",
        "item",
    ),
    "Compass (Water Temple)": (
        ["a treasure tracker for a vast lake", "a magnetic needle for a vast lake"],
    ...
    "Compass (Bottom of the Well)": (
        ["a treasure tracker for a shadow's prison", "a magnetic needle for a shadow's prison"],
        "a Bottom of the Well Compass",
        "item",
    ),
    "Compass (Ice Cavern)": (
        ["a treasure tracker for a frozen maze", "a magnetic needle for a frozen maze"],
    ...
    "SmallKey": (
        ["a tool for unlocking", "a dungeon pass", "a lock remover", "a lockpick"],
        "a Small Key",
        "item",
    ),
    ...
    "Boss Key (Forest Temple)": (
        ["a master of unlocking for a deep forest", "a master pass for a deep forest"],
        "the Forest Temple Boss Key",
        "item",
    ),
    "Boss Key (Fire Temple)": (
        ["a master of unlocking for a high mountain", "a master pass for a high mountain"],
        "the Fire Temple Boss Key",
        "item",
    ),
    "Boss Key (Water Temple)": (
        ["a master of unlocking for under a vast lake", "a master pass for under a vast lake"],
    ...
    "Boss Key (Ganons Castle)": (
        ["a master of unlocking for a conquered citadel", "a floating dungeon's master pass"],
    ...
    "Small Key Ring (Forest Temple)": (
        [
            "a toolbox for unlocking a deep forest",
            "a dungeon season pass for a deep forest",
            "a jingling ring for a deep forest",
            "a skeleton key for a deep forest",
        ],
        "a Forest Temple Small Key Ring",
        "item",
    ),
    "Small Key Ring (Fire Temple)": (
        [
            "a toolbox for unlocking a high mountain",
            "a dungeon season pass for a high mountain",
            "a jingling ring for a high mountain",
            "a skeleton key for a high mountain",
        ],
        "a Fire Temple Small Key Ring",
        "item",
    ),
    "Small Key Ring (Water Temple)": (
        [
            "a toolbox for unlocking a vast lake",
            "a dungeon season pass for under a vast lake",
            "a jingling ring for under a vast lake",
            "a skeleton key for under a vast lake",
        ],
        "a Water Temple Small Key Ring",
        "item",
    ),
    "Small Key Ring (Shadow Temple)": (
        [
            "a toolbox for unlocking the house of the dead",
            "a dungeon season pass for the house of the dead",
            "a jingling ring for the house of the dead",
            "a skeleton key for the house of the dead",
        ],
        "a Shadow Temple Small Key Ring",
        "item",
    ),
    "Small Key Ring (Spirit Temple)": (
        [
            "a toolbox for unlocking a goddess of the sand",
            "a dungeon season pass for a goddess of the sand",
            "a jingling ring for a goddess of the sand",
            "a skeleton key for a goddess of the sand",
        ],
        "a Spirit Temple Small Key Ring",
        "item",
    ),
    "Small Key Ring (Bottom of the Well)": (
        [
            "a toolbox for unlocking a shadow's prison",
            "a dungeon season pass for a shadow's prison",
            "a jingling ring for a shadow's prison",
            "a skeleton key for a shadow's prison",
        ],
        "a Bottom of the Well Small Key Ring",
        "item",
    ),
    "Small Key Ring (Gerudo Training Ground)": (
        [
            "a toolbox for unlocking the test of thieves",
            "a dungeon season pass for the test of thieves",
            "a jingling ring for the test of thieves",
            "a skeleton key for the test of thieves",
        ],
        "a Gerudo Training Ground Small Key Ring",
        "item",
    ),
    "Small Key Ring (Ganons Castle)": (
        [
            "a toolbox for unlocking a conquered citadel",
            "a dungeon season pass for a conquered citadel",
            "a jingling ring for a conquered citadel",
            "a skeleton key for a conquered citadel",
        ],
        "a Ganon's Castle Small Key Ring",
        "item",
    ),
    "Small Key Ring (Thieves Hideout)": (
        ["a deck of get out of jail free cards"],
        "a Jail Key Ring",
        "item",
    ),
    "Small Key Ring (Treasure Chest Game)": (
        ["an abundance of keys to becoming a winner"],
        "a Game Key Ring",
        "item",
    ),
    "Silver Rupee (Dodongos Cavern Staircase)": (
        ["an entry fee for an immense cavern", "a priced artifact from an immense cavern"],
        "a Silver Rupee for Dodongo's Cavern",
        "item",
    ),
    "Silver Rupee (Ice Cavern Spinning Scythe)": (
        ["an entry fee for a frozen maze", "a priced artifact from a frozen maze"],
        "a Silver Rupee for the Ice Cavern",
        "item",
    ),
    ...
    "Silver Rupee (Bottom of the Well Basement)": (
        ["an entry fee for a shadow's prison", "a priced artifact from a shadow's prison"],
        "a Silver Rupee for the Bottom of the Well",
        "item",
    ),
    "Silver Rupee (Shadow Temple Scythe Shortcut)": (
        ["an entry fee for the house of the dead", "a priced artifact from the house of the dead"],
        "a Silver Rupee for the Shadow Temple",
        "item",
    ),
    ...
    "Silver Rupee (Gerudo Training Ground Slopes)": (
        ["an entry fee for the test of thieves", "a priced artifact from the test of thieves"],
        "a Silver Rupee for the Gerudo Training Ground",
        "item",
    ),
    ...
    "Silver Rupee (Spirit Temple Child Early Torches)": (
        ["an entry fee for a goddess of the sand", "a priced artifact from a goddess of the sand"],
        "a Silver Rupee for the Spirit Temple",
        "item",
    ),
    ...
    "Silver Rupee (Ganons Castle Spirit Trial)": (
        ["an entry fee for a conquered citadel", "a priced artifact from a conquered citadel"],
        "a Silver Rupee for Ganon's Castle",
        "item",
    ),
    ...
    "Silver Rupee Pouch (Dodongos Cavern Staircase)": (
        [
            "a silver lining for an immense cavern",
            "a stash of silver shekels for an immense cavern",
        ],
        "a Pouch of Silver Rupees for Dodongo's Cavern",
        "item",
    ),
    "Silver Rupee Pouch (Ice Cavern Spinning Scythe)": (
        ["a silver lining for a frozen maze", "a stash of silver shekels for a frozen maze"],
        "a Pouch of Silver Rupees for the Ice Cavern",
        "item",
    ),
    ...
    "Silver Rupee Pouch (Bottom of the Well Basement)": (
        [
            "a silver lining for a shadow's prison",
            "a stash of silver shekels for a shadow's prison",
        ],
        "a Pouch of Silver Rupees for the Bottom of the Well",
        "item",
    ),
    "Silver Rupee Pouch (Shadow Temple Scythe Shortcut)": (
        [
            "a silver lining for the house of the dead",
            "a stash of silver shekels for the house of the dead",
        ],
        "a Pouch of Silver Rupees for the Shadow Temple",
        "item",
    ),
    ...
    "Silver Rupee Pouch (Gerudo Training Ground Slopes)": (
        [
            "a silver lining for the test of thieves",
            "a stash of silver shekels for the test of thieves",
        ],
        "a Pouch of Silver Rupees for the Gerudo Training Ground",
        "item",
    ),
    ...
    "Silver Rupee Pouch (Spirit Temple Child Early Torches)": (
        [
            "a silver lining for a goddess of the sand",
            "a stash of silver shekels for a goddess of the sand",
        ],
        "a Pouch of Silver Rupees for the Spirit Temple",
        "item",
    ),
    ...
    "Silver Rupee Pouch (Ganons Castle Spirit Trial)": (
        [
            "a silver lining for a conquered citadel",
            "a stash of silver shekels for a conquered citadel",
        ],
        "a Pouch of Silver Rupees for Ganon's Castle",
        "item",
    ),
    ...
    "KeyError": (
        ["something mysterious", "an unknown treasure"],
    ...
    "Arrows (30)": (
        ["plenty of danger darts", "plenty of sharp shafts"],
        "Arrows (30 pieces)",
        "item",
    ),
    ...
    "Ice Trap": (
        ["a gift from Ganon", "a chilling discovery", "frosty fun"],
        "an Ice Trap",
        "item",
    ),
    ...
    "Bombchus": (
        ["mice bombs", "proximity mice", "wall crawlers", "trail blazers"],
        "Bombchus",
        "item",
    ),
    "Bombchus (5)": (
        ["a few mice bombs", "a few proximity mice", "a few wall crawlers", "a few trail blazers"],
        "Bombchus (5 pieces)",
        "item",
    ),
    "Bombchus (10)": (
        ["some mice bombs", "some proximity mice", "some wall crawlers", "some trail blazers"],
        "Bombchus (10 pieces)",
        "item",
    ),
    "Bombchus (20)": (
    ...
    "Deku Nuts (5)": (
        ["some nuts", "some flashbangs", "some scrub spit"],
        "Deku Nuts (5 pieces)",
        "item",
    ),
    "Deku Nuts (10)": (
        ["lots-o-nuts", "plenty of flashbangs", "plenty of scrub spit"],
        "Deku Nuts (10 pieces)",
        "item",
    ),
    "Deku Seeds (30)": (["catapult ammo", "lots-o-seeds"], "Deku Seeds (30 pieces)", "item"),
    "Gold Skulltula Token": (
        ["proof of destruction", "an arachnid chip", "spider remains", "one percent of a curse"],
    ...
    "Song from Ocarina of Time": (
        ...
        "the song taught by the #Ocarina of Time# is",
        ["song", "sometimes"],
    ),
    "Song from Royal Familys Tomb": (
        ["#ReDead in the royal tomb# guard", "the #Composer Brothers wrote#"],
        "the song written in the #royal tomb# is",
        ["song", "sometimes"],
    ),
    "Sheik in Forest": (
        "#in a meadow# Sheik teaches",
        "in the #Sacred Forest Meadow#, Sheik teaches",
        ["song", "sometimes"],
    ),
    "Sheik at Temple": (
        "Sheik waits at a #monument to time# to teach",
        "the #Temple of Time# chimes with the music of",
        ["song", "sometimes"],
    ),
    "Sheik in Crater": (
        "the #crater's melody# is",
        "Sheik waits in the #Death Mountain Crater# to teach",
        ["song", "sometimes"],
    ),
    "Sheik in Ice Cavern": (
        "the #frozen cavern# echoes with",
        "the #Ice Cavern# corridors ring with",
        ["song", "sometimes"],
    ),
    "Sheik in Kakariko": (
        "a #ravaged village# mourns with",
        "amidst flames in #Kakariko Village#, Sheik gives",
        ["song", "sometimes"],
    ),
    "Sheik at Colossus": (
        "a hero ventures #beyond the wasteland# to learn",
        "the #Desert Colossus# sands echo with",
        ["song", "sometimes"],
    ),
    "Market 10 Big Poes": (
    ...
    "Deku Theater Skull Mask": (
        "the #Skull Mask# yields",
        "wearing the #Skull Mask in the Deku Theater# rewards",
        ["overworld", "sometimes"],
    ),
    "Deku Theater Mask of Truth": (
        "showing a #truthful eye to the crowd# rewards",
        "showing the #Mask of Truth in the Deku Theater# rewards",
        ["overworld", "sometimes"],
    ),
    ...
    "DMT Biggoron": (
        "#Biggoron# crafts",
        "showing the #Claim Check to Biggoron# rewards",
        ["overworld", "sometimes"],
    ),
    ...
    "Kak Anju as Child": (
        ["#wrangling roosters# rewards", "#chucking chickens# gifts"],
    ...
    "LH Sun": (
        "staring into #the sun# grants",
        "shooting #the sun# grants",
        ["overworld", "sometimes"],
    ),
    "Market Treasure Chest Game Reward": (
        ["#gambling in the market# grants", "there is a #1/32 chance# to win"],
        "winning the #treasure chest game# rewards",
        ["overworld", "sometimes"],
    ),
    "GF HBA 1500 Points": (
    ...
    "HC GS Storms Grotto": (
        "a #spider behind a muddy wall# in a grotto holds",
        None,
        ["overworld", "sometimes"],
    ),
    "HF GS Cow Grotto": (
        "a #spider behind webs# in a grotto holds",
        None,
        ["overworld", "sometimes"],
    ),
    "HF Cow Grotto Cow": (
    ...
    "ZF GS Hidden Cave": (
        "a spider high #above the icy waters# holds",
        None,
        ["overworld", "sometimes"],
    ),
    "Wasteland Chest": (
        ["#deep in the wasteland# is", "beneath #the sands#, flames reveal"],
        "the #Haunted Wasteland torches# reveal",
        ["overworld", "sometimes"],
    ),
    "Wasteland GS": ("a #spider in the wasteland# holds", None, ["overworld", "sometimes"]),
    "Graveyard Royal Familys Tomb Chest": (
        ["#flames in the royal tomb# reveal", "the #Composer Brothers hid#"],
        "#lighting flames in the royal tomb# rewards",
        ["overworld", "sometimes"],
    ),
    "ZF Bottom Freestanding PoH": (
        "#under the icy waters# lies",
        "at the #bottom of Zora's Fountain# lies",
        ["overworld", "sometimes"],
    ),
    "GC Pot Freestanding PoH": (
        "spinning #Goron pottery# contains",
        "the #Goron Pot's happy face# spits out",
        ["overworld", "sometimes"],
    ),
    "ZD King Zora Thawed": (
    ...
    "DMC Deku Scrub": (
        "a single #scrub in the crater# sells",
        "a lone #scrub in Death Mountain Crater# sells",
        ["overworld", "sometimes"],
    ),
    "DMC GS Crate": (
        "a spider under a #crate in the crater# holds",
        None,
        ["overworld", "sometimes"],
    ),
    "LW Target in Woods": (
        "shooting a #target in the woods# grants",
        None,
        ["overworld", "sometimes"],
    ),
    "ZR Frogs in the Rain": (
        "#frogs in a storm# gift",
        "playing #Song of Storms to Frogs# rewards",
        ["overworld", "sometimes"],
    ),
    "LH Lab Dive": (
        "a #diving experiment# is rewarded with",
        "a #lakeside lab diving experiment# rewards",
        ["overworld", "sometimes"],
    ),
    ...
    "Gerudo Training Ground Underwater Silver Rupee Chest": (
        ...
        "obtaining the #underwater silver rupees in Gerudo Training Ground# rewards",
        ["dungeon", "sometimes"],
    ),
    ...
    "Gerudo Training Ground Maze Path Final Chest": (
        ...
        "#Gerudo Training Ground final reward# contains",
        ["dungeon", "sometimes"],
    ),
    ...
    "Spirit Temple Mirror Shield Chest": (
        "upon the #Colossus's left hand# is",
        None,
        ["dungeon", "sometimes"],
    ),
    ...
    "Shadow Temple MQ Bomb Flower Chest": (
        "a #grasping ghoul surrounded by Bomb Flowers# guards",
        "the #Dead Hand surrounded by Bomb Flowers# guards",
        ["dungeon", "sometimes"],
    ),
    ...
    "HF Ocarina of Time Retrieval": (
        "during her escape, #Princess Zelda# entrusted you with both...^",
        "the #Ocarina of Time# rewards both...^",
        "dual",
    ),
    "HF Valley Grotto": ("in a grotto with a #spider and a cow# you will find...^", None, "dual"),
    "Market Bombchu Bowling Rewards": (
        "at the #Bombchu Bowling Alley#, you will be rewarded with...^",
        None,
        "dual",
    ),
    ...
    "LH Adult Bean Destination Checks": (
        "#riding the bean in Lake Hylia# leads to...^",
        None,
        "dual",
    ),
    "GV Pieces of Heart Ledges": (
        "within the #valley#, the crate and waterfall conceal...^",
        None,
        "dual",
    ),
    "GF Horseback Archery Rewards": ("the #Gerudo Horseback Archery# rewards...^", None, "dual"),
    "Colossus Nighttime GS": (
        "#at the Desert Colossus#, skulltulas at night hold...^",
        None,
        "dual",
    ),
    "Graveyard Dampe Race Rewards": ("racing #Dampé's ghost# rewards...^", None, "dual"),
    "Graveyard Royal Family Tomb Contents": (
        "inside the #Royal Family Tomb#, you will find...^",
        None,
        "dual",
    ),
    "DMC Child Upper Checks": (
        "in the #crater, a spider in a crate and a single scrub# guard...^",
        None,
        "dual",
    ),
    "Haunted Wasteland Checks": (
        "deep in the #wasteland a spider and a chest# hold...^",
        None,
        "dual",
    ),
    "Castle Fairy Checks": (
        "Great Fairies outside #Hyrule and Ganon's castles# reward...^",
        None,
        "dual",
    ),
    "King Zora Items": (
        "#unfreezing King Zora and giving him the Prescription# rewards...^",
        None,
        "dual",
    ),
    "Deku Tree MQ Basement GS": (
        "in the back of the #basement of the Great Deku Tree# two spiders hold...^",
        None,
        "dual",
    ),
    "Dodongos Cavern Upper Business Scrubs": (
        "deep in #Dodongo's Cavern a pair of scrubs# sell...^",
        None,
        "dual",
    ),
    "Dodongos Cavern MQ Larvae Room": (
        "amid #larvae in Dodongo's Cavern# a chest and a spider hold...^",
        None,
        "dual",
    ),
    "Fire Temple Lower Loop": (
        "under the #entrance of the Fire Temple# a blocked path leads to...^",
        None,
        "dual",
    ),
    ...
    "Water Temple River Loop Chests": (
        "#chests past a shadowy fight# in the Water Temple hold...^",
        "#chests past Dark Link# in the Water Temple hold...^",
        "dual",
    ),
    "Water Temple River Checks": ("in the #river in the Water Temple# lies...^", None, "dual"),
    "Water Temple North Basement Checks": (
        "the #northern basement of the Water Temple# contains...^",
        None,
        "dual",
    ),
    ...
    "Water Temple MQ Lower Checks": (
        "#a chest and a crate in locked basements# in the Water Temple hold...^",
        None,
        "dual",
    ),
    "Spirit Temple Colossus Hands": (
        "upon the #Colossus's right and left hands# lie...^",
        None,
        "dual",
    ),
    "Spirit Temple Child Lower": (
        "between the #crawl spaces in the Spirit Temple# chests contain...^",
        None,
        "dual",
    ),
    "Spirit Temple Child Top": (
        "on the path to the #right hand of the Spirit Temple# a chest and a spider hold...^",
        None,
        "dual",
    ),
    "Spirit Temple Adult Lower": (
        "past a #silver block in the Spirit Temple# boulders and a melody conceal...^",
        None,
        "dual",
    ),
    "Spirit Temple MQ Child Top": (
        "on the path to the #right hand of the Spirit Temple# a chest and a spider hold respectively...^",
        None,
        "dual",
    ),
    "Spirit Temple MQ Symphony Room": (
        "#the symphony room# in the Spirit Temple protects...^",
        None,
        "dual",
    ),
    "Spirit Temple MQ Throne Room GS": (
        "in the #nine thrones room# of the Spirit Temple spiders hold...^",
        None,
        "dual",
    ),
    "Shadow Temple Invisible Blades Chests": (
        "an #invisible spinning blade# in the Shadow Temple guards...^",
        None,
        "dual",
    ),
    "Shadow Temple Single Pot Room": (
        "a room containing #a single skull-shaped pot# holds...^",
        "a room containing a #large pot in the Shadow Temple# holds...^",
        "dual",
    ),
    "Shadow Temple Spike Walls Room": (
        "#wooden walls# in the Shadow Temple hide...^",
        None,
        "dual",
    ),
    "Shadow Temple MQ Upper Checks": (
        "#before the Truth Spinner gap# in the Shadow Temple locked chests contain...^",
        None,
        "dual",
    ),
    ...
    "Bottom of the Well Inner Rooms GS": (
        "in the #central rooms of the well# spiders hold...^",
        None,
        "dual",
    ),
    ...
    "Bottom of the Well MQ Basement": (
        "in the #depths of the well# a spider and a chest hold...^",
        None,
        "dual",
    ),
    ...
    "Ganons Castle Spirit Trial Chests": (
        "#within the Spirit Trial#, chests contain...^",
        None,
        "dual",
    ),
    "Queen Gohma Rewards": (
    ...
    "LW Gift from Saria": (
        ["a #potato hoarder# holds", "a rooty tooty #flutey cutey# gifts"],
        "#Saria's Gift# is",
        "exclude",
    ),
    ...
    "LW Ocarina Memory Game": (
        ["the prize for a #game of Simon Says# is", "a #child sing-a-long# holds"],
    ...
    "Jabu Jabus Belly MQ Map Chest": (
        ["#pop rocks# hide", "an #explosive palate# holds"],
    ...
    "Water Temple Longshot Chest": (
        ["#facing yourself# reveals", "a #dark reflection# of yourself guards"],
        "#Dark Link# guards",
        "exclude",
    ),
    ...
    "Shadow Temple Invisible Floormaster Chest": (
        "the #Floormaster in the house of the dead# guards",
        "the #Floormaster in the Shadow Temple# guards",
        "exclude",
    ),
    ...
    "Bottom of the Well Lens of Truth Chest": (
        ["the well's #grasping ghoul# hides", "a #nether dweller in the well# holds"],
    ...
    "ToT Reward from Rauru": (
        "#coming of age# grants",
        "beyond the #Door of Time# waits",
        "exclude",
    ),
    ...
    "1002": (
        "${12 68 79}They say that monarchy is a terrible system of governance.",
        None,
        "junk",
    ),  # sfx: Zelda gasp
    ...
    "1008": (
        "Remember when Ganon was a blue pig?^I remember.",
        None,
        "junk",
    ),  # ref: A Link to the Past
    ...
    "1013": (
        ...
        None,
        "junk",
    ),  # ref: Kaepora Gaebora (the owl)
    "1014": (
        "Gorons are stupid. They eat rocks. Except, apparently, the big rock blocking Dodongo's Cavern.",
        None,
        "junk",
    ),
    ...
    "1045": (
        ...
        None,
        "junk",
    ),  # ref: people often say that Okami feels and plays like a Zelda game
    ...
    "1059": (
        "They say that looking into darkness may find darkness looking back into you.",
        None,
        "junk",
    ),  # ref: Nietzsche
    ...
    "1066": (
        "You're playing a Randomizer. I'm randomized!^${12 48 31}Here's a random number:  #4#.&Enjoy your Randomizer!",
        None,
        "junk",
    ),  # ref: xkcd comic / sfx: get small item from chest
    ...
    "1073": (
        ...
        None,
        "junk",
    ),  # ref: Gossip Stone in Phantom Hourglass
    "1074": (
        "They say that if you don't use your slingshot at all when you play the slingshot minigame, the owner gets upset with you.",
        None,
        "junk",
    ),
    "1075": (
    ...
    "1076": (
        "They say it's 106 miles to Hyrule Castle, we have half a bar of magic, it's dark, and we're wearing sunglasses.",
        None,
        "junk",
    ),  # ref: Blues Brothers
    "1078": (
        "It would be a shame if something... unfortunate... were to happen to you.^Have you considered saving lately?",
        None,
        "junk",
    ),  # ref: meme
    "1079": (
        "They say that something wonderful happens when playing the Song of Storms after planting a magic bean.",
        None,
        "junk",
    ),
    "1080": (
        "Long time watcher, first time player. Greetings from Termina. Incentive goes to Randobot's choice.",
        None,
        "junk",
    ),  # ref: GDQ meme
    "1081": (
        "No matter what happens...Do not give up, do not complain, and do NOT stay up all night playing!",
        None,
        "junk",
    ),  # ref: Wind Waker
    "1082": (
        "That's a nice wall you got there. Would be a shame if I just... clipped right through that.",
        None,
        "junk",
    ),
    "1083": (
        "Ganondorf used to be an adventurer like me, but then he took a light arrow to the knee.",
        None,
        "junk",
    ),  # ref: Skyrim
    "1084": (
        "They say that the easiest way to kill Peahats is using Din's Fire while they're grounded.",
        None,
        "junk",
    ),
    "1085": (
        "They say that the castle guards' routes have major security vulnerabilities.",
        None,
        "junk",
    ),
    "1086": (
        "They say that Epona is an exceptional horse. Able to clear canyons in a single bound.",
        None,
        "junk",
    ),
    "1087": (
        "They say only one heart piece in all of Hyrule will declare the holder a winner.",
        None,
        "junk",
    ),
    "1088": (
        "Are you stuck? Try asking for help in our Discord server or check out our Wiki!",
        None,
        "junk",
    ),
    "1089": (
        "You would be surprised at all the things you can Hookshot in the Spirit Temple!",
        None,
        "junk",
    ),
    "1090": (
        "I once glued a set of false teeth to the Boomerang.^${12 39 c7}That came back to bite me.",
        None,
        "junk",
    ),  # sfx: Ganondorf laugh
    "1091": (
        "They say that most of the water in Hyrule flows through King Zora's buttocks.",
        None,
        "junk",
    ),
    "1092": (
    ...
    "1093": (
        'They say that you must read the names of "Special Deal" items in shops carefully.',
        None,
        "junk",
    ),
    "1094": (
        "Did you know that the Boomerang instantly stuns Phantom Ganon's second form?",
        None,
        "junk",
    ),
    "1095": (
        "I came here to chew bubblegum and play rando. And I'm all out of bubblegum.",
        None,
        "junk",
    ),  # ref: They Live
    "1096": (
        "Did you know that Stalchildren leave you alone when wearing the Bunny Hood?",
        None,
        "junk",
    ),
    "1097": (
        "This Gossip Stone Is Dedicated to Those Who Perished Before Ganon Was Defeated.",
        None,
        "junk",
    ),
    "1098": (
        "Did you know that Blue Fire destroys mud walls and detonates Bomb Flowers?",
        None,
        "junk",
    ),
    "1099": (
        "Are you sure you want to play this? Wanna go get some tacos or something?",
        None,
        "junk",
    ),
    "1100": (
        "What did Zelda suggest that Link do when diplomacy didn't work?^${12 39 C7}Triforce.",
        None,
        "junk",
    ),  # sfx: Ganondorf laugh
    "1101": (
    ...
    "1102": (
        "Hi @, we've been trying to reach you about your horse's extended warranty.",
        None,
        "junk",
    ),
    "1103": (
        "Ganondorf brushes his rotten teeth with salted slug flavoured tooth paste!",
        None,
        "junk",
    ),  # ref: Banjo Kazooie
    "1104": (
        "I'm Commander Shepard, and this is my favorite Gossip Stone in Hyrule!",
        None,
        "junk",
    ),  # ref: Mass Effect
    ...
    "1112": (
        ...
        None,
        "junk",
    ),  # ref: Jojo's Bizarre Adventure
    "1113": ("They say you can cut corners to get to your destination faster.", None, "junk"),
    "1114": (
        "Three things are certain: death, taxes, and forgetting a check.",
        None,
        "junk",
    ),  # ref: Benjamin Franklin, allegedly
    "1115": ("Have you thought about going where the items are?^Just saying.", None, "junk"),
    "1116": (
        "They say that the true reward is the friends we made along the way.",
        None,
        "junk",
    ),  # ref: common meme with unknown origins
    ...
    "1122": (
        "Fell for it, didn't you, fool? Junk hint cross split attack!",
        None,
        "junk",
    ),  # ref: Jojo's Bizarre Adventure
    ...
    "1130": (
        "The hint stone you have dialed&has been disconnected.",
        None,
        "junk",
    ),  # ref: telephone error message
    ...
    "1139": (
        "They say that Mido is easily confused by sick flips.",
        None,
        "junk",
    ),  # ref: Mido Skip
    "1140": (
        "They say that the path to Termina is a one-way trip.",
        None,
        "junk",
    ),  # ref: Majora's Mask
    ...
    "1146": (
        "On second thought, let's not go to Hyrule Castle. 'Tis a silly place.",
        None,
        "junk",
    ),  # ref: Monty Python
    ...
    "1156": (
        ...
        None,
        "junk",
    ),  # ref: Nintendo (various games and platforms)
    ...
    "1159": (
        "Twenty-three is number one!^And thirty-one is number two!",
        None,
        "junk",
    ),  # ref: Deku Scrubs in Deku Tree
    "1160": ("Ya ha ha! You found me!", None, "junk"),  # ref: Breath of the Wild
    "1161": ("Do you like Like Likes?", None, "junk"),
    "1162": (
        'Next you\'ll say:^"Why am I still reading these?"',
        None,
        "junk",
    ),  # ref: Jojo's Bizarre Adventure
    ...
    "1170": (
        "They say that there is an alcove with a Recovery Heart behind the lava wall in Dodongo's Cavern.",
        None,
        "junk",
    ),
    "1171": ("Having regrets? Reset without saving!", None, "junk"),
    "1172": (
        "Did you know that Gorons understood SRM long before speedrunners did?",
        None,
        "junk",
    ),  # ref: Goron City murals
    "1173": (
        "Did you know that the Discord server has a public Plandomizer library?",
        None,
        "junk",
    ),
    ...
    "1176": (
        "${12 68 08}Aah! You startled me!",
        None,
        "junk",
    ),  # sfx: adult Link scream (when falling)
    "1178": (
        "Use Multiworld to cross the gaps between worlds and engage in jolly co-operation!",
        None,
        "junk",
    ),  # ref: Dark Souls
    "1179": ("${12 68 51}What in tarnation!", None, "junk"),  # sfx: Talon surprised at being woken
    "1180": (
        "Press \u00a5\u00a5\u00a6\u00a6\u00a7\u00a8\u00a7\u00a8\u00a0\u009f to warp to&the credits.",
        None,
        "junk",
    ),  # ref: Konami Code
    "1181": (
    ...
    "1183": (
        "I am the King of Gossip Stones, but fear not - I have the common touch! That means I can make conversation with everyone^from foreign dignitaries to the lowliest bumpkin - such as yourself!",
        None,
        "junk",
    ),  # ref: Dragon Quest XI
    "1184": (
        "I am @, hero of the Gossip Stones! Hear my name and tremble!",
        None,
        "junk",
    ),  # ref: Link the Goron
    "1185": (
        "Having trouble defeating Dark Link?^Look away from him while holding Z-Target and then when Dark Link walks up behind you, strafe sideways and slash your sword.",
        None,
        "junk",
    ),
    "1186": (
        "They say that if Link could say a few words, he'd be a better public speaker.",
        None,
        "junk",
    ),
    "1187": (
        "Did you know that you only need to play the Song of Time to open the Door of Time? The Spiritual Stones are not needed.",
        None,
        "junk",
    ),
    "1188": (
        "Where did Anju meet her lover?^${12 39 C7}At a Kafei.",
        None,
        "junk",
    ),  # ref: Majora's Mask / sfx: Ganondorf laugh
    "1189": (
        "Did you know that you can access the Fire Temple boss door without dropping the pillar by using the Hover boots?",
        None,
        "junk",
    ),
    "1190": ("Key-locked in Fire Temple? Maybe Volvagia has your Small Key.", None, "junk"),
    "1191": (
        "Expired Spoiler Log? Don't worry! The OoTR Discord staff can help you out.",
        None,
        "junk",
    ),
    "1192": ("Try holding a D-pad button on the item screen.", None, "junk"),
    "1193": (
        "Did you know that in the Forest Temple you can reach the alcove in the block push room with Hover Boots?",
        None,
        "junk",
    ),
    "1194": ("Dodongo's Cavern is much easier and faster to clear as Adult.", None, "junk"),
    "1195": (
        "Did you know that the solution to the Truth Spinner in Shadow Temple is never one of the two positions closest to the initial position?",
        None,
        "junk",
    ),
    "1196": (
        "Did you know that the Kokiri Sword is as effective as Deku Sticks against Dead Hand?",
        None,
        "junk",
    ),
    "1197": (
        "Did you know that Ruto is strong enough to defeat enemies and activate ceiling switches inside Jabu Jabu's Belly?",
        None,
        "junk",
    ),
    "1198": (
        "Did you know that Barinade, Volvagia and Twinrova hard require the Boomerang, Megaton Hammer and Mirror Shield, respectively?",
        None,
        "junk",
    ),
    "1199": ("Did you know that Dark Link's max health is equal to @'s max health?", None, "junk"),
    "1200": (
        "Did you know that you can reach the invisible Hookshot target before the fans room in Shadow Temple with just the Hookshot if you backflip onto the chest?",
        None,
        "junk",
    ),
    "1201": ("${12 68 54}Objection!", None, "junk"),  # ref: Ace Attorney / sfx: Ingo's BWAAAAAH
    "1202": (
        "They say that in the castle courtyard you can see a portrait of a young Talon.",
        None,
        "junk",
    ),  # ref: Talon = Mario joke
    "1203": (
        "They say that Phantom Ganon is a big Louisa May Alcott fan.",
        None,
        "junk",
    ),  # ref: The Poe Sisters are named after characters from one of her novels
    "1204": (
        "Have you found all 41 Gossip Stones?^Only 40 of us give hints.",
        None,
        "junk",
    ),  # The 41th stone is the Lake Hylia water level stone
    "1205": (
        "It's time for you to look inward and begin asking yourself the big questions:^How did Medigoron get inside that hole, and how does he get out for the credits?",
        None,
        "junk",
    ),  # ref: Avatar The Last Airbender
    "1206": ("They say that Jabu Jabu is no longer a pescetarian in Master Quest.", None, "junk"),
    "1207": (
        'Why are the floating skulls called "Bubbles" and the floating bubbles "Shaboms"?',
        None,
        "junk",
    ),
    ...
    "1210": (
        "Did you know that you can Boomerang items that are freestanding Heart Pieces in the unrandomized game?",
        None,
        "junk",
    ),
    "1211": ("Did you know that ReDead won't attack if you walk very slowly?", None, "junk"),
    "1212": (
        "Did you know that ReDead and Gibdo have their own version of Sun's Song that freezes you?",
        None,
        "junk",
    ),
    "1213": (
        ...
        None,
        "junk",
    ),  # ref: Frogs 2 / sfx: Frogs
    "1214": ("${12 28 A2}Help! I'm melting away!", None, "junk"),  # sfx: red ice melting
    "1215": (
        "${12 38 80}Eek!^I'm a little shy...",
        None,
        "junk",
    ),  # sfx: Scrub hurt/stunned by Link
    "1216": (
        "Master, there is a 0 percent chance that this hint is useful in any way.",
        None,
        "junk",
    ),  # ref: Skyward Sword
    ...
    "1219": (
        "${12 68 31}Don't forget to stand up and stretch regularly.",
        None,
        "junk",
    ),  # sfx: child Link stretching and yawning
    "1220": (
        "Remember that time you did that really embarrassing thing?^${12 68 3A}Yikes.",
        None,
        "junk",
    ),  # sfx: child Link fall damage
    "1221": (
        "@ tries to read the Gossip Stone...^${12 48 06}but he's standing on the wrong side of it!",
        None,
        "junk",
    ),  # ref: Dragon Quest XI / sfx: error (e.g. trying to equip an item as the wrong age)
    "1222": (
        "Plandomizer is a pathway to many abilities some consider to be... unnatural.",
        None,
        "junk",
    ),  # ref: Star Wars
    "1223": (
        "Did you know that you can have complete control over the item placement, item pool, and more, using Plandomizer?",
        None,
        "junk",
    ),
    "1224": ("They say that the earth is round.^Just like pizza.", None, "junk"),
    "1225": (
        "${12 68 62}Keeeyaaaah!^What is this?! A Hylian?!",
        None,
        "junk",
    ),  # ref: Ruto meeting Big Octo / sfx: Ruto screaming
    "1226": (
        "For you, the day you read this hint was the most important day of your life.^But for me, it was Tuesday.",
        None,
        "junk",
    ),  # ref: Street Fighter (the movie)
    "1227": ("Did you know that Barinade is allergic to bananas?", None, "junk"),
    "1228": (
        'Have you seen my dodongo? Very large, eats everything, responds to "King".^Call Darunia in Goron City if found. Huge rupee reward!',
        None,
        "junk",
    ),
    "1229": (
        "Having trouble breathing underwater?^Have you tried wearing more BLUE?",
        None,
        "junk",
    ),
    "1230": (
        "Hi! I'm currently on an exchange program from Termina.^They say that East Clock Town is on the way of the hero.",
        None,
        "junk",
    ),  # ref: Majora's Mask
    "1231": (
        "Why are you asking me? I don't have any answers! I'm just as confused as you are!",
        None,
        "junk",
    ),
    "1232": (
        "What do you call a group of Gorons?^${12 39 C7}A rock band.",
        None,
        "junk",
    ),  # sfx: Ganondorf laugh
    "1233": (
        "When the moon hits Termina like a big pizza pie that's game over.",
        None,
        "junk",
    ),  # ref: That's Amore by Dean Martin + Majora's Mask
    "1234": (
        "Ganondorf doesn't specialize in hiding items, nor in keeping secrets for that matter.",
        None,
        "junk",
    ),
    "1235": (
        "While you're wasting time reading this hint, the others are playing the seed.",
        None,
        "junk",
    ),
    "1236": (
        "Have you ever tried hammering the ground or wall in a room with Torch Slugs, Flare Dancers, Tektites, Walltulas, Scrubs or Deku Babas?",
        None,
        "junk",
    ),
    "1237": (
        "Did you know that there's a 1/201 chance per Rupee that the Zora from the diving minigame tosses a 500 Rupee?^Keep winning and the odds go up!",
        None,
        "junk",
    ),
    "1238": (
        "J = 0;&while J < 10;&   Press \u009f;&   J++;^   Press \u009f;&   J++;^   Press \u009f;&   J++;^   Press \u009f;&   J++;^   Press \u009f;&   J++;^   Press \u009f;&   J++;^   Press \u009f;&   J++;^   Press \u009f;&   J++;^   Press \u009f;&   J++;^   Press \u009f;^break;",
        None,
        "junk",
    ),  # \u009F = A button
    ...
    "ganonBK_triforce": (
        "given to the Hero once the #Triforce# is completed",
        None,
        "ganonBossKey",
    ),
    "Spiritual Stone Text Start": ("3 Spiritual Stones found in Hyrule...", None, "altar"),
    "Child Altar Text End": (
        "\x13\x07Ye who may become a Hero...&Stand with the Ocarina and&play the Song of Time.",
        None,
        "altar",
    ),
    "Adult Altar Text Start": (
        "When evil rules all, an awakening&voice from the Sacred Realm will&call those destined to be Sages,&who dwell in the \x05\x41five temples\x05\x40.",
        None,
        "altar",
    ),
...
def hint_exclusions(world: World, clear_cache: bool = False) -> list[str]:
    ...
    for hint in location_hints:
        if any(item in hint.type for item in ["dual", "dual_always"]):

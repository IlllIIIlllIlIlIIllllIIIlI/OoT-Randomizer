...
class Item:
    def __init__(self, name: str = "", world: Optional[World] = None, event: bool = False) -> None:
        ...
        self.alias_id: Optional[int] = (
            ItemInfo.solver_ids[escape_name(self.alias[0])] if self.alias else None
        )
    ...
    def smallkey(self) -> bool:
        return (
            self.type == "SmallKey" or self.type == "HideoutSmallKey" or self.type == "TCGSmallKey"
        )
    ...
    def dungeonitem(self) -> bool:
        return (
            self.smallkey or self.bosskey or self.map or self.compass or self.type == "SilverRupee"
        )
    ...
    def unshuffled_dungeon_item(self) -> bool:
        ...
        return (
            (
                self.type == "SmallKey"
                and self.world.settings.shuffle_smallkeys in ("remove", "vanilla", "dungeon")
            )
            or (
                self.type == "HideoutSmallKey"
                and self.world.settings.shuffle_hideoutkeys == "vanilla"
            )
            or (
                self.type == "TCGSmallKey"
                and self.world.settings.shuffle_tcgkeys in ("remove", "vanilla")
            )
            or (
                self.type == "BossKey"
                and self.world.settings.shuffle_bosskeys in ("remove", "vanilla", "dungeon")
            )
            or (
                self.type == "GanonBossKey"
                and self.world.settings.shuffle_ganon_bosskey in ("remove", "vanilla", "dungeon")
            )
            or (
                (self.map or self.compass)
                and (
                    self.world.settings.shuffle_mapcompass
                    in ("remove", "startwith", "vanilla", "dungeon")
                )
            )
            or (
                self.type == "SilverRupee"
                and self.world.settings.shuffle_silver_rupees in ("remove", "vanilla", "dungeon")
            )
            or (
                self.type == "DungeonReward"
                and self.world.settings.shuffle_dungeon_rewards in ("vanilla", "reward", "dungeon")
            )
        )
    ...
    def majoritem(self) -> bool:
        ...
        if self.type == "Token":
            return (
                self.world.settings.bridge == "tokens"
                or self.world.settings.shuffle_ganon_bosskey == "tokens"
                or (
                    self.world.settings.shuffle_ganon_bosskey == "on_lacs"
                    and self.world.settings.lacs_condition == "tokens"
                )
            )
        ...
        if self.name == "Heart Container" or self.name.startswith("Piece of Heart"):
            return (
                self.world.settings.bridge == "hearts"
                or self.world.settings.shuffle_ganon_bosskey == "hearts"
                or (
                    self.world.settings.shuffle_ganon_bosskey == "on_lacs"
                    and self.world.settings.lacs_condition == "hearts"
                )
            )

        if self.map or self.compass:
            return False
        if self.type == "DungeonReward" and self.world.settings.shuffle_dungeon_rewards in (
            "vanilla",
            "reward",
            "dungeon",
        ):
            return False
        if self.type == "SmallKey" and self.world.settings.shuffle_smallkeys in (
            "dungeon",
            "vanilla",
        ):
            return False
        ...
        if self.type == "TCGSmallKey" and self.world.settings.shuffle_tcgkeys == "vanilla":
            return False
        if self.type == "BossKey" and self.world.settings.shuffle_bosskeys in (
            "dungeon",
            "vanilla",
        ):
            return False
        if self.type == "GanonBossKey" and self.world.settings.shuffle_ganon_bosskey in (
            "dungeon",
            "vanilla",
        ):
            return False
        if self.type == "SilverRupee" and self.world.settings.shuffle_silver_rupees in (
            "dungeon",
            "vanilla",
        ):
            return False
...
@overload
def ItemFactory(
    items: Iterable[str], world: Optional[World] = None, event: bool = False
) -> list[Item]:
    pass


def ItemFactory(
    items: str | Iterable[str], world: Optional[World] = None, event: bool = False
) -> Item | list[Item]:
...
def make_event_item(name: str, location: Location, item: Optional[Item] = None) -> Item:
    if location.world is None:
        raise Exception(
            f"`make_event_item` called with location '{location.name}' that doesn't have a world."
        )
    if item is None:
...
def ItemIterator(
    predicate: Callable[[Item], bool] = lambda item: True, world: Optional[World] = None
) -> Iterable[Item]:
    for item_name in item_table:

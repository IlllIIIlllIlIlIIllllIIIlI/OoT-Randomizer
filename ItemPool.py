...
ludicrous_items_extended: list[str] = [
    ...
    "Eyeball Frog",
    "Eyedrops",
    "Claim Check" "Silver Rupee (Dodongos Cavern Staircase)",
...
shopsanity_rupees: list[str] = ["Rupees (20)"] * 5 + ["Rupees (50)"] * 3 + ["Rupees (200)"] * 2
...
normal_bottles: list[str] = [
    bottle
    for bottle in sorted(ItemInfo.bottles)
    if bottle not in ("Deliver Letter", "Sell Big Poe")
] + ["Bottle with Big Poe"]
reward_list: list[str] = [
    item.name
    for item in sorted(
        [i for n, i in ItemInfo.items.items() if i.type == "DungeonReward"],
        key=lambda x: x.special["item_id"],
    )
]
song_list: list[str] = [
    item.name
    for item in sorted(
        [i for n, i in ItemInfo.items.items() if i.type == "Song"],
        key=lambda x: x.index if x.index is not None else 0,
    )
]
junk_pool_base: list[tuple[str, int]] = [
    (item, weight) for (item, weight) in sorted(ItemInfo.junk_weight.items()) if weight > 0
]
remove_junk_items: list[str] = [
    item for (item, weight) in sorted(ItemInfo.junk_weight.items()) if weight >= 0
]
...
item_groups: dict[str, Sequence[str]] = {
    ...
    "ProgressItem": sorted(
        [name for name, item in ItemInfo.items.items() if item.type == "Item" and item.advancement]
    ),
    "MajorItem": sorted(
        [
            name
            for name, item in ItemInfo.items.items()
            if item.type in ("Item", "Song") and item.advancement and name not in exclude_from_major
        ]
    ),
...
def get_junk_item(
    ...
    plando_pool: Optional[dict[str, ItemPoolRecord]] = None,
) -> list[str]:
    if count < 1:
    ...
    return_pool.extend(
        random.choices(list(jw_dict.keys()), weights=list(jw_dict.values()), k=count)
    )
...
def generate_itempool(world: World) -> None:
    ...
    world.available_tokens = (
        placed_items_count.get("Gold Skulltula Token", 0)
    ...
    if world.max_progressions["Gold Skulltula Token"] > world.available_tokens:
        raise ValueError(
            f"Not enough available Gold Skulltula Tokens to meet requirements. Available: {world.available_tokens}, Required: {world.max_progressions['Gold Skulltula Token']}."
        )
...
def get_pool_core(world: World) -> tuple[list[str], dict[str, Item]]:
    ...
    if world.settings.item_pool_value == "plentiful":
        ...
        if world.settings.shuffle_child_trade:
            ...
            if (
                "Weird Egg" in world.settings.shuffle_child_trade
                and "Chicken" in world.settings.shuffle_child_trade
            ):
        ...
        if world.settings.adult_trade_shuffle:
            ...
            # Make the duplicate item consistent with that.
            if (
                "Pocket Egg" in world.settings.adult_trade_start
                and "Pocket Cucco" in world.settings.adult_trade_start
            ):
        ...
        if world.settings.gerudo_fortress != "open" and world.settings.shuffle_hideoutkeys in (
            "any_dungeon",
            "overworld",
            "keysanity",
            "regional",
        ...
        if world.settings.shuffle_smallkeys in (
            "any_dungeon",
            "overworld",
            "keysanity",
            "regional",
        ...
        if world.settings.shuffle_ganon_bosskey in (
            "any_dungeon",
            "overworld",
            "keysanity",
            "regional",
        ...
        if world.settings.shuffle_silver_rupees in (
            "any_dungeon",
            "overworld",
            "anywhere",
            "regional",
        ...
        if world.settings.shuffle_dungeon_rewards in (
            "any_dungeon",
            "overworld",
            "anywhere",
            "regional",
    ...
    for location in world.get_locations():
        ...
        if (
            ...
            in ("Triforce", "Scarecrow Song", "Deliver Letter", "Time Travel", "Bombchu Drop")
        ...
        # Business Scrubs
            ...
            if location.vanilla_item in (
                ...
                if isinstance(item, list):
                    item = random.choices([i[0] for i in item], weights=[i[1] for i in item], k=1)[
                        0
                    ]
                shuffle_item = True
        ...
        # Bombchus
            ...
            shuffle_item = (
                location.name != "Wasteland Bombchu Salesman"
                or world.settings.shuffle_expensive_merchants
            )
        ...
        # Gerudo Card
            ...
            shuffle_item = (
                world.settings.shuffle_gerudo_card and world.settings.gerudo_fortress != "open"
            )
        ...
        # Adult Trade Quest Items
            ...
            if not world.settings.adult_trade_shuffle:
                ...
                if (
                    location.vanilla_item == "Pocket Egg"
                    and "Pocket Cucco" in world.settings.adult_trade_start
                ):
                    item = "Pocket Cucco"
                    shuffle_item = True
        ...
        # Child Trade Quest Items
            ...
            if location.vanilla_item == "Weird Egg" and world.skip_child_zelda:
                ...
                # If both are selected to be shuffled, only the Egg gets shuffled.
                if (
                    location.vanilla_item == "Weird Egg"
                    and "Chicken" in world.settings.shuffle_child_trade
                ):
                    item = "Chicken"
                    shuffle_item = True
        ...
        # Thieves' Hideout
            ...
            if (
                world.settings.gerudo_fortress == "open"
                or world.settings.gerudo_fortress == "fast"
                and location.name != "Hideout 1 Torch Jail Gerudo Key"
            ):
                item = IGNORE_LOCATION
                shuffle_item = False
            if shuffle_item and world.keyring("Thieves Hideout"):
                item = (
                    ...
                    else "Small Key Ring (Thieves Hideout)"
                )

        # Treasure Chest Game Key Shuffle
        elif (
            location.vanilla_item != "Piece of Heart (Treasure Chest Game)"
            and location.scene == 0x10
        ):
            if world.settings.shuffle_tcgkeys in (
                "regional",
                "overworld",
                "any_dungeon",
                "keysanity",
                ...
                if (
                    world.keyring("Treasure Chest Game")
                    and location.vanilla_item == "Small Key (Treasure Chest Game)"
                ):
                    item = (
                        get_junk_item()[0]
                        if location.name != "Market Treasure Chest Game Salesman"
                        else "Small Key Ring (Treasure Chest Game)"
                    )
                shuffle_item = True
        ...
        # Freestanding Rupees and Hearts
            ...
            if world.settings.shuffle_freestanding_items == "all":
                ...
                world.settings.shuffle_freestanding_items == "dungeons"
                and location.dungeon is not None
            ):
                shuffle_item = True
            elif (
                world.settings.shuffle_freestanding_items == "overworld"
                and location.dungeon is None
            ):
                shuffle_item = True
            else:
                shuffle_item = False
        ...
        # Pots
            ...
            if world.settings.shuffle_pots == "all":
                shuffle_item = True
            elif world.settings.shuffle_pots == "dungeons" and (
                location.dungeon is not None
                or (location.parent_region is not None and location.parent_region.is_boss_room)
            ):
                shuffle_item = True
            elif world.settings.shuffle_pots == "overworld" and not (
                location.dungeon is not None
                or (location.parent_region is not None and location.parent_region.is_boss_room)
            ):
                shuffle_item = True
            ...
            if shuffle_item and (
                location.vanilla_item != "Nothing" or world.settings.shuffle_empty_pots
            ):
                shuffle_item = True
            else:
                shuffle_item = False
        ...
        # Crates
            ...
            if world.settings.shuffle_crates == "all":
                ...
                shuffle_item = True
            if shuffle_item and (
                location.vanilla_item != "Nothing" or world.settings.shuffle_empty_crates
            ):
                shuffle_item = True
            else:
                shuffle_item = False
        ...
        # Dungeon Rewards
            ...
            if world.settings.shuffle_dungeon_rewards in ("vanilla", "reward"):
                pass  # handled in World.fill_bosses
            elif world.settings.shuffle_dungeon_rewards in (
                "any_dungeon",
                "overworld",
                "regional",
                "anywhere",
        ...
        # Dungeon Items
            ...
            if location.vanilla_item == dungeon.item_name("Boss Key"):
                if world.keyring_give_bk(dungeon.name):
                    ...
                    shuffle_setting = (
                        world.settings.shuffle_bosskeys
                        if dungeon.name != "Ganons Castle"
            ...
            # Small Key
                ...
                if shuffle_setting == "vanilla":
                    ...
                    world.keyring(dungeon.name)
                    and not vanilla_items_processed[location.vanilla_item]
                ):
            ...
            if shuffle_setting is not None and dungeon_collection is not None and not shuffle_item:
                if shuffle_setting in ("remove", "startwith"):
                    ...
                    shuffle_setting
                    in ("any_dungeon", "overworld", "keysanity", "regional", "anywhere")
                    and not world.empty_dungeons[dungeon.name].empty
                ):
                    shuffle_item = True
    ...
    if (
        ...
        or (
            ...
            and world.settings.shuffle_smallkeys != "remove"
        )
    ...
    if pending_junk_pool:
        ...
        while pending_junk_pool:
            ...
            if not junk_candidates:
                raise RuntimeError(
                    "Not enough junk exists in item pool for %s (+%d others) to be added."
                    % (pending_item, len(pending_junk_pool) - 1)
                )
    ...
    if world.settings.item_pool_value == "ludicrous":
        ...
        duplicate_candidates = [
            ...
            and (
                ItemInfo.items[item].type != "DungeonReward"
        ...
        max_extra_copies = int(
            Decimal(len(junk_items) / len(duplicate_candidates)).to_integral_value(
                rounding=ROUND_UP
            )
        )

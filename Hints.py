...
from HintList import (
    Hint,
    get_hint,
    get_multi,
    get_hint_group,
    get_upgrade_hint_list,
    hint_exclusions,
...
class GossipText:
    def __init__(
        ...
        hinted_locations: Optional[list[str]] = None,
        hinted_items: Optional[list[str]] = None,
        prefix: str = "They say that ",
...
def is_restricted_dungeon_item(item: Item) -> bool:
    if item.world is None:
        return False
    return (
        ((item.map or item.compass) and item.world.settings.shuffle_mapcompass == "dungeon")
        or (item.type == "SmallKey" and item.world.settings.shuffle_smallkeys == "dungeon")
        or (item.type == "BossKey" and item.world.settings.shuffle_bosskeys == "dungeon")
        or (item.type == "GanonBossKey" and item.world.settings.shuffle_ganon_bosskey == "dungeon")
        or (item.type == "SilverRupee" and item.world.settings.shuffle_silver_rupees == "dungeon")
        or (
            item.type == "DungeonReward"
            and item.world.settings.shuffle_dungeon_rewards in ("vanilla", "reward", "dungeon")
        )
    )
...
def add_hint(
    ...
    gossip_text: GossipText,
    count: int,
    locations: Optional[list[Location]] = None,
    force_reachable: bool = False,
    hint_type: str = None,
) -> bool:
    ...
    if "priority_stones" in world.hint_dist_user["distribution"][hint_type]:
        ...
        for priority_stone in reversed(priority_stones):
            matching_groups = list(
                filter(
                    lambda group: list(
                        set([priority_stone]) & set([gossipLocations[id].name for id in group])
                    ),
                    groups,
                )
            )
        ...
        if groups:
            ...
            if any(map(lambda id: gossipLocations[id].reachable, group)):
                ...
                if locations:
                    for location in locations:
                        if not any(
                            map(
                                lambda stone_location: can_reach_hint(
                                    spoiler.worlds, stone_location, location
                                ),
                                stone_locations,
                            )
                        ):
                    ...
                    if first and locations:
                        ...
                        for i, stone_name in enumerate(stone_names):
                            ...
                            if event_item is None:
                                event_item = make_event_item(
                                    stone_name, stone_locations[i], event_item
                                )
                            else:
            ...
            if not force_reachable and len(duplicates) >= total:
                ...
                unr = [
                    ...
                    if all(map(lambda id: not gossipLocations[id].reachable, group))
                ]
                if len(unr) >= total:
...
def can_reach_hint(worlds: list[World], hint_location: Location, location: Location) -> bool:
    ...
    return search.spot_access(hint_location) and (
        hint_location.type != "HintStone" or search.state_list[location.world.id].guarantee_hint()
    )
...
class HintArea(Enum):
    ...
    def at(spot: Spot, use_alt_hint: bool = False) -> HintArea:
        ...
        while spot_queue or fallback_spot_queue:
            ...
            if parent_region.hint and (
                original_parent.name == "Root" or parent_region.name != "Root"
            ):
        ...
        raise HintAreaNotFound(
            "No hint area could be found for %s [World %d]" % (spot, spot.world.id)
        )
...
def get_woth_hint(spoiler: Spoiler, world: World, checked: set[str]) -> HintReturn:
    ...
    locations = list(
        filter(
            lambda location: location.name not in checked
            and not (
                world.woth_dungeon >= world.hint_dist_user["dungeons_woth_limit"]
                and HintArea.at(location).is_dungeon
            )
    ...
    return GossipText(
        "%s is on the way of the hero." % location_text,
        ["Light Blue"],
        [location.name],
        [location.item.name],
    ), [location]
...
def get_checked_areas(world: World, checked: set[str]) -> set[HintArea | str]:
    def get_area_from_name(check: str) -> HintArea | str:
        ...
        if (
            location.type == "Boss" or location.name == "ToT Reward from Rauru"
...
def get_goal_category(
    spoiler: Spoiler, world: World, goal_categories: dict[str, GoalCategory]
) -> GoalCategory:
    cat_sizes = []
    cat_names = []
    zero_weights = True
    ...
    for cat_name, category in goal_categories.items():
        ...
        if world.id in spoiler.goal_locations and cat_name in spoiler.goal_locations[world.id]:
            ...
            if (
                not world.one_hint_per_goal
                or len([goal for goal in category.goals if goal.weight > 0]) > 0
                or len(
                    [
                        goal
                        for cat in world.goal_categories.values()
                        for goal in cat.goals
                        if goal.weight == 0
                    ]
                )
                == len([goal for cat in world.goal_categories.values() for goal in cat.goals])
            ):
...
def get_goal_hint(spoiler: Spoiler, world: World, checked: set[str]) -> HintReturn:
    ...
    hintable_required_locations_filter = (
        lambda required_location: required_location[0].name not in checked
    ...
    while not required_location_reverse_map:
        ...
        for goal in goals:
            if zero_weights or goal.weight > 0:
                hintable_required_locations = list(
                    filter(hintable_required_locations_filter, goal.required_locations)
                )
    ...
    for other_goal in goals:
        ...
        hintable_required_locations = list(
            filter(hintable_required_locations_filter, other_goal.required_locations)
        )
    ...
    if world_id == world.id:
        ...
        goal_text = (
            spoiler.goal_categories[world_id][goal_category.name].get_goal(goal.name).hint_text
        )

    return GossipText(
        "%s is on %s %s." % (location_text, player_text, goal_text),
        ["Light Blue", goal.color],
        [location.name],
        [location.item.name],
    ), [location]
...
def get_barren_hint(
    spoiler: Spoiler, world: World, checked: set[str], all_checked: set[str]
) -> HintReturn:
    ...
    areas = list(
        filter(
            lambda area: area not in checked_areas
            and str(area) not in world.hint_type_overrides["barren"]
            and not (
                world.barren_dungeon >= world.hint_dist_user["dungeons_barren_limit"]
                and world.empty_areas[area]["dungeon"]
            )
            and any(
    ...
    return (
        GossipText(
            "plundering %s is a foolish choice." % area.text(world.settings.clearer_hints), ["Pink"]
        ),
        None,
    )
...
def is_not_checked(locations: Iterable[Location], checked: set[HintArea | str]) -> bool:
    return not any(
        location.name in checked or HintArea.at(location) in checked for location in locations
    )
...
def get_good_item_hint(spoiler: Spoiler, world: World, checked: set[str]) -> HintReturn:
    locations = list(
        filter(
            lambda location: is_not_checked([location], checked)
            and (
                (location.item.majoritem and location.item.name not in unHintableWothItems)
    ...
    if hint_area.is_dungeon:
        location_text = hint_area.text(world.settings.clearer_hints)
        return GossipText(
            "%s hoards #%s#." % (location_text, item_text),
            ["Red", "Green"],
            [location.name],
            [location.item.name],
        ), [location]
        ...
        return GossipText(
            "#%s# can be found %s." % (item_text, location_text),
            ["Green", "Red"],
            [location.name],
            [location.item.name],
        ), [location]
...
def get_specific_item_hint(spoiler: Spoiler, world: World, checked: set[str]) -> HintReturn:
    ...
    if world.settings.world_count == 1:
        ...
        checked.add(location.name)
        item_text = get_hint(
            get_item_generic_name(location.item), world.settings.clearer_hints
        ).text
        ...
        if world.hint_dist_user.get("vague_named_items", False):
            location_text = hint_area.text(world.settings.clearer_hints)
            return GossipText(
                "%s may be on the hero's path." % location_text,
                ["Green"],
                [location.name],
                [location.item.name],
            ), [location]
        ...
        while True:
            ...
            try:
                ...
                all_named_items = set(
                    itertools.chain.from_iterable([w.named_item_pool for w in worlds])
                )
                ...
                named_item_locations = [
                    location
                    for w in worlds
                    for location in w.get_filled_locations()
                    if (location.item.name in all_named_items)
                ]
                spoiler._cached_named_item_locations = named_item_locations

                always_hints = [
                    (hint, w.id) for w in worlds for hint in get_hint_group("always", w)
                ]
                always_locations = []
                for hint, id in always_hints:
                    location = worlds[id].get_location(hint.name)
                    if (
                        location.item.name in bingoBottlesForHints
                        and world.settings.hint_dist == "bingo"
                    ):
                        always_item = "Bottle"
                    else:
            ...
            if itemname == "Bottle" and world.settings.hint_dist == "bingo":
                locations = [
                    location
                    for location in named_item_locations
                    if (
                        is_not_checked([location], checked)
            ...
            if len(locations) > 0:
                ...
                world.hint_dist_user["named_items_required"]
                and (itemname, world.id) not in always_locations
            ):
                ...
                if (itemname, world.id) not in spoiler._cached_always_locations:
                    logger.info(
                        "Hint for item {} in world {} skipped due to Always hint".format(
                            itemname, world.id
                        )
                    )
                else:
        ...
        hint_area = HintArea.at(location)
        if world.hint_dist_user.get("vague_named_items", False):
            location_text = hint_area.text(
                world.settings.clearer_hints, world=location.world.id + 1
            )
            return GossipText(
                "%s may be on the hero's path." % location_text,
                ["Green"],
            ...
            location_text = hint_area.text(
                world.settings.clearer_hints, preposition=True, world=location.world.id + 1
            )
            return GossipText(
                "#%s# can be found %s." % (item_text, location_text),
                ["Green", "Red"],
...
def get_random_location_hint(spoiler: Spoiler, world: World, checked: set[str]) -> HintReturn:
    locations = list(
        filter(
            ...
            and location.item.name not in world.item_hint_type_overrides["item"]
            and (
                location.world.settings.empty_dungeons_mode == "none"
                or not location.world.empty_dungeons[HintArea.at(location).dungeon_name].empty
            ),
            world.get_filled_locations(),
        )
    )
...
def get_specific_hint(
    spoiler: Spoiler, world: World, checked: set[str], hint_type: str
) -> HintReturn:
    def is_valid_hint(hint: Hint) -> bool:
        ...
        if (
            location.world.settings.empty_dungeons_mode != "none"
            and location.world.empty_dungeons[HintArea.at(location).dungeon_name].empty
        ):
            return False
        return True
    ...
    if world.hint_dist_user["upgrade_hints"] in ["on", "limited"]:
        ...
        upgrade_list = list(
            filter(
                lambda upgrade: is_not_checked(
                    [
                        world.get_location(location)
                        for location in get_multi(upgrade.name).locations
                    ],
                    checked,
                ),
                upgrade_list,
...
def get_random_multi_hint(
    spoiler: Spoiler, world: World, checked: set[str], hint_type: str
) -> HintReturn:
    hint_group = get_hint_group(hint_type, world)
    multi_hints = list(
        filter(
            lambda hint: is_not_checked(
                [world.get_location(location) for location in get_multi(hint.name).locations],
                checked,
            ),
            hint_group,
        )
...
def get_specific_multi_hint(
    spoiler: Spoiler, world: World, checked: set[str], hint: Hint
) -> HintReturn:
    multi = get_multi(hint.name)
    ...
    text_segments = [multi_text] + [
        get_hint(get_item_generic_name(item), world.settings.clearer_hints).text for item in items
    ]
    return (
        GossipText(
            gossip_string % tuple(text_segments),
            colors,
...
def get_entrance_hint(spoiler: Spoiler, world: World, checked: set[str]) -> HintReturn:
    ...
    entrance_hints = list(
        filter(lambda hint: hint.name not in checked, get_hint_group("entrance", world))
    )
    shuffled_entrance_hints = list(
        filter(
            lambda entrance_hint: world.get_entrance(entrance_hint.name).shuffled, entrance_hints
        )
    )

    regions_with_hint = [hint.name for hint in get_hint_group("region", world)]
    valid_entrance_hints = list(
        filter(
            lambda entrance_hint: (
                world.get_entrance(entrance_hint.name).connected_region.name in regions_with_hint
                or world.get_entrance(entrance_hint.name).connected_region.dungeon
            ),
            shuffled_entrance_hints,
        )
    )
...
def get_important_check_hint(spoiler: Spoiler, world: World, checked: set[str]) -> HintReturn:
    ...
    for location in world.get_filled_locations():
        if (
            ...
            and (HintArea.at(location).text(world.settings.clearer_hints) + " Important Check")
            not in checked
    ...
    for location in world.get_filled_locations():
        ...
        if region == hint_loc:
            if (
                ...
                or (
                    location.item.type == "SmallKey"
                    and not (
                        world.settings.shuffle_smallkeys == "dungeon"
                        or world.settings.shuffle_smallkeys == "vanilla"
                    )
                )
                or (
                    location.item.type == "HideoutSmallKey"
                    and not world.settings.shuffle_hideoutkeys == "vanilla"
                )
                or (
                    location.item.type == "TCGSmallKey"
                    and not world.settings.shuffle_tcgkeys == "vanilla"
                )
                or (
                    location.item.type == "BossKey"
                    and not (
                        world.settings.shuffle_bosskeys == "dungeon"
                        or world.settings.shuffle_bosskeys == "vanilla"
                    )
                )
                or (
                    location.item.type == "GanonBossKey"
                    and not (
                        world.settings.shuffle_ganon_bosskey == "vanilla"
                        or world.settings.shuffle_ganon_bosskey == "dungeon"
                        or world.settings.shuffle_ganon_bosskey == "on_lacs"
                        or world.settings.shuffle_ganon_bosskey == "stones"
                        or world.settings.shuffle_ganon_bosskey == "medallions"
                        or world.settings.shuffle_ganon_bosskey == "dungeons"
    ...
    return (
        GossipText(
            "#%s# has #%d# major item%s." % (hint_loc, item_count, "s" if item_count != 1 else ""),
            ["Green", numcolor],
        ),
        None,
    )
...
def build_bingo_hint_list(board_url: str) -> list[str]:
    ...
    if len(hints) == 0:
        raise Exception(
            "No item hints found for goals on Bingosync card. Verify Bingosync URL is correct, or leave field blank for generic bingo hints."
        )
    return hints
    ...
    for world in worlds:
            ...
            if world.settings.enhance_map_compass:
                if world.mixed_pools_bosses or world.settings.shuffle_dungeon_rewards not in (
                    ...
                    compass_locations = [
                        ...
                        if Dungeon.from_vanilla_reward(location.item)
                        is None  # Light Medallion area is shown in menu from beginning of game
                        or (
                            compass_location.item.name
                            == Dungeon.from_vanilla_reward(location.item).item_name("Compass")
                    ...
                    compass_locations = [
                        ...
                        for compass_world in worlds
                        for compass_location in compass_world.get_filled_locations()
                        if HintArea.at(location).dungeon_name
                        is None  # free/ToT reward is shown in menu from beginning of game
                        or (
                            compass_location.item.name
                            == HintArea.at(location).dungeon(location.world).item_name("Compass")
                            and compass_location.item.world == world
                        )
                    ]
                ...
                if "altar" in world.settings.misc_hints and can_reach_hint(
                    worlds,
                    world.get_location(
                        "ToT Child Altar Hint"
                        if location.item.info.stone
                        else "ToT Adult Altar Hint"
                    ),
                    location,
                ):
                    item_world = location.world
        ...
        for hint_type, location in world.misc_hint_item_locations.items():
            if hint_type in world.settings.misc_hints and can_reach_hint(
                worlds,
                world.get_location(misc_item_hint_table[hint_type]["hint_location"]),
                location,
            ):
                item_world = location.world
                if item_world.id not in checked_locations:
        ...
        for hint_type in world.misc_hint_location_items.keys():
            ...
            if hint_type in world.settings.misc_hints and can_reach_hint(
                worlds,
                world.get_location(misc_location_hint_table[hint_type]["hint_location"]),
                location,
            ):
                item_world = location.world
                if item_world.id not in checked_locations:
            ...
            if info.empty:
                for region in world.regions:
                    if region.dungeon is not None and region.dungeon.name == dungeon_name:
                        precompleted_locations = list(
                            map(lambda location: location.name, region.locations)
                        )
...
def build_world_gossip_hints(
    spoiler: Spoiler, world: World, checked_locations: Optional[set[str]] = None
) -> None:
    ...
    if world.settings.hint_dist == "bingo":
        ...
        if world.settings.bingosync_url and world.settings.bingosync_url.startswith(
            "https://bingosync.com/"
        ...
        if (
            world.settings.tokensanity in ("overworld", "all")
            and "Suns Song" not in world.item_hints
        ):
    ...
    if world.settings.hint_dist == "bingo":
        ...
        if (
            2 * len(world.item_hints)
            + 2 * len(get_hint_group("always", world))
            + 2 * num_trial_hints[world.settings.trials]
            > 40
        ) and (world.hint_dist_user["named_items_required"]):
    ...
    if "dual_always" in hint_dist and hint_dist["dual_always"][1] > 0:
        ...
        for hint in always_duals:
            ...
            if "#" not in location_text:
                location_text = "#%s#" % location_text
            first_item_text = get_hint(
                get_item_generic_name(first_location.item), world.settings.clearer_hints
            ).text
            second_item_text = get_hint(
                get_item_generic_name(second_location.item), world.settings.clearer_hints
            ).text
            add_hint(
                spoiler,
                world,
    ...
    if hint_dist["always"][1] > 0:
        always_locations = list(
            filter(
                lambda hint: is_not_checked(
                    [world.get_location(hint.name)], checked_always_locations
                ),
    ...
    if (
        world.entrance_shuffle
        and "entrance_always" in hint_dist
        and hint_dist["entrance_always"][1] > 0
    ):
        ...
        for entrance_hint in always_entrances:
            ...
            if entrance.shuffled and (
                connected_region.dungeon
                or any(
                    hint.name == connected_region.name for hint in get_hint_group("region", world)
                )
            ):
                ...
                if connected_region.dungeon:
                    region_text = get_hint(
                        connected_region.dungeon.name, world.settings.clearer_hints
                    ).text
                else:
                    region_text = get_hint(connected_region.name, world.settings.clearer_hints).text
                if "#" not in region_text:
                ...
                add_hint(
                    ...
                    world,
                    stone_groups,
                    GossipText("%s %s." % (entrance_text, region_text), ["Green", "Light Blue"]),
    ...
    if hint_dist["trial"][1] > 0:
        if world.settings.trials_random and world.settings.trials == 6:
            add_hint(
                ...
                world,
                stone_groups,
                GossipText("#Ganon's Tower# is protected by a powerful barrier.", ["Pink"]),
                hint_dist["trial"][1],
                force_reachable=True,
                hint_type="trial",
            )
            ...
            add_hint(
                ...
                world,
                stone_groups,
                GossipText("Sheik dispelled the barrier around #Ganon's Tower#.", ["Yellow"]),
                hint_dist["trial"][1],
                force_reachable=True,
                hint_type="trial",
            )
            ...
            if world.hint_dist_user["combine_trial_hints"] and world.settings.trials < 5:
                add_hint(
                    ...
                    stone_groups,
                    GossipText(
                        "the #%s Trials# were dispelled by Sheik."
                        % natjoin(
                            trial for trial, skipped in world.skipped_trials.items() if skipped
                        ),
                        ["Yellow"],
                    ),
                    hint_dist["trial"][1],
                ...
                for trial, skipped in world.skipped_trials.items():
                    if skipped:
                        add_hint(
                            ...
                            stone_groups,
                            GossipText(
                                "the #%s Trial# was dispelled by Sheik." % trial, ["Yellow"]
                            ),
                            hint_dist["trial"][1],
                            force_reachable=True,
                            hint_type="trial",
            ...
            if world.hint_dist_user["combine_trial_hints"] and world.settings.trials > 1:
                add_hint(
                    ...
                    stone_groups,
                    GossipText(
                        "the #%s Trials# protect Ganon's Tower."
                        % natjoin(
                            trial for trial, skipped in world.skipped_trials.items() if not skipped
                        ),
                        ["Pink"],
                    ),
                    hint_dist["trial"][1],
                ...
                for trial, skipped in world.skipped_trials.items():
                    if not skipped:
                        add_hint(
                            ...
                            world,
                            stone_groups,
                            GossipText("the #%s Trial# protects Ganon's Tower." % trial, ["Pink"]),
                            hint_dist["trial"][1],
                            force_reachable=True,
                            hint_type="trial",
                        )
            ...
            for i in range(0, len(world.named_item_pool)):
                ...
                if hint:
                    ...
                    place_ok = add_hint(
                        ...
                        stone_groups,
                        gossip_text,
                        hint_dist["named-item"][1],
                        location,
                        hint_type="named-item",
                    )
                    if not place_ok:
    ...
    while stone_groups:
        if fixed_hint_types:
            ...
            if copies > len(stone_groups):
                ...
                logging.getLogger("").debug(
                    f"Not enough gossip stone locations ({len(stone_groups)} groups) for fixed hint type {hint_type} with {copies} copies, proceeding with available stones."
                )
            ...
            hint_types = list(
                filter(lambda htype: hint_dist[htype][1] <= len(stone_groups), hint_types)
            )
                    ...
                    if p != 0:  # If the base prob is 0, then it's 0
                        for w2_type, w2_prob in zip(hint_types, hint_prob):
                            if w2_prob != 0:  # If the other prob is 0, then it has no effect
                                ...
                                p = p * (
                                    ((hint_counts.get(w2_type, 0) / w2_prob) + 1)
                                    / ((hint_counts.get(w1_type, 0) / w1_prob) + 1)
                                )
        ...
        if hint is None:
            ...
            place_ok = add_hint(
                spoiler, world, stone_groups, gossip_text, copies, locations, hint_type=hint_type
            )
            if place_ok:
            ...
            if not place_ok and custom_fixed:
                logging.getLogger("").debug(
                    "Failed to place %s fixed hint for %s.",
                    hint_type,
                    ", ".join([location.name for location in locations]),
                )
...
def build_altar_hints(
    ...
    if include_rewards:
        ...
        child_text += (
            get_hint("Spiritual Stone Text Start", world.settings.clearer_hints).text + "\x04"
        )
...
def build_boss_string(reward: str, color: str, world: World) -> str:
    ...
    if (
        reward in world.distribution.effective_starting_items
        and world.distribution.effective_starting_items[reward].count > 0
    ):
        ...
        location_text = HintArea.at(location).text(
            world.settings.clearer_hints,
            preposition=True,
            world=None if location.world.id == world.id else location.world.id + 1,
        )
    ...
    if world.settings.bridge == "open":
        ...
        if world.settings.bridge == "vanilla":
            ...
            count, singular, plural = {
                ...
                "dungeons": (
                    world.settings.bridge_rewards,
                    "#Spiritual Stone or Medallion#",
                    "#Spiritual Stones and Medallions#",
                ),
                "tokens": (
                    world.settings.bridge_tokens,
                    "#Gold Skulltula Token#",
    ...
    if world.settings.shuffle_ganon_bosskey == "remove":
        ...
        if world.settings.shuffle_ganon_bosskey == "on_lacs":
            if world.settings.lacs_condition == "vanilla":
                ...
                count, singular, plural = {
                    "stones": (
                        world.settings.lacs_stones,
                        "#Spiritual Stone#",
                        "#Spiritual Stones#",
                    ),
                    "medallions": (world.settings.lacs_medallions, "#Medallion#", "#Medallions#"),
                    "dungeons": (
                        world.settings.lacs_rewards,
                        "#Spiritual Stone or Medallion#",
                        "#Spiritual Stones and Medallions#",
                    ),
                    "tokens": (
                        world.settings.lacs_tokens,
                        "#Gold Skulltula Token#",
                        "#Gold Skulltula Tokens#",
                    ),
        ...
        elif world.settings.shuffle_ganon_bosskey in (
            "stones",
            "medallions",
            "dungeons",
            "tokens",
            ...
            count, singular, plural = {
                "stones": (
                    world.settings.ganon_bosskey_stones,
                    "#Spiritual Stone#",
                    "#Spiritual Stones#",
                ),
                "medallions": (
                    world.settings.ganon_bosskey_medallions,
                    "#Medallion#",
                    "#Medallions#",
                ),
                "dungeons": (
                    world.settings.ganon_bosskey_rewards,
                    "#Spiritual Stone or Medallion#",
                    "#Spiritual Stones and Medallions#",
                ),
                "tokens": (
                    world.settings.ganon_bosskey_tokens,
                    "#Gold Skulltula Token#",
                    "#Gold Skulltula Tokens#",
                ),
            ...
            bk_location_string = get_hint(
                "ganonBK_" + world.settings.shuffle_ganon_bosskey, world.settings.clearer_hints
            ).text
...
def build_misc_item_hints(
    world: World, messages: list[Message], allow_duplicates: bool = False
) -> None:
    for hint_type, data in misc_item_hint_table.items():
        if hint_type in world.settings.misc_hints:
            ...
            if (
                item in world.distribution.effective_starting_items
                and world.distribution.effective_starting_items[item].count > 0
            ):
            ...
            update_message_by_id(
                messages,
                data["id"],
                str(GossipText(text, ["Green"], prefix="")),
...
def build_misc_location_hints(world: World, messages: list[Message]) -> None:
    for hint_type, data in misc_location_hint_table.items():
        ...
        update_message_by_id(
            messages, data["id"], str(GossipText(text, ["Green"], prefix="")), 0x23
        )
    ...
    for d in hint_dist_files():
        with open(d, "r") as dist_file:
            try:
                ...
                raise ValueError(
                    f"Could not parse hint distribution file {os.path.basename(d)!r}. Make sure the file is valid JSON or reach out to Support on Discord for help. Details: {e}"
                ) from e

...
def get_setting_json(
    setting: str, web_version: bool, as_array: bool = False
    ...
    setting_json: dict[str, Any] = {
        ...
        "tooltip": remove_trailing_lines(
            "<br>".join(line.strip() for line in setting_info.gui_tooltip.split("\n"))
        ),
    ...
    if setting_json["type"] in types_with_options:
        ...
        for option_name in setting_info.choice_list:
            ...
            option_tooltip = setting_info.gui_params.get("choice_tooltip", {}).get(
                option_name, None
            )
...
def get_section_json(
    section: dict[str, Any], web_version: bool, as_array: bool = False
) -> dict[str, Any]:
    if as_array:
        section_json = {"name": section["name"], "settings": []}
    else:
        section_json = {"settings": {}}
...
def get_tab_json(tab: dict[str, Any], web_version: bool, as_array: bool = False) -> dict[str, Any]:
    if as_array:
        tab_json = {"name": tab["name"], "sections": []}
    else:
        tab_json = {"sections": {}}
...
def create_settings_list_json(path: str, web_version: bool = False) -> None:
    ...
    for d in hint_dist_files():
        ...
        if (
            ...
            and "goal" in dist["distribution"]
            and (
                dist["distribution"]["goal"]["fixed"] != 0

...
def patch_music(rom: Rom, settings: Settings, log: CosmeticsLog, symbols: dict[str, int]) -> None:
    # patch music
    if (
        settings.background_music != "normal"
        or settings.fanfares != "normal"
        or log.src_dict.get("bgm", {})
    ):
...
def patch_model_colors(
    rom: Rom, color: Optional[list[int]], model_addresses: tuple[list[int], list[int], list[int]]
) -> None:
...
def patch_tunic_icon(
    rom: Rom, tunic: str, color: Optional[list[int]], rainbow: bool = False
) -> None:
    ...
    if color is not None:
        tunic_icon = (
            IconManip.generate_rainbow_tunic_icon()
            if rainbow
            else IconManip.generate_tunic_icon(color)
        )
    else:
...
def patch_tunic_colors(
    ...
    tunic_address = symbols.get(
        "CFG_TUNIC_COLORS", 0x00B6DA38
    )  # Use new tunic color ROM address. Fall back to vanilla tunic color ROM address.
    tunics = [
    ...
    for tunic, tunic_setting, address in tunics:
        ...
        # Handle Plando
        if (
            log.src_dict.get("equipment_colors", {}).get(tunic, {}).get("color", "")
            and log.src_dict["equipment_colors"][tunic][":option"] != "Rainbow"
        ):
        ...
        if [tunic, tunic_option] not in [
            ["Kokiri Tunic", "Kokiri Green"],
...
def patch_navi_colors(
    ...
    navi = [
        ...
        (
            ...
            symbols.get("CFG_RAINBOW_NAVI_IDLE_INNER_ENABLED", None),
            symbols.get("CFG_RAINBOW_NAVI_IDLE_OUTER_ENABLED", None),
        ),
        (
            "Navi Targeting Enemy",
            "navi_color_enemy",
            [0x00B5E19C, 0x00B5E1BC],  # Enemy, Boss
            symbols.get("CFG_RAINBOW_NAVI_ENEMY_INNER_ENABLED", None),
            symbols.get("CFG_RAINBOW_NAVI_ENEMY_OUTER_ENABLED", None),
        ),
        (
            "Navi Targeting NPC",
            "navi_color_npc",
            [0x00B5E194],  # NPC
            symbols.get("CFG_RAINBOW_NAVI_NPC_INNER_ENABLED", None),
            symbols.get("CFG_RAINBOW_NAVI_NPC_OUTER_ENABLED", None),
        ),
        (
            "Navi Targeting Prop",
            "navi_color_prop",
            ...
            symbols.get("CFG_RAINBOW_NAVI_PROP_INNER_ENABLED", None),
            symbols.get("CFG_RAINBOW_NAVI_PROP_OUTER_ENABLED", None),
        ),
    ]
    ...
    for (
        ...
        navi_setting,
        navi_addresses,
        rainbow_inner_symbol,
        rainbow_outer_symbol,
    ) in navi:
        ...
        for address_index, address in enumerate(navi_addresses):
            ...
            for index, (navi_part, option, rainbow_symbol) in enumerate(
                ...
                # Plando
                if len(plando_colors) > address_index and plando_colors[address_index].get(
                    navi_part, ""
                ):
        ...
        if option_dict["inner"] != "Rainbow" or option_dict["outer"] != "Rainbow" or rainbow_error:
            for address_colors in colors:
                ...
                for part, color in address_colors.items():
                    if (
                        log.misc_colors[navi_action][f":option_{part}"] != "Rainbow"
                        or rainbow_error
                    ):
...
def patch_sword_trails(
    ...
    sword_trails = [
        (
            ...
            symbols.get("CFG_RAINBOW_SWORD_INNER_ENABLED", None),
            symbols.get("CFG_RAINBOW_SWORD_OUTER_ENABLED", None),
        ),
    ]
    ...
    for (
        ...
        for address_index, (
            address,
            inner_transparency,
            inner_white_transparency,
            outer_transparency,
            outer_white_transparency,
        ) in enumerate(trail_addresses):
            address_colors = {}
            colors.append(address_colors)
            transparency_dict = {}
            for index, (
            ...
            # write color
            color = (
                address_colors["outer"]
                + [transparency_dict["outer"]]
                + address_colors["inner"]
                + [transparency_dict["inner"]]
            )
            rom.write_bytes(address, color)
...
def patch_bombchu_trails(
    ...
    bombchu_trails = [
        (
            "Bombchu Trail",
            "bombchu_trail_color",
            Colors.get_bombchu_trail_colors(),
            Colors.bombchu_trail_colors,
            (
                symbols["CFG_BOMBCHU_TRAIL_INNER_COLOR"],
                symbols["CFG_BOMBCHU_TRAIL_OUTER_COLOR"],
                symbols["CFG_RAINBOW_BOMBCHU_TRAIL_INNER_ENABLED"],
                symbols["CFG_RAINBOW_BOMBCHU_TRAIL_OUTER_ENABLED"],
            ),
        ),
    ]
...
def patch_boomerang_trails(
    ...
    boomerang_trails = [
        (
            ...
            Colors.get_boomerang_trail_colors(),
            Colors.boomerang_trail_colors,
            (
                symbols["CFG_BOOM_TRAIL_INNER_COLOR"],
                symbols["CFG_BOOM_TRAIL_OUTER_COLOR"],
                symbols["CFG_RAINBOW_BOOM_TRAIL_INNER_ENABLED"],
                symbols["CFG_RAINBOW_BOOM_TRAIL_OUTER_ENABLED"],
            ),
        ),
    ]
...
def patch_trails(rom: Rom, settings: Settings, log: CosmeticsLog, trails) -> None:
    for trail_name, trail_setting, trail_color_list, trail_color_dict, trail_symbols in trails:
        color_inner_symbol, color_outer_symbol, rainbow_inner_symbol, rainbow_outer_symbol = (
            trail_symbols
        )
        option_inner = getattr(settings, f"{trail_setting}_inner")
        option_outer = getattr(settings, f"{trail_setting}_outer")
...
def patch_gauntlet_colors(
    ...
    gauntlets = [
        (
            ...
            0x00B6DA44,
            ([0x173B4CC], [0x173B4D4, 0x173B50C, 0x173B514], []),
        ),  # GI Model DList colors
        (
            "Gold Gauntlets",
            "golden_gauntlets_color",
            0x00B6DA47,
            ([0x173B4EC], [0x173B4F4, 0x173B52C, 0x173B534], []),
        ),  # GI Model DList colors
    ]
...
def patch_heart_colors(
    ...
    hearts = [
        ...
        ),  # GI Model and Potion DList colors
    ]
...
def patch_magic_colors(
    ...
    for magic_color, magic_setting, symbol, model_addresses in magic:
        ...
        if magic_option != "Green" and settings.correct_model_colors:
            patch_model_colors(rom, color, model_addresses)
            IconManip.patch_overworld_icon(
                rom, color, 0xF45650, data_path("icons/magicSmallExtras.raw")
            )  # Overworld Small Pot
            IconManip.patch_overworld_icon(
                rom, color, 0xF47650, data_path("icons/magicLargeExtras.raw")
            )  # Overworld Big Pot
        else:
            patch_model_colors(rom, None, model_addresses)
...
def patch_button_colors(
    ...
    buttons = [
        (
            "A Button Color",
            "a_button_color",
            Colors.a_button_colors,
            [
                ("A Button Color", symbols["CFG_A_BUTTON_COLOR"], None),
                (
                    "Text Cursor Color",
                    symbols["CFG_TEXT_CURSOR_COLOR"],
                    [(0xB88E81, 0xB88E85, 0xB88E9)],
                ),  # Initial Inner Color
                ("Shop Cursor Color", symbols["CFG_SHOP_CURSOR_COLOR"], None),
                (
                    "Save/Death Cursor Color",
                    None,
                    [(0xBBEBC2, 0xBBEBC3, 0xBBEBD6), (0xBBEDDA, 0xBBEDDB, 0xBBEDDE)],
                ),  # Save Cursor / Death Cursor
                (
                    "Pause Menu A Cursor Color",
                    None,
                    [
                        (0xBC7849, 0xBC784B, 0xBC784D),
                        (0xBC78A9, 0xBC78AB, 0xBC78AD),
                        (0xBC78BB, 0xBC78BD, 0xBC78BF),
                    ],
                ),  # Inner / Pulse 1 / Pulse 2
                ("Pause Menu A Icon Color", None, [(0x845754, 0x845755, 0x845756)]),
                (
                    "A Note Color",
                    symbols["CFG_A_NOTE_COLOR"],  # For Textbox Song Display
                    [
                        (0xBB299A, 0xBB299B, 0xBB299E),
                        (0xBB2C8E, 0xBB2C8F, 0xBB2C92),
                        (0xBB2F8A, 0xBB2F8B, 0xBB2F96),
                    ],
                ),  # Pause Menu Song Display
            ],
        ),
        (
            "B Button Color",
            "b_button_color",
            Colors.b_button_colors,
            [
        ...
        (
            "C Button Color",
            "c_button_color",
            Colors.c_button_colors,
            [
                ("C Button Color", symbols["CFG_C_BUTTON_COLOR"], None),
                (
                    "Pause Menu C Cursor Color",
                    None,
                    [
                        (0xBC7843, 0xBC7845, 0xBC7847),
                        (0xBC7891, 0xBC7893, 0xBC7895),
                        (0xBC78A3, 0xBC78A5, 0xBC78A7),
                    ],
                ),  # Inner / Pulse 1 / Pulse 2
                ("Pause Menu C Icon Color", None, [(0x8456FC, 0x8456FD, 0x8456FE)]),
                (
                    "C Note Color",
                    symbols["CFG_C_NOTE_COLOR"],  # For Textbox Song Display
                    [
                        (0xBB2996, 0xBB2997, 0xBB29A2),
                        (0xBB2C8A, 0xBB2C8B, 0xBB2C96),
                        (0xBB2F86, 0xBB2F87, 0xBB2F9A),
                    ],
                ),  # Pause Menu Song Display
            ],
        ),
    ...
    for button, button_setting, button_colors, patches in buttons:
        ...
        # grab the color from the list
            ...
            color_set = (
                [button_colors[button_option]]
                if isinstance(button_colors[button_option][0], int)
                else list(button_colors[button_option])
            )
            color = color_set[0]
        # build color from hex code
        else:
...
def patch_instrument(
    ...
    if (
        log.src_dict.get("sfx", {}).get("Ocarina", "")
        and log.src_dict["sfx"]["Ocarina"] in ocarina_options_inv
    ):
...
def patch_silent_voice(
    rom: Rom,
    sfxidlist: Iterable[int],
    soundbank_entries: dict[str, dict[str, int]],
    log: CosmeticsLog,
) -> None:
    binsfxfilename = os.path.join(data_path("Voices"), "SilentVoiceSFX.bin")
    if not os.path.isfile(binsfxfilename):
        log.errors.append(
            f"Could not find silent voice sfx at {binsfxfilename}. Skipping voice patching"
        )
        return
...
def apply_voice_patch(
    rom: Rom, voice_path: str, soundbank_entries: dict[str, dict[str, int]]
) -> None:
...
def patch_voices(rom: Rom, settings: Settings, log: CosmeticsLog, symbols: dict[str, int]) -> None:
    ...
    voice_ages = (
        (
            "Child",
            settings.sfx_link_child,
            Sounds.get_voice_sfx_choices(0, False),
            chain([0x14, 0x87], range(0x1C, 0x36 + 1), range(0x3E, 0x4C + 1)),
        ),
        (
            "Adult",
            settings.sfx_link_adult,
            Sounds.get_voice_sfx_choices(1, False),
            chain(
                [0x37, 0x38, 0x3C, 0x3D, 0x86],
                range(0x00, 0x13 + 1),
                range(0x15, 0x1B + 1),
                range(0x4D, 0x58 + 1),
            ),
    ...
    for name, voice_setting, choices, silence_sfx_ids in voice_ages:
        ...
        voice_setting = (
            log.src_dict["sfx"][log_key]
            if log.src_dict.get("sfx", {}).get(log_key, "")
            else voice_setting
        )
        ...
        if voice_setting == "Silent":
            ...
            voice_path = (
                os.path.join(age_path, voice_setting)
                if os.path.isdir(os.path.join(age_path, voice_setting))
                else None
            )
            ...
            if voice_path is None:
                voice_dirs = (
                    [f for f in os.listdir(age_path) if os.path.isdir(os.path.join(age_path, f))]
            ...
            if voice_path is None:
                log.errors.append(
                    f"{name} Voice not patched: Cannot find voice data directory: {os.path.join(age_path, voice_setting)}"
                )
                break
...
def patch_correct_model_colors(
    rom: Rom, settings: Settings, log: CosmeticsLog, symbols: dict[str, int]
) -> None:
    if settings.correct_model_colors:
...
patch_sets[0x1F073FDD] = {
    ...
    "symbols": {**patch_sets[0x1F073FDC]["symbols"], "CFG_AUDIOBANK_TABLE_EXTENDED_ADDR": 0x0064},
}
...
patch_sets[0x1F073FDE] = {
    ...
    "symbols": {**patch_sets[0x1F073FDD]["symbols"], "CFG_CORRECT_MODEL_COLORS": 0x0068},
}
...
def patch_cosmetics(settings: Settings, rom: Rom) -> CosmeticsLog:
    ...
    if 0x80000000 <= cosmetic_context <= 0x80F7FFFC:
        cosmetic_context = (
            cosmetic_context - 0x80400000
        ) + 0x3480000  # convert from RAM to ROM address
    ...
    if versioned_patch_set:
        ...
        for patch_func in [
            patch for patch in global_patch_sets if patch not in versioned_patch_set["patches"]
        ]:
...
class CosmeticsLog:
    def __init__(self, settings: Settings) -> None:
        ...
        if self.settings.enable_cosmetic_file:
            if self.settings.cosmetic_file:
                try:
                    ...
                    message = (
                        "Cosmetic Plandomizer file not found at %s" % self.settings.cosmetic_file
                    )
        ...
        self.bgm_groups["favorites"] = CollapseList(
            self.src_dict.get("bgm_groups", {}).get("favorites", []).copy()
        )
        self.bgm_groups["exclude"] = CollapseList(
            self.src_dict.get("bgm_groups", {}).get("exclude", []).copy()
        )
        self.bgm_groups["groups"] = AlignedDict(
            self.src_dict.get("bgm_groups", {}).get("groups", {}).copy(), 1
        )

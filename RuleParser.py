...
def load_aliases() -> None:
    ...
    for s, repl in j.items():
        if "(" in s:
            ...
            args = [re.compile(rf"\b{a.strip()}\b") for a in args.split(",")]
        else:
            rule = s
            args = []
...
class Rule_AST_Transformer(ast.NodeTransformer):
    ...
    def visit_Name(self, node: ast.Name) -> Any:
        if node.id in dir(self):
            ...
            if args:
                raise Exception(
                    f"Parse Error: expected {len(args):d} args for {node.id}, not 0",
                    self.current_spot,
                    ast.dump(node, False),
                )
            ...
            return ast.Call(
                func=ast.Attribute(
                    value=ast.Name(id="state", ctx=ast.Load()), attr="has", ctx=ast.Load()
                ),
                args=[node],
                keywords=[],
            )
            ...
            raise Exception(
                "Parse Error: invalid node name %s" % node.id,
                self.current_spot,
                ast.dump(node, False),
            )
    ...
    def visit_Tuple(self, node: ast.Tuple) -> Any:
        if len(node.elts) != 2:
            raise Exception(
                "Parse Error: Tuple must have 2 values", self.current_spot, ast.dump(node, False)
            )

        item, count = node.elts

        if not isinstance(item, ast.Name) and not (
            isinstance(item, ast.Constant) and isinstance(item.value, str)
        ):
            raise Exception(
                "Parse Error: first value must be an item. Got %s" % item.__class__.__name__,
        ...
        if not (
            isinstance(count, ast.Name)
            or (isinstance(count, ast.Constant) and isinstance(count.value, int))
        ):
            raise Exception(
                "Parse Error: second value must be a number. Got %s" % item.__class__.__name__,
                self.current_spot.name,
                ast.dump(node, False),
            )
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if node.func.id in dir(self):
            ...
            if len(args) != len(node.args):
                raise Exception(
                    f"Parse Error: expected {len(args):d} args for {node.func.id}, not {len(node.args):d}",
                    self.current_spot,
                    ast.dump(node, False),
                )
            ...
            for arg_re, arg_val in zip(args, node.args):
                if isinstance(arg_val, ast.Name):
                    ...
                    raise Exception(
                        "Parse Error: invalid argument %s" % ast.dump(arg_val, False),
                        self.current_spot,
                        ast.dump(node, False),
                    )
        ...
        for child in node.args:
            if isinstance(child, ast.Name):
                if child.id in self.world.__dict__:
                    child = ast.Attribute(
                        value=ast.Attribute(
                            value=ast.Name(id="state", ctx=ast.Load()), attr="world", ctx=ast.Load()
                        ),
                        attr=child.id,
                        ctx=ast.Load(),
                    )
    ...
    def visit_Compare(self, node: ast.Compare) -> Any:
        ...
        if (
            ...
            and node.left.id not in self.world.__dict__
            and node.comparators[0].id not in self.world.__dict__
            and node.left.id not in self.world.settings.settings_dict
            and node.comparators[0].id not in self.world.settings.settings_dict
        ):
        ...
        if isliteral(node.left) and all(map(isliteral, node.comparators)):
            ...
            try:
                res = eval(
                    compile(ast.fix_missing_locations(ast.Expression(node)), "<string>", "eval")
                )
            except TypeError as e:
                raise Exception(
                    "Parse Error: %s" % e, self.current_spot.name, ast.dump(node, False)
                )
    ...
    def visit_BoolOp(self, node: ast.BoolOp) -> Any:
        ...
        for elt in list(node.values):
            if isinstance(elt, ast.Constant) and isinstance(elt.value, str):
                ...
                and elt.id not in rule_aliases
                ...
                if isinstance(elt, ast.Constant) and isinstance(elt.value, bool):
                    ...
                    and elt.func.attr in ("has", groupable)
                    and len(elt.args) == 1
                ):
                    args = elt.args[0]
        ...
        if items:
            node.values = [
                ast.Call(
                    ...
                    args=[
                        ast.Tuple(
                            elts=[ast.Name(id=i, ctx=ast.Load()) for i in items], ctx=ast.Load()
                        )
                    ],
                    keywords=[],
                )
    ...
    def make_call(
        self, node: ast.AST, name: str, args: list[Any], keywords: list[ast.keyword]
    ) -> ast.Call:
        if not hasattr(State, name):
            raise Exception(
                "Parse Error: No such function State.%s" % name,
                self.current_spot.name,
                ast.dump(node, False),
            )
        ...
        return ast.Call(
            func=ast.Attribute(
                value=ast.Name(id="state", ctx=ast.Load()), attr=name, ctx=ast.Load()
            ),
            args=args,
            keywords=keywords,
        )
    ...
    def create_delayed_rules(self) -> None:
        for region_name, node, subrule_name in self.delayed_rules:
            ...
            if access_rule is self.rule_cache.get(
                "NameConstant(False)"
            ) or access_rule is self.rule_cache.get("Constant(False)"):
                ...
                if access_rule is self.rule_cache.get(
                    "NameConstant(True)"
                ) or access_rule is self.rule_cache.get("Constant(True)"):
                    event.always = True
    ...
    def make_access_rule(self, body: ast.AST) -> AccessRule:
        ...
        if rule_str not in self.rule_cache:
            ...
            try:
                ...
                raise Exception(
                    "Parse Error: %s" % e, self.current_spot.name, ast.dump(body, False)
                )
    ...
    def at(self, node: ast.Call) -> ast.Call:
        ...
        if len(node.args) < 2 or not (
            ...
            raise Exception(
                "Parse Error: invalid at() arguments", self.current_spot.name, ast.dump(node, False)
            )
    ...
    def here(self, node: ast.Call) -> ast.Call:
        if not node.args:
            raise Exception(
                "Parse Error: missing here() argument",
                self.current_spot.name,
                ast.dump(node, False),
            )
    ...
    def at_day(self, node: ast.Call) -> ast.expr:
        if self.world.ensure_tod_access:
            ...
            return ast.parse(
                "(tod & TimeOfDay.DAY) if tod else ((state.has_all_of((Ocarina, Suns_Song)) and state.has_all_notes_for_song('Suns Song')) or state.search.can_reach(spot.parent_region, age=age, tod=TimeOfDay.DAY))",
                mode="eval",
            ).body
    ...
    def at_dampe_time(self, node: ast.Call) -> ast.expr:
        if self.world.ensure_tod_access:
            ...
            # parsing is better than constructing this expression by hand
            return ast.parse(
                "(tod & TimeOfDay.DAMPE) if tod else state.search.can_reach(spot.parent_region, age=age, tod=TimeOfDay.DAMPE)",
                mode="eval",
            ).body
        return ast.Constant(True)
    ...
    def at_night(self, node: ast.Call) -> ast.expr:
        if (
            self.current_spot.type == "GS Token"
            and self.world.settings.logic_no_night_tokens_without_suns_song
        ):
        ...
        if self.world.ensure_tod_access:
            ...
            # parsing is better than constructing this expression by hand
            return ast.parse(
                "(tod & TimeOfDay.DAMPE) if tod else ((state.has_all_of((Ocarina, Suns_Song)) and state.has_all_notes_for_song('Suns Song')) or state.search.can_reach(spot.parent_region, age=age, tod=TimeOfDay.DAMPE))",
                mode="eval",
            ).body
        return ast.Constant(True)
    ...
    def parse_rule(
        self, rule_string: str, spot: Optional[Location | Entrance] = None
    ) -> AccessRule:
    ...
    def parse_spot_rule(self, spot: Location | Entrance) -> None:
        ...
        if access_rule is self.rule_cache.get(
            ...
            spot.never = True
        elif access_rule is self.rule_cache.get(
            "NameConstant(True)"
        ) or access_rule is self.rule_cache.get("Constant(True)"):
            spot.always = True

...
class Record:
    def __init__(
        self, properties: Optional[dict[str, Any]] = None, src_dict: Optional[dict[str, Any]] = None
    ) -> None:
        self.properties: dict[str, Any] = (
            properties if properties is not None else getattr(self, "properties")
        )
...
class EmptyDungeonRecord(Record):
    def __init__(
        self, src_dict: Optional[bool | str | dict[str, Optional[bool]]] = "random"
    ) -> None:
...
class GossipRecord(Record):
    def __init__(self, src_dict: dict[str, Any]) -> None:
        ...
        super().__init__(
            {"text": None, "colors": None, "hinted_locations": None, "hinted_items": None}, src_dict
        )
...
class LocationRecord(Record):
    ...
    def from_item(item: Item) -> LocationRecord:
        if item.world.settings.world_count > 1:
            ...
            player = (
                None
                if item.location is not None and item.world is item.location.world
                else (item.world.id + 1)
            )
        ...
        return LocationRecord(
            {
                ...
                "player": player,
                "model": (
                    item.looks_like_item.name
                    if item.looks_like_item is not None
                    and item.location.has_preview()
                    and can_cloak(item, item.looks_like_item)
                    else None
                ),
...
class EntranceRecord(Record):
    ...
    def from_entrance(entrance: Entrance) -> EntranceRecord:
        if entrance.replaces.primary and entrance.replaces.type in (
            "Interior",
            "SpecialInterior",
            "Grotto",
            "Grave",
...
class WorldDistribution:
    def __init__(
        self, distribution: Distribution, id: int, src_dict: Optional[dict[str, Any]] = None
    ) -> None:
        ...
        self.goal_locations: Optional[
            dict[str, dict[str, dict[str, LocationRecord | dict[str, LocationRecord]]]]
        ] = None
    ...
    def update(self, src_dict: dict[str, Any], update_all: bool = False) -> None:
        update_dict = {
            "randomized_settings": {
                name: record for (name, record) in src_dict.get("randomized_settings", {}).items()
            },
            "dungeons": {
                name: DungeonRecord(record)
                for (name, record) in src_dict.get("dungeons", {}).items()
            },
            "empty_dungeons": {
                name: EmptyDungeonRecord(record)
                for (name, record) in src_dict.get("empty_dungeons", {}).items()
            },
            "trials": {
                name: TrialRecord(record) for (name, record) in src_dict.get("trials", {}).items()
            },
            "songs": {
                name: SongRecord(record) for (name, record) in src_dict.get("songs", {}).items()
            },
            "item_pool": {
                name: ItemPoolRecord(record)
                for (name, record) in src_dict.get("item_pool", {}).items()
            },
            "entrances": {
                name: EntranceRecord(record)
                for (name, record) in src_dict.get("entrances", {}).items()
            },
            "locations": {
                name: (
                    [LocationRecord(rec) for rec in record]
                    if is_pattern(name)
                    else LocationRecord(record)
                )
                for (name, record) in src_dict.get("locations", {}).items()
                if not is_output_only(name)
            },
            "woth_locations": None,
            "goal_locations": None,
            "barren_regions": None,
            "gossip_stones": {
                name: (
                    [GossipRecord(rec) for rec in record]
                    if is_pattern(name)
                    else GossipRecord(record)
                )
                for (name, record) in src_dict.get("gossip_stones", {}).items()
            },
        }
    ...
    def to_json(self) -> dict[str, Any]:
        return {
            ...
            "empty_dungeons": {
                name: record.to_json() for (name, record) in self.empty_dungeons.items()
            },
            ...
            "item_pool": SortedDict(
                {name: record.to_json() for (name, record) in self.item_pool.items()}
            ),
            "entrances": {name: record.to_json() for (name, record) in self.entrances.items()},
            "locations": {
                name: [rec.to_json() for rec in record] if is_pattern(name) else record.to_json()
                for (name, record) in self.locations.items()
            },
            ":skipped_locations": {
                loc.name: LocationRecord.from_item(loc.item).to_json()
                for loc in self.skipped_locations
            },
            ":woth_locations": (
                None
                if self.woth_locations is None
                else {name: record.to_json() for (name, record) in self.woth_locations.items()}
            ),
            ":goal_locations": self.goal_locations,
            ":barren_regions": self.barren_regions,
            "gossip_stones": SortedDict(
                {
                    name: (
                        [rec.to_json() for rec in record] if is_pattern(name) else record.to_json()
                    )
                    for (name, record) in self.gossip_stones.items()
                }
            ),
        }
    ...
    def pattern_matcher(self, pattern: str | list[str]) -> Callable[[str], bool]:
        if isinstance(pattern, list):
            ...
            return reduce(
                lambda acc, sub_matcher: lambda item: sub_matcher(item) or acc(item),
                pattern_list,
                lambda _: False,
            )
            ...
            if pattern == "#MajorItem":
                if not self.major_group:  # If necessary to compute major_group, do so only once
                    ...
                    if self.distribution.settings.shuffle_song_items != "any":
                        self.major_group = [
                            x for x in self.major_group if x not in item_groups["Song"]
                        ]
                    ...
                    major_tokens = (
                        (
                            self.distribution.settings.shuffle_ganon_bosskey == "on_lacs"
                            and self.distribution.settings.lacs_condition == "tokens"
                        )
                        or self.distribution.settings.shuffle_ganon_bosskey == "tokens"
                        or self.distribution.settings.bridge == "tokens"
                    )
                    ...
                    major_hearts = (
                        (
                            self.distribution.settings.shuffle_ganon_bosskey == "on_lacs"
                            and self.distribution.settings.lacs_condition == "hearts"
                        )
                        or self.distribution.settings.shuffle_ganon_bosskey == "hearts"
                        or self.distribution.settings.bridge == "hearts"
                    )
                    if major_hearts:
                        self.major_group += [
                            "Heart Container",
                    ...
                    if self.distribution.settings.shuffle_bosskeys == "keysanity":
                        keys = [
                            ...
                            if item.type == "BossKey" and name != "Boss Key"
                        ]
    ...
    def configure_dungeons(
        self, world: World, mq_dungeon_pool: list[str], empty_dungeon_pool: list[str]
    ) -> tuple[int, int]:
    ...
    def pool_remove_item(
        self,
        pools: list[list[str | Item]],
        item_name: str,
        count: int,
        world_id: Optional[int] = None,
        use_base_pool: bool = True,
    ) -> list[str | Item]:
        removed_items = []

        base_remove_matcher = self.pattern_matcher(item_name)
        remove_matcher = lambda item: base_remove_matcher(item) and (
            (item in self.base_pool) ^ (not use_base_pool)
        )
        if world_id is None:
            ...
            if removed_item is None:
                if not use_base_pool:
                    if is_item(item_name):
                        raise KeyError(
                            'No remaining items matching "%s" to be removed.' % (item_name)
                        )
                    else:
                        raise KeyError('No items matching "%s"' % (item_name))
                else:
                    removed_items.extend(
                        self.pool_remove_item(
                            pools, item_name, count - i, world_id=world_id, use_base_pool=False
                        )
                    )
                    break
            if use_base_pool:
    ...
    def pool_add_item(self, pool: list[str], item_name: str, count: int) -> list[str]:
        if item_name == "#Junk":
            ...
            candidates = [
                item.name
                for item in ItemIterator(predicate=add_matcher)
            ...
            if len(candidates) == 0:
                raise RuntimeError(
                    "Unknown item, or item set to 0 in the item pool could not be added: "
                    + repr(item_name)
                    + ". "
                    + build_close_match(item_name, "item")
                )
            added_items = random.choices(candidates, k=count)
        else:
            if not is_item(item_name):
                raise RuntimeError(
                    "Unknown item could not be added: "
                    + repr(item_name)
                    + ". "
                    + build_close_match(item_name, "item")
                )
        ...
        for item_name, record in self.item_pool.items():
            if record.type == "set":
                if item_name == "#Junk":
                    ...
                    raise ValueError(
                        "Cannot add Ice Arrows to item pool with Blue Fire Arrows enabled"
                    )
                elif item_name == "Blue Fire Arrows" and not world.settings.blue_fire_arrows:
                    raise ValueError(
                        "Cannot add Blue Fire Arrows to item pool with Blue Fire Arrows disabled"
                    )
                elif (
                    child_trade_matcher(item_name)
                    and item_name not in world.settings.shuffle_child_trade
                ):
                    remove_trade.append(item_name)
                    continue
                elif child_trade_matcher(item_name) and world.settings.item_pool_value not in (
                    "plentiful",
                    "ludicrous",
                ):
                ...
                if add_count > 0:
                    ...
                    for item in added_items:
                        if bottle_matcher(item):
                            bottles += 1
                        elif adult_trade_matcher(item) and not (
                            world.settings.item_pool_value in ("plentiful", "ludicrous")
                            or world.settings.adult_trade_shuffle
                        ):
            ...
            if bottle_matcher(item_name):
                ...
                try:
                    if "Pocket Egg" in world.settings.adult_trade_start:
                        try:
                            ...
                            raise KeyError(
                                "Tried to start with a Pocket Egg but could not remove it from the item pool. Are both Pocket Egg and Pocket Cucco shuffled?"
                            )
                    elif "Pocket Cucco" not in world.settings.adult_trade_start:
                        raise RuntimeError(
                            f"An unshuffled trade item was included as a starting item. Please either remove {item_name} from starting items or add it to Adult Trade Sequence Items."
                        )
                    else:
                        self.pool_remove_item([pool], "Pocket Cucco", record.count)
                except KeyError:
                    raise KeyError(
                        "Tried to start with a Pocket Egg or Pocket Cucco but could not remove it from the item pool. Are both Pocket Egg and Pocket Cucco shuffled?"
                    )
                ...
                try:
                    if "Weird Egg" in world.settings.shuffle_child_trade:
                        ...
                        raise RuntimeError(
                            f"An unshuffled trade item was included as a starting item. Please either remove {item_name} from starting items or add it to Shuffled Child Trade Sequence Items."
                        )
                    else:
                        self.pool_remove_item([pool], "Chicken", record.count)
                except KeyError:
                    raise KeyError(
                        "Tried to start with a Weird Egg or Chicken but could not remove it from the item pool. Are both Weird Egg and the Chicken shuffled?"
                    )
    ...
    def collect_starters(self, state: State) -> None:
        for name, record in self.starting_items.items():
            for _ in range(record.count):
                item = ItemFactory(
                    "Bottle" if name == "Bottle with Milk (Half)" else name, state.world
                )
                state.collect(item)

    def pool_replace_item(
        self,
        item_pools: list[list[Item]],
        item_group: str,
        player_id: int,
        new_item: str,
        worlds: list[World],
    ...
    def set_shuffled_entrances(
        ...
        target_entrance_pools: dict[str, list[Entrance]],
        ...
        for name, record in self.entrances.items():
            ...
            try:
                if not worlds[self.id].get_entrance(name):
                    raise RuntimeError(
                        "Unknown entrance in world %d: %s. %s"
                        % (self.id + 1, name, build_close_match(name, "entrance", entrance_pools))
                    )
            except KeyError:
                raise RuntimeError(
                    "Unknown entrance in world %d: %s. %s"
            ...
            for pool_type, entrance_pool in entrance_pools.items():
                try:
                    matched_entrance = next(
                        filter(lambda entrance: entrance.name == name, entrance_pool)
                    )
                ...
                if matched_entrance.connected_region is not None:
                    if matched_entrance.type == "Overworld":
                        ...
                        raise RuntimeError(
                            "Entrance already shuffled in world %d: %s" % (self.id + 1, name)
                        )

                target_region = record.region

                matched_targets_to_region = list(
                    filter(
                        lambda target: target.connected_region
                        and target.connected_region.name == target_region,
                        target_entrance_pools[pool_type],
                    )
                )
                if not matched_targets_to_region:
                    raise RuntimeError(
                        "No entrance found to replace with %s that leads to %s in world %d"
                ...
                if record.origin:
                    ...
                    try:
                        matched_target = next(
                            filter(
                                lambda target: target.replaces.parent_region.name == target_parent,
                                matched_targets_to_region,
                            )
                        )
                    except StopIteration:
                        raise RuntimeError(
                            "No entrance found to replace with %s that leads to %s from %s in world %d"
                ...
                if matched_target.connected_region is None:
                    raise RuntimeError(
                        "Entrance leading to %s from %s is already shuffled in world %d"
                ...
                try:
                    ...
                    raise RuntimeError(
                        "Cannot connect %s To %s in world %d (Reason: %s)"
                        % (
                            matched_entrance,
                            matched_entrance.connected_region or matched_target.connected_region,
                            self.id + 1,
                            error,
                        )
                    )
            ...
            if not entrance_found:
                raise RuntimeError(
                    "Entrance does not belong to a pool of shuffled entrances in world %d: %s"
                    % (self.id + 1, name)
                )
    ...
    def get_valid_items_from_record(
        self, itempool: list[Item], used_items: list[str], record: LocationRecord
    ) -> list[str]:
    ...
    def pull_item_or_location(
        self, pools: list[list[Item | Location]], world: World, name: str, remove: bool = True
    ) -> Optional[Item | Location]:
        ...
        if is_pattern(name):
            ...
            return pull_random_element(
                pools, lambda e: e.world is world and matcher(e.name), remove
            )
        else:
        ...
        for name, record in self.pattern_dict_items(self.locations):
            ...
            if boss is None:
                try:
                    ...
                    raise RuntimeError(
                        "Unknown location in world %d: %r. %s"
                        % (world.id + 1, name, build_close_match(name, "location"))
                    )
                if location.type == "Boss":
                    raise RuntimeError(
                        "Boss or already placed in world %d: %s" % (world.id + 1, name)
                    )
                else:
                    continue
            ...
            if (
                record.item == "#Vanilla"
            ):  # Get vanilla item at this location from the location table
            ...
            if (
                valid_items
            ):  # Choices still available in the item pool, choose one, mark it as a used item
            ...
            if reward is None:
                ...
                if is_item(record.item):
                    raise RuntimeError(
                        "Reward already placed in world %d: %s" % (world.id + 1, record.item)
                    )
                else:
                    raise RuntimeError(
                        "Reward unknown in world %d: %s" % (world.id + 1, record.item)
                    )
            count += 1
    ...
    def fill(
        self,
        worlds: list[World],
        location_pools: list[list[Location]],
        item_pools: list[list[Item]],
    ) -> None:
        ...
        fillable_locations = [
            location for location_pool in location_pools for location in location_pool
        ]
        locations = {}
        if self.locations:
            locations = {
                loc: self.locations[loc]
                for loc in random.sample(sorted(self.locations), len(self.locations))
            }
        used_items = []
        ...
        for location_name, record in self.pattern_dict_items(locations):
            ...
            location_matcher = (
                lambda loc: loc.world.id == world.id and loc.name.lower() == location_name.lower()
            )
                ...
                try:
                    ...
                    raise RuntimeError(
                        ...
                        % (
                            world.id + 1,
                            location_name,
                            build_close_match(location_name, "location"),
                        )
                    )
                if location.type == "Boss":
                    continue
                    ...
                    raise RuntimeError(
                        "Location already filled in world %d: %s" % (self.id + 1, location_name)
                    )
                else:
                    continue
                ...
                if isinstance(record.item, list):
                    ...
                    for item in record.item:
                        if (
                            ...
                            or item in item_groups["Bottle"]
                            or item in item_groups["AdultTrade"]
                            or item in item_groups["ChildTrade"]
                        ):
                            continue
            ...
            if (
                record.item == "#Junk"
                and location.type == "Song"
                and world.settings.shuffle_song_items == "song"
                and not any(
                    name in song_list and r.count
                    for name, r in world.settings.starting_items.items()
                )
            ):
            ...
            if item.advancement:
                search = Search.max_explore(
                    [world.state for world in worlds], itertools.chain.from_iterable(item_pools)
                )
        ...
        try:
            ...
            if location.type == "Shop" and "Buy" in record.item:
                try:
                    ...
                    raise RuntimeError(
                        f"Too many shop buy items were added to world {self.id + 1}, and not enough shop buy items are available in the item pool to be removed."
                    )
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"Too many bottles were added to world {self.id + 1}, and not enough bottles are available in the item pool to be removed."
                    )
            elif (
                record.item in item_groups["AdultTrade"] and not world.settings.adult_trade_shuffle
            ):
                try:
                    item = self.pool_replace_item(
                        pool, "#AdultTrade", player_id, record.item, worlds
                    )
                except KeyError:
                    raise RuntimeError(
                        f"Too many adult trade items were added to world {self.id + 1}, and not enough adult trade items are available in the item pool to be removed."
                    )
            elif (
                record.item in item_groups["ChildTrade"]
                and record.item not in world.settings.shuffle_child_trade
            ):
                try:
                    item = self.pool_replace_item(
                        pool, "#ChildTrade", player_id, record.item, worlds
                    )
                except KeyError:
                    raise RuntimeError(
                        f"Too many child trade items were added to world {self.id + 1}, and not enough child trade items are available in the item pool to be removed."
                    )
                ...
                try:
                    item = self.pool_replace_item(
                        item_pools, "#Junk", player_id, record.item, worlds
                    )
                except KeyError:
                    raise RuntimeError(
                        f"Too many items were added to world {self.id + 1}, and not enough junk is available to be removed."
                    )
                except IndexError:
                    raise RuntimeError(
                        f'Unknown item {record.item!r} being placed on location {location} in world {self.id + 1}. {build_close_match(record.item, "item")}'
                    )
    ...
    def cloak(
        ...
        for name, record in self.pattern_dict_items(self.locations):
            ...
            location = self.pull_item_or_location(
                location_pools, worlds[self.id], name, remove=False
            )
            if location is None:
    ...
    def configure_gossip(self, spoiler: Spoiler, stone_ids: list[int]) -> None:
        for name, record in self.pattern_dict_items(self.gossip_stones):
            matcher = self.pattern_matcher(name)
            stone_id = pull_random_element(
                [stone_ids], lambda id: matcher(gossipLocations[id].name)
            )
            if stone_id is None:
                ...
                if match:
                    ...
                    raise RuntimeError(
                        "Gossip stone unknown or already assigned in world %d: %r. %s"
                        % (self.id + 1, name, build_close_match(name, "stone"))
                    )
            spoiler.hints[self.id][stone_id] = GossipText(
                text=record.text, colors=record.colors, prefix=""
            )
    ...
    def configure_effective_starting_items(self, worlds: list[World], world: World) -> None:
        ...
        for iter_world in worlds:
            ...
            if iter_world.skip_child_zelda:
                skipped_locations += [
                    iter_world.get_location("HC Zeldas Letter"),
                    iter_world.get_location("Song from Impa"),
                ]
            if (
                iter_world.settings.gerudo_fortress == "open"
                and not iter_world.settings.shuffle_gerudo_card
            ):
            ...
            if iter_world.settings.empty_dungeons_mode != "none":
                ...
                if iter_world.settings.shuffle_dungeon_rewards in ("vanilla", "reward"):
                    skipped_locations_from_dungeons += [
                        world.get_location(loc_name) for loc_name in location_groups["Boss"]
                    ]
                elif world.settings.shuffle_dungeon_rewards == "dungeon":
                    skipped_locations_from_dungeons += [
                        location
                        for location in iter_world.get_filled_locations()
                        if location.item.type == "DungeonReward"
                    ]
                if world.settings.shuffle_song_items == "song":
                    skipped_locations_from_dungeons += [
                        world.get_location(loc_name) for loc_name in location_groups["Song"]
                    ]
                elif world.settings.shuffle_song_items == "dungeon":
                    skipped_locations_from_dungeons += [
                        world.get_location(loc_name) for loc_name in location_groups["BossHeart"]
                    ]
                for location in skipped_locations_from_dungeons:
                    hint_area = HintArea.at(location)
                    if (
                        hint_area.is_dungeon
                        and iter_world.empty_dungeons[hint_area.dungeon_name].empty
                    ):
...
class Distribution:
    def __init__(self, settings: Settings, src_dict: Optional[dict[str, Any]] = None) -> None:
        ...
        if self.src_dict:
            ...
            if "starting_items" in self.src_dict:
                raise ValueError(
                    '"starting_items" at the top level is no longer supported, please move it into "settings"'
                )

        self.world_dists: list[WorldDistribution] = [
            WorldDistribution(self, id) for id in range(settings.world_count)
        ]
        # One-time init
        update_dict = {
            "file_hash": (self.src_dict.get("file_hash", []) + [None, None, None, None, None])[0:5],
            "password": (self.src_dict.get("password", []) + [None, None, None, None, None, None])[
                0:6
            ],
            "playthrough": None,
        ...
        if not update_dict["_settings"].get("starting_items", None):
            if (
                update_dict["_settings"].get("starting_equipment", None)
                or update_dict["_settings"].get("starting_inventory", None)
    ...
    def add_location(self, new_location: str, new_item: str) -> None:
        for world_dist in self.world_dists:
            try:
                ...
                print(
                    "Cannot place item at excluded location because it already has an item defined in the Distribution."
                )
    ...
    def configure_triforce_hunt(self, worlds: list[World]) -> None:
        ...
        for world in worlds:
            ...
            if (
                world.skip_child_zelda
                and "Song from Impa" in world.distribution.locations
                and world.distribution.locations["Song from Impa"].item == "Triforce Piece"
            ):
        ...
        if total_count < worlds[0].triforce_goal:
            raise RuntimeError(
                "Not enough Triforce Pieces in the worlds. There should be at least %d and there are only %d."
                % (worlds[0].triforce_goal, total_count)
            )

        if total_starting_count >= worlds[0].triforce_goal:
            raise RuntimeError(
                "Too many Triforce Pieces in starting items. There should be at most %d and there are %d."
                % (worlds[0].triforce_goal - 1, total_starting_count)
            )
    ...
    def reset(self) -> None:
        ...
        for k in per_world_keys:
            ...
            if k in self.src_dict and k[0] != ":":
                if isinstance(self.src_dict[k], dict):
                    ...
                    src_all = {
                        key: val for key, val in self.src_dict[k].items() if key not in world_names
                    }
                    if src_all:
                        for world in self.world_dists:
        ...
        starting_items = itertools.chain(
            self.settings.starting_equipment,
            self.settings.starting_songs,
            self.settings.starting_inventory,
        )
        data: dict[str, StarterRecord | dict[str, StarterRecord]] = defaultdict(
            lambda: StarterRecord(0)
        )
        if isinstance(self.settings.starting_items, dict) and self.settings.starting_items:
            ...
            for name, record in self.settings.starting_items.items():
                if name in world_names:
                    data[name] = {
                        item_name: (
                            count if isinstance(count, StarterRecord) else StarterRecord(count)
                        )
                        for item_name, count in record.items()
                    }
                    add_starting_ammo(data[name])
                else:
                    data[name] = (
                        record if isinstance(record, StarterRecord) else StarterRecord(record)
                    )
        ...
        # add hearts
        if (
            self.settings.starting_hearts > 3
            and "Piece of Heart" not in self.settings.starting_items
            and "Heart Container" not in self.settings.starting_items
        ):
    ...
    def update_spoiler(self, spoiler: Spoiler, output_spoiler: bool) -> None:
        ...
        for world in spoiler.worlds:
            ...
            world_dist.randomized_settings = {
                randomized_item: getattr(world.settings, randomized_item)
                for randomized_item in world.randomized_list
            }
            world_dist.dungeons = {
                dung: DungeonRecord({"mq": world.dungeon_mq[dung]}) for dung in world.dungeon_mq
            }
            world_dist.empty_dungeons = {
                dung: EmptyDungeonRecord({"empty": world.empty_dungeons[dung].empty})
                for dung in world.empty_dungeons
            }
            world_dist.trials = {
                trial: TrialRecord({"active": not world.skipped_trials[trial]})
                for trial in world.skipped_trials
            }
            if hasattr(world, "song_notes"):
                world_dist.songs = {
                    song: SongRecord({"notes": str(world.song_notes[song])})
                    for song in world.song_notes
                }
            world_dist.entrances = {
                ent.name: EntranceRecord.from_entrance(ent) for ent in spoiler.entrances[world.id]
            }
            world_dist.locations = {
                loc: LocationRecord.from_item(item)
                for (loc, item) in spoiler.locations[world.id].items()
            }
            world_dist.woth_locations = {
                loc.name: LocationRecord.from_item(loc.item)
                ...
                for cat_name, goals in spoiler.goal_locations[world.id].items():
                    ...
                    for goal_name, location_worlds in goals.items():
                        ...
                        if goal.items[0]["name"] == "Triforce Piece":
                            goal_text += (
                                " ("
                                + str(goal.items[0]["quantity"])
                                + "/"
                                + str(world.triforce_count)
                                + " reachable)"
                            )
                        ...
                        if goal.items[0]["name"] == "Piece of Heart":
                            goal_text += (
                                " (" + str(goal.items[0]["quantity"]) + "/68 reachable)"
                            )  # TODO adjust total based on starting_hearts?
                        ...
                        for location_world, locations in location_worlds.items():
                            if len(self.world_dists) == 1:
                                ...
                                world_dist.goal_locations[cat_name][goal_text][
                                    "from World " + str(location_world + 1)
                                ] = {
                                    loc.name: LocationRecord.from_item(loc.item).to_json()
                                    for loc in locations
                                }
    ...
    def from_file(settings: Settings, filename: str) -> Distribution:
        if any(map(filename.endswith, [".z64", ".n64", ".v64"])):
            raise InvalidFileException(
                "Your Ocarina of Time ROM doesn't belong in the plandomizer setting. If you don't know what plandomizer is, or don't plan to use it, leave that setting blank and try again."
            )
        ...
        try:
            ...
            raise InvalidFileException(
                f"Invalid Plandomizer File. Make sure the file is a valid JSON file. Failure reason: {str(e)}"
            ) from None
...
def add_starting_item_with_ammo(
    starting_items: dict[str, StarterRecord], item_name: str, count: int = 1
) -> None:
...
def pull_first_element(
    pools: list[list[Any]], predicate: Callable[[Any], bool] = lambda k: True, remove: bool = True
) -> Optional[Any]:
    for pool in pools:
        for element in pool:

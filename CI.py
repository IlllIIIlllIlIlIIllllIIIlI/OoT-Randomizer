...
def check_presets_formatting(fix_errors: bool = False) -> None:
    ...
    presets = {
        preset_name: {
            ...
            for setting_name, setting in SettingInfos.setting_infos.items()
            if setting_name != "starting_items"
            and (setting.shared or setting_name == "aliases")
            and setting_name in preset
        }
        for preset_name, preset in presets.items()
    }
...
def check_hell_mode_tricks(fix_errors: bool = False) -> None:
    ...
    if set(presets["Hell Mode"]["allowed_tricks"]) == {
        trick["name"] for trick in logic_tricks.values()
    }:
        if presets["Hell Mode"]["allowed_tricks"] != [
            trick["name"] for trick in logic_tricks.values()
        ]:
            error(
                "Order of logic tricks in Hell Mode preset does not match definition order in SettingsList.py",
                True,
            )
            ...
            for j in range(i, len(new_item_messages)):
                if i != j:
                    ...
                    if message_id1 == message_id2:
                        error(
                            f"Duplicate MessageID found: {hex(message_id1)}, {message1}, {message2}",
                            False,
                        )
...
def check_table_sizes() -> None:
    from SceneFlags import (
        build_xflag_tables,
        build_xflags_from_world,
        get_alt_list_bytes,
    )
    from World import World

    filename = "plando-table-tests"
    distribution_file = Tests.load_spoiler(
        os.path.join(Tests.test_dir, "plando", filename + ".json")
    )
    settings = Tests.load_settings(
...
def run_ci_checks() -> NoReturn:
    ...
    parser.add_argument(
        "--fix", help="Automatically apply fixes where possible", action="store_true"
    )
    ...
    if hasattr(error, "count") and error.count:
        ...
        if fix_errors:
            ...
            if getattr(error, "can_fix", False):
                ...
                print(
                    f"Run `CI.py --fix --no_unit_tests{release_arg}` to automatically fix {which_errors}.",
                    file=sys.stderr,
                )
            sys.exit(1)
    else:

...
logging.basicConfig(
    level=logging.INFO, filename=os.path.join(output_dir, "LAST_TEST_LOG"), filemode="w"
)
...
never = {
    ...
    if item.priority
    or any(map(name.startswith, never_prefix))
    or any(map(name.endswith, never_suffix))
}
...
progressive = {
    name for name, item in ItemInfo.items.items() if item.special.get("progressive", None)
}
bottles = {
    name
    for name, item in ItemInfo.items.items()
    if item.special.get("bottle", None) and name != "Deliver Letter"
}
...
ludicrous_set = (
    ...
    | set(trade_items)
    | set(bottles)
    | set(ludicrous_exclusions)
    | {"Bottle with Big Poe"}
    | shop_items
)


def make_settings_for_test(
...
def load_settings(
    settings_file: dict[str, Any] | str, seed: Optional[str] = None, filename: Optional[str] = None
) -> Settings:
    if isinstance(
        settings_file, dict
    ):  # Check if settings_file is a distribution file settings dict
        if filename is None:
            raise RuntimeError(
                "Running test with in memory file but did not supply a filename for output file."
            )
        j = settings_file
        j.update(
            {
...
@overload
def generate_with_plandomizer(
    filename: str, live_copy: Literal[False] = False, max_attempts: int = 10
...
@overload
def generate_with_plandomizer(
    filename: str, live_copy: Literal[True], max_attempts: int = 10
) -> tuple[dict[str, Any], Spoiler]:
    pass


def generate_with_plandomizer(
    filename: str, live_copy: bool = False, max_attempts: int = 10
) -> tuple[dict[str, Any], Spoiler | dict[str, Any]]:
    ...
    try:
        ...
        KeyError
    ):  # No settings dict in distribution file, create minimal consistent configuration
        settings = Settings(
            {
                "enable_distribution_file": True,
                "distribution_file": os.path.join(test_dir, "plando", filename + ".json"),
...
class TestPlandomizer(unittest.TestCase):
    def test_item_list(self):
        filenames = ["plando-list", "plando-item-list-implicit", "plando-item-list-explicit"]
    ...
    def test_explicit_item_pool(self):
        ...
        with self.subTest("except when not enough junk can be added"):
            self.assertRaises(
                RuntimeError, generate_with_plandomizer, "plando-explicit-item-pool-2"
            )
    ...
    def test_ludicrous_pool_junk(self):
        ...
        for filename in filenames:
            with self.subTest(filename + " ludicrous junk"):
                ...
                self.assertEqual(
                    set(), pool_set - ludicrous_set, "Ludicrous pool has regular junk items"
                )
    ...
    def test_key_rings(self):
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-bosskey-forest-anywhere-minimal"
        )
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-forest-keysy-plentiful"
        )
        self.assertNotIn("Small Key (Forest Temple)", spoiler["locations"].values())
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-forest-vanilla-plentiful"
        )
        self.assertNotIn("Small Key (Forest Temple)", spoiler["locations"].values())
        self.assertNotIn("Small Key Ring (Forest Temple)", spoiler["locations"].values())
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-hideout-default-vanilla"
        )
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-hideout-fast-vanilla"
        )
        self.assertNotIn("Small Key (Thieves Hideout)", spoiler["locations"].values())
        self.assertNotIn("Small Key Ring (Thieves Hideout)", spoiler["locations"].values())
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-hideout-fast-anywhere-balanced"
        )
        self.assertEqual(get_actual_pool(spoiler)["Small Key (Thieves Hideout)"], 1)
        self.assertNotIn("Small Key Ring (Thieves Hideout)", spoiler["locations"].values())
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-hideout-fast-anywhere-plentiful"
        )
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-hideout-default-anywhere-balanced"
        )
        self.assertNotIn("Small Key (Thieves Hideout)", spoiler["locations"].values())
        self.assertEqual(get_actual_pool(spoiler)["Small Key Ring (Thieves Hideout)"], 1)
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-hideout-default-anywhere-plentiful"
        )
        self.assertNotIn("Small Key (Thieves Hideout)", spoiler["locations"].values())
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-hideout-open-plentiful"
        )
        self.assertNotIn("Small Key (Thieves Hideout)", spoiler["locations"].values())
        self.assertNotIn("Small Key Ring (Thieves Hideout)", spoiler["locations"].values())
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-none-anywhere-plentiful"
        )
        ...
        distribution_file, spoiler = generate_with_plandomizer(
            "plando-keyrings-none-anywhere-ludicrous"
        )
        ...
        for filename in filenames:
            with self.subTest(filename):
                ...
                if spoiler["settings"]["shuffle_bosskeys"] not in ["dungeon", "vanilla"]:
                    continue
                if (
                    spoiler["settings"]["bridge"] == "tokens"
                    or spoiler["settings"]["shuffle_ganon_bosskey"] == "tokens"
                ):
                    continue
                if (
                    spoiler["settings"]["shuffle_ganon_bosskey"] == "on_lacs"
                    and spoiler["settings"]["lacs_condition"] == "tokens"
                ):
                    continue
    ...
    def test_fix_broken_drops(self):
        ...
        self.assertEqual(
            len(
                [
                    ...
                    if "Child Spirit Temple Beyond Metal Bridges Deku Shield Pot" in sphere
                ]
            ),
            0,
        )
        ...
        self.assertRaises(
            ShuffleError,
            lambda: generate_with_plandomizer("plando-fix-broken-drops-bad", max_attempts=1),
        )
...
class TestHints(unittest.TestCase):
        ...
        for filename in filenames:
            with self.subTest(filename):
                ...
                self.assertNotEqual(
                    ...
                    spoiler.worlds[0].misc_hint_item_locations["ganondorf"].name,
                )
    ...
    def test_one_hint_per_goal(self):
        ...
        for filename in filenames:
            with self.subTest(filename):
                ...
                goals = list(
                    goal_name
                    for goal in list(spoiler[":goal_locations"].values())
                    for goal_name in list(goal.keys())
                )
                ...
                for g in range(len(goals)):
                    ...
                    for hint in spoiler["gossip_stones"].values():
                        if (
                            goals[g].lower() in hint["text"].replace("#", "").lower()
                            or goals[g].lower() == "path of the hero"
                        ):
    ...
    def test_those_pots_over_there(self):
        ...
        area = HintArea.at(location, use_alt_hint=True).text(
            world.settings.clearer_hints,
            world=(
                None
                if not location.world or location.world.id == world.id
                else location.world.id + 1
            ),
        )
        ...
        for message in messages:
            if message.id == 0x70CC:  # Ganondorf hint message
                self.assertTrue(
                    "thosepotsoverthere" in message.text.replace("\n", "").replace(" ", "")
                )
    ...
    def test_blue_fire_arrows(self):
        ...
        woth = [
            x["item"] if isinstance(x, dict) else x for x in spoiler[":woth_locations"].values()
        ]
...
class TestValidSpoilers(unittest.TestCase):
    ...
    def required_checks(self, spoiler, locations, items, locitems):
        ...
        multi = {p: {it for it, ct in c.items() if ct > 1} for p, c in items.items()}
    ...
    def verify_playthrough(self, spoiler):
        ...
        if spoiler["settings"].get("triforce_hunt", False) or spoiler["randomized_settings"].get(
            "triforce_hunt", False
        ):
            item_pool = self.normalize_worlds_dict(spoiler["item_pool"])
            # playthrough assumes each player gets exactly the goal
            req = spoiler["settings"].get("triforce_goal_per_world", None) or spoiler[
                "randomized_settings"
            ].get("triforce_goal_per_world", None)
            self.assertEqual(
                {p: req or item_pool[p]["Triforce Piece"] for p in items},
    ...
    def verify_disables(self, spoiler):
        ...
        disables = set(spoiler["settings"].get("disabled_locations", []))
        dmap = {k: {loc: v[loc] for loc in disables if loc in v} for k, v in locmap.items()}
    ...
    def test_testcases(self):
        test_files = [filename for filename in os.listdir(test_dir) if filename.endswith(".sav")]
            ...
            with self.subTest(out=output_file, settings=settings_file):
                try:
                    ...
                    logging.getLogger("").exception(
                        f"Failed to generate with these settings:\n{settings.get_settings_display()}\n"
                    )
                    raise

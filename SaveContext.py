...
class Address:
    ...
    def get_writes(self, save_context: SaveContext) -> None:
        ...
        values = zip(Address.to_bytes(value, self.size), Address.to_bytes(self.mask, self.size))
...
class SaveContext:
    ...
    def write_byte(
        self, address: int, value: int, predicate: Optional[Callable[[int], bool]] = None
    ) -> None:
        if predicate and not predicate(value):
            return
    ...
    # will overwrite the byte at offset with the given value
    def write_bytes(
        self, address: int, bytes: Iterable[int], predicate: Optional[Callable[[int], bool]] = None
    ) -> None:
    ...
    def write_permanent_flag(
        self, scene: int, type: int, byte_offset: int, bit_values: int
    ) -> None:
    ...
    def write_save_table(self, rom: Rom) -> None:
        ...
        if table_len > 0x400:
            raise Exception(
                "The Initial Save Table has exceeded its maximum capacity: 0x%03X/0x400" % table_len
            )
        rom.write_bytes(rom.sym("INITIAL_SAVE_DATA"), save_table)
        extended_table_len = len(extended_table)
        if extended_table_len > 0x100:
            raise Exception(
                "The Initial Extended Save Table has exceeded its maximum capacity: 0x%03X/0x100"
                % extended_table_len
            )
            ...
            if item.startswith("Silver Rupee (") or item.startswith("Silver Rupee Pouch ("):
                ...
                needed_count = {
                    ...
                    "Shadow Temple Invisible Spikes": (
                        10 if world.dungeon_mq["Shadow Temple"] else 5
                    ),
                    "Gerudo Training Ground Slopes": 5,
                    "Gerudo Training Ground Lava": (
                        6 if world.dungeon_mq["Gerudo Training Ground"] else 5
                    ),
                    "Gerudo Training Ground Water": (
                        3 if world.dungeon_mq["Gerudo Training Ground"] else 5
                    ),
                ...
                if count >= needed_count:
                    save_writes = {
                        "Dodongos Cavern Staircase": {
                            "silver_rupee_counts.dc_staircase": needed_count,
                            "scene_flags.dodongo.swch.silver_rupees_staircase": True,
                        },
                        "Ice Cavern Spinning Scythe": {
                            "silver_rupee_counts.ice_scythe": needed_count,
                            "scene_flags.ice.swch.silver_rupees_scythe": True,
                        },
                        "Ice Cavern Push Block": {
                            "silver_rupee_counts.ice_block": needed_count,
                            "scene_flags.ice.swch.silver_rupees_block": True,
                        },
                        "Bottom of the Well Basement": {
                            "silver_rupee_counts.botw_basement": needed_count,
                            "scene_flags.botw.swch.silver_rupees_basement": True,
                        },
                        "Shadow Temple Scythe Shortcut": {
                            "silver_rupee_counts.shadow_scythe": needed_count,
                            "scene_flags.shadow.swch.silver_rupees_scythe": True,
                        },
                        "Shadow Temple Invisible Blades": {
                            "silver_rupee_counts.shadow_blades": needed_count,
                            "scene_flags.shadow.swch.silver_rupees_blades": True,
                        },
                        "Shadow Temple Huge Pit": {
                            "silver_rupee_counts.shadow_pit": needed_count,
                            (
                                "scene_flags.shadow.swch.silver_rupees_mq_pit"
                                if world.dungeon_mq["Shadow Temple"]
                                else "scene_flags.shadow.swch.silver_rupees_vanilla_pit"
                            ): True,
                        },
                        "Shadow Temple Invisible Spikes": {
                            "silver_rupee_counts.shadow_spikes": needed_count,
                            "scene_flags.shadow.swch.silver_rupees_spikes": True,
                        },
                        "Gerudo Training Ground Slopes": {
                            "silver_rupee_counts.gtg_slopes": needed_count,
                            "scene_flags.gtg.swch.silver_rupees_slopes": True,
                            "scene_flags.gtg.clear.slopes_room": True,
                        },
                        "Gerudo Training Ground Lava": {
                            "silver_rupee_counts.gtg_lava": needed_count,
                            "scene_flags.gtg.swch.silver_rupees_lava": True,
                        },
                        "Gerudo Training Ground Water": {
                            "silver_rupee_counts.gtg_water": needed_count,
                            "scene_flags.gtg.swch.silver_rupees_water": True,
                        },
                        "Spirit Temple Child Early Torches": {
                            "silver_rupee_counts.spirit_torches": needed_count,
                            "scene_flags.spirit.swch.silver_rupees_torches": True,
                        },
                        "Spirit Temple Adult Boulders": {
                            "silver_rupee_counts.spirit_boulders": needed_count,
                            "scene_flags.spirit.swch.silver_rupees_boulders": True,
                        },
                        "Spirit Temple Lobby and Lower Adult": {
                            "silver_rupee_counts.spirit_lobby": needed_count,
                            "scene_flags.spirit.swch.silver_rupees_lobby": True,
                        },
                        "Spirit Temple Sun Block": {
                            "silver_rupee_counts.spirit_sun": needed_count,
                            "scene_flags.spirit.swch.silver_rupees_sun": True,
                        },
                        "Spirit Temple Adult Climb": {
                            "silver_rupee_counts.spirit_adult_climb": needed_count,
                            "scene_flags.spirit.swch.silver_rupees_adult_climb": True,
                        },
                        "Ganons Castle Spirit Trial": {
                            "silver_rupee_counts.trials_spirit": needed_count,
                            "scene_flags.gc.swch.silver_rupees_shadow_spirit": True,
                        },
                        "Ganons Castle Light Trial": {
                            "silver_rupee_counts.trials_light": needed_count,
                            "scene_flags.gc.swch.silver_rupees_light": True,
                        },
                        "Ganons Castle Fire Trial": {
                            "silver_rupee_counts.trials_fire": needed_count,
                            (
                                "scene_flags.gc.swch.silver_rupees_mq_fire"
                                if world.dungeon_mq["Ganons Castle"]
                                else "scene_flags.gc.swch.silver_rupees_vanilla_fire"
                            ): True,
                        },
                        "Ganons Castle Shadow Trial": {
                            "silver_rupee_counts.trials_shadow": needed_count,
                            "scene_flags.gc.swch.silver_rupees_shadow_spirit": True,
                        },
                        "Ganons Castle Water Trial": {
                            "silver_rupee_counts.trials_water": needed_count,
                            "scene_flags.gc.swch.silver_rupees_water": True,
                        },
                        "Ganons Castle Forest Trial": {
                            "silver_rupee_counts.trials_forest": needed_count,
                            "scene_flags.gc.swch.silver_rupees_forest": True,
                        },
                    }[puzzle]
                else:
            ...
            for address, value in save_writes.items():
                ...
                for sub_address in address.split("."):
                    if sub_address not in address_value:
                        raise ValueError(
                            "Unknown key %s in %s of SaveContext" % (sub_address, prev_sub_address)
                        )
                ...
                if not isinstance(address_value, Address):
                    raise ValueError(
                        "%s does not resolve to an Address in SaveContext" % sub_address
                    )
    ...
    def equip_items(self, age: str, equip_type: str) -> None:
        ...
        if equip_type not in ["equips", "equips_child", "equips_adult"]:
            raise ValueError(
                "Equip type must be 'equips', 'equips_child' or 'equips_adult', not %s" % equip_type
            )
    ...
    def get_save_context_addresses() -> AddressesDict:
        return {
            ...
            "scene_flags": {
                "dodongo": {
                    "swch": {
                        "silver_rupees_staircase": Address(
                            0xD4 + 0x1C * 0x01 + 0x04, mask=0x80000000
                        ),
                    },
                },
                "spirit": {
                    "swch": {
                        "silver_rupees_adult_climb": Address(
                            0xD4 + 0x1C * 0x06 + 0x04, mask=0x00000001
                        ),
                        "silver_rupees_boulders": Address(
                            0xD4 + 0x1C * 0x06 + 0x04, mask=0x00000004
                        ),
                        "silver_rupees_torches": Address(
                            0xD4 + 0x1C * 0x06 + 0x04, mask=0x00000020
                        ),
                ...
                "shadow": {
                    "swch": {
                        ...
                        "silver_rupees_vanilla_pit": Address(
                            0xD4 + 0x1C * 0x07 + 0x04, mask=0x00000200
                        ),
                ...
                "botw": {
                    "swch": {
                        "silver_rupees_basement": Address(
                            0xD4 + 0x1C * 0x08 + 0x04, mask=0x80000000
                        ),
                    },
                },
                "ice": {
                ...
                "gtg": {
                    ...
                    "clear": {"slopes_room": Address(0xD4 + 0x1C * 0x0B + 0x08, mask=0x00000004)},
                },
                "gc": {
                    "swch": {
                        "silver_rupees_mq_fire": Address(
                            0xD4 + 0x1C * 0x0D + 0x04, mask=0x00000002
                        ),
                        "silver_rupees_water": Address(0xD4 + 0x1C * 0x0D + 0x04, mask=0x00000004),
                        "silver_rupees_vanilla_fire": Address(
                            0xD4 + 0x1C * 0x0D + 0x04, mask=0x00000200
                        ),
                        "silver_rupees_shadow_spirit": Address(
                            0xD4 + 0x1C * 0x0D + 0x04, mask=0x00000800
                        ),
            ...
            "triforce_pieces": Address(
                0xD4 + 0x1C * 0x48 + 0x10, size=4
            ),  # Unused word in scene x48
            "pending_freezes": Address(
                0xD4 + 0x1C * 0x49 + 0x10, size=4
            ),  # Unused word in scene x49
            "Ocarina_A_Button": Address(
                0xD4 + 0x1C * 0x50 + 0x10, mask=0x01
            ),  # Unused word in scene x50
            "Ocarina_C_up_Button": Address(
                0xD4 + 0x1C * 0x50 + 0x10, mask=0x02
            ),  # Unused word in scene x50
            "Ocarina_C_down_Button": Address(
                0xD4 + 0x1C * 0x50 + 0x10, mask=0x04
            ),  # Unused word in scene x50
            "Ocarina_C_left_Button": Address(
                0xD4 + 0x1C * 0x50 + 0x10, mask=0x08
            ),  # Unused word in scene x50
            "Ocarina_C_right_Button": Address(
                0xD4 + 0x1C * 0x50 + 0x10, mask=0x10
            ),  # Unused word in scene x50
    ...
    save_writes_table: dict[str, dict[str, Any]] = {
        ...
        "Magic Bean Pack": {"item_slot.beans": "beans", "ammo.beans": 10},
        ...
        "Silver Rupee Pouch (Bottom of the Well Basement)": {
            "silver_rupee_counts.botw_basement": 5
        },
        "Silver Rupee Pouch (Shadow Temple Scythe Shortcut)": {
            "silver_rupee_counts.shadow_scythe": 5
        },
        "Silver Rupee Pouch (Shadow Temple Invisible Blades)": {
            "silver_rupee_counts.shadow_blades": 10
        },
        ...
        "Silver Rupee Pouch (Shadow Temple Invisible Spikes)": {
            "silver_rupee_counts.shadow_spikes": 10
        },

...
class Settings(SettingInfos):
    ...
    def __init__(self, settings_dict: dict[str, Any], strict: bool = False) -> None:
        ...
        if settings_dict.get("compress_rom", None):
            ...
            settings_dict["create_patch_file"] = settings_dict[
                "compress_rom"
            ] == "Patch" or settings_dict.get("create_patch_file", False)
            settings_dict["create_compressed_rom"] = settings_dict[
                "compress_rom"
            ] == "True" or settings_dict.get("create_compressed_rom", False)
            settings_dict["create_uncompressed_rom"] = settings_dict[
                "compress_rom"
            ] == "False" or settings_dict.get("create_uncompressed_rom", False)
    ...
    def update(self, settings_dict: dict[str, Any], *, initialize: bool = False) -> None:
        for info in self.setting_infos.values():
            ...
            setattr(
                self,
                info.name,
                settings_dict[info.name] if info.name in settings_dict else info.default,
            )
    ...
    def update_with_settings_string(self, text: str) -> None:
        ...
        setattr(
            self, "starting_items", {}
        )  # Settings string contains the GUI format, so clear the current value of the dict format.
            ...
            if self.distribution_file:
                try:
                    ...
                    logging.getLogger("").warning(
                        "Distribution file not found at %s" % (self.distribution_file)
                    )
    ...
    def get_dependency(self, setting_name: str, check_random: bool = True) -> Any:
        ...
        not_in_dist = (
            "_settings" not in self.distribution.src_dict
            or info.name not in self.distribution.src_dict["_settings"].keys()
        )
        if (
            check_random
            and "randomize_key" in info.gui_params
            and getattr(self, info.gui_params["randomize_key"])
            and not_in_dist
        ):
    ...
    def to_json(self, *, legacy_starting_items: bool = False) -> dict[str, Any]:
        ...
        return {  # TODO: This should be done in a way that is less insane than a double-digit line dictionary comprehension.
            setting.name: (
                {
                    name: (
                        {name: record.to_json() for name, record in record.items()}
                        if isinstance(record, dict)
                        else record.to_json()
                    )
                    for name, record in getattr(settings, setting.name).items()
                }
                if setting.name == "starting_items" and not legacy_starting_items
                else getattr(settings, setting.name)
            )
            for setting in self.setting_infos.values()
            if setting.shared
            and (
                setting.name not in self._disabled
                or
                # We want to still include settings disabled by randomized settings options if they're specified in distribution
                (
                    "_settings" in self.distribution.src_dict
                    and setting.name in self.distribution.src_dict["_settings"].keys()
                )
            )
...
def get_settings_from_command_line_args() -> tuple[Settings, bool, str, bool, str]:
    ...
    parser.add_argument(
        "--convert_settings",
        help="Only convert the specified settings to a settings string. If a settings string is specified output the used settings instead.",
        action="store_true",
    )
    parser.add_argument("--settings", help="Use the specified settings file to use for generation")
    parser.add_argument(
        "--settings_preset",
        help="Use the given preset for base settings. Anything defined in the --settings file or the --settings_string will override the preset.",
    )
    parser.add_argument("--seed", help="Generate the specified seed.")
    parser.add_argument(
        "--no_log", help="Suppresses the generation of a log file.", action="store_true"
    )
    parser.add_argument(
        "--output_settings",
        help="Always outputs a settings.json file even when spoiler is enabled.",
        action="store_true",
    )
            ...
            with open(fn, encoding="utf-8") as f:
                ...
                for name, preset in copy.copy(presets).items():
                    ...
                    for alias in preset["aliases"]:
                        if alias in presets:
                            logging.getLogger("").warning(
                                f"Preset {name}, alias {alias} collides with existing preset or alias. Ignoring."
                            )
                            continue

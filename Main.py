...
def resolve_settings(settings: Settings) -> Optional[Rom]:
    ...
    outputting_specific_world = (
        settings.create_uncompressed_rom
        or settings.create_compressed_rom
        or settings.create_wad_file
    )
    using_rom = (
        outputting_specific_world or settings.create_patch_file or settings.patch_without_output
    )
    if not (using_rom or settings.patch_without_output) and not settings.create_spoiler:
        raise Exception(
            "You must have at least one output type or spoiler log enabled to produce anything."
        )
...
def compress_rom(input_file: str, output_file: str, delete_input: bool = False) -> None:
    ...
    if platform.system() == "Windows":
        ...
        raise Exception(
            "This operating system does not support ROM compression. You may only output patch files or uncompressed ROMs."
        )
...
def patch_and_output(settings: Settings, spoiler: Spoiler, rom: Optional[Rom]) -> None:
    ...
    settings_string_hash = (
        hashlib.sha1(settings.settings_string.encode("utf-8")).hexdigest().upper()[:5]
    )
    ...
    if generate_rom and rom is not None:
        ...
        for world in worlds:
            ...
            if settings.create_patch_file:
                ...
                if settings.create_cosmetics_log and patch_cosmetics_log:
                    if separate_cosmetics:
                        cosmetics_log_filename = (
                            f"{output_filename_base}{player_filename_suffix}.zpf_Cosmetics.json"
                        )
                    else:
                        cosmetics_log_filename = (
                            f"{output_filename_base}{player_filename_suffix}_Cosmetics.json"
                        )
            ...
            uncompressed_filename = (
                f"{output_filename_base}{player_filename_suffix}_uncompressed.z64"
            )
            ...
            channel_title = (
                settings.wad_channel_title
                if settings.wad_channel_title != "" and settings.wad_channel_title is not None
                else "OoTRandomizer"
            )
            channel_id = (
                settings.wad_channel_id
                if settings.wad_channel_id != "" and settings.wad_channel_id is not None
                else "OOTE"
            )
            generate_wad(
                settings.wad_file,
        ...
        if settings.world_count > 1 and settings.create_patch_file:
            ...
            with zipfile.ZipFile(patch_archive_path, mode="w") as patch_archive:
                for file in file_list:
                    ...
                    patch_archive.write(
                        file_path,
                        file.replace(output_filename_base, "").replace(
                            ".zpf_Cosmetics", "_Cosmetics"
                        ),

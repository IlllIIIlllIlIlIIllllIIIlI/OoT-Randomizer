...
int main(int argc, char** argv) {
	...
	for (i = 3; i < tabCount; i++) {
		...
		if (tempTab.startP >= DCMPSIZE || tempTab.endP == 0xFFFFFFFF) continue;

		/* Copy if not compressed, decompress otherwise */
		if (tempTab.endP == 0x00000000)
			memcpy((void*)outROM + tempTab.startV, (void*)inROM + tempTab.startP,
			       size);
		else
			decompress((void*)inROM + tempTab.startP, (void*)outROM + tempTab.startV,
			           size);
...
void loadROM(char* name) {
	...
	/* bSwap16 if needed */
	if (inROM[0] == 0x37)
		for (i = 0; i < UINTSIZE; i++) tempROM[i] = bSwap16(tempROM[i]);

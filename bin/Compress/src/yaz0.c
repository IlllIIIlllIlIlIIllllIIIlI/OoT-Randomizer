...
uint32_t findBest(uint8_t*, int, int, uint32_t*, uint32_t*, uint32_t*,
                  uint8_t*);
	...
	for (i = startPos; i < srcPos; i++) {
		...
		if (curHash == hash) {
			for (curSize = 3; curSize < smp; curSize++)
				if (src[i + curSize] != src[srcPos + curSize]) break;

			/* Uodate best if needed */
			if (curSize > bestSize) {
				bestSize = curSize;
				bestPos = i;
				if (bestSize == 0x111) break;
			}
		}
...
uint32_t findBest(uint8_t* src, int srcSize, int srcPos, uint32_t* matchPos,
                  uint32_t* pMatch, uint32_t* pSize, uint8_t* pFlag) {
	int rv;
...
int yaz0_internal(uint8_t* src, int srcSize, uint8_t* dst) {
	...
	while (srcPos < srcSize) {
		...
		numBytes =
			findBest(src, srcSize, srcPos, &matchPos, &pMatch, &pSize, &pFlag);
		...
		/* Three byte encoding */
			...
			if (numBytes > 0x111) numBytes = 0x111;
	...
	if (bitmask != 0) dst[codeBytePos] = codeByte;

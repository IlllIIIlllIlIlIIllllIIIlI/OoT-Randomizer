...
int main(int argc, char** argv) {
	...
	for (i = 0; i < numCores; i++) pthread_join(threads[i], NULL);
	printf("\n");

	/* Get size of new ROM */
	/* Start with size of first 3 files */
	tempSize = tabStart + tabSize;
	for (i = 3; i < tabCount; i++) tempSize += out[i].size;

	/* If ROM is too big, update size */
	if (tempSize > outSize) outSize = tempSize;
	...
	for (i = 3; i < tabCount; i++) {
		...
		prev += size;
		if (out[i].data != NULL) free(out[i].data);
	}
	free(out);
...
uint32_t findTable(uint8_t* argROM) {
	...
	for (i = 1048; i + 4 < UINTSIZE; i += 4) {
		if (tempROM[i] == 0x00000000)
			if (tempROM[i + 1] == 0x60100000) return (i * 4);
	}
...
void* threadFunc(void* null) {
	...
	while ((i = getNext()) != -1) {
		...
		if (refTab[i] == 1) {
			...
			if ((archive != NULL) && (memcmp(src, archive->ref[nextArchive],
			                                 archive->refSize[nextArchive]) == 0)) {
				out[i].comp = 1;
...
void makeArchive() {
	...
	for (i = 3; i < tabCount; i++) {
		...
		if (tab.endP != 0 && tab.endP != 0xFFFFFFFF) fileCount++;
	}

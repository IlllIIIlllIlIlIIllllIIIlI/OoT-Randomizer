...
def add_hue(
    image: MutableSequence[int], color: Sequence[int], tiff: bool = False
) -> MutableSequence[int]:
        ...
        for x in range(0, width):
            try:
                ...
                color = tuple(
                    int(
                        color_start[i]
                        + (color_end[i] - color_start[i])
                        * ((x + y) % (width / len(colors)))
                        / (width / len(colors))
                    )
                    for i in range(3)
                )
                for c in range(3):
                    img[start + (y * width * 4) + (x * 4) + c] = int(
                        ((img[start + (y * width * 4) + (x * 4) + c] / 255) * (color[c] / 255))
                        * 255
                    )
            except:
                pass
...
def add_belt(
    tunic: MutableSequence[int], belt: MutableSequence[int], tiff: bool = False
) -> MutableSequence[int]:
    start = 154 if tiff else 0
...
def generate_tunic_icon(color: Sequence[int]) -> MutableSequence[int]:
    with open(data_path("icons/grey.tiff"), "rb") as grey_fil, open(
        data_path("icons/belt.tiff"), "rb"
    ) as belt_fil:
...
def add_extra_data(rgb_values: RGBValues, filename: str, intensity: float = 0.5) -> None:
    ...
    for i in range(len(rgb_values)):
        ...
        for x in range(3):
            rgb_values[i][x] = int(
                (file_rgb[i][x] * alpha + intensity) + (rgb_values[i][x] * (1 - alpha - intensity))
            )
...
def greyscale_rgb(rgb_values: RGBValues, intensity: int = 2) -> RGBValues:
    for rgb in rgb_values:
        rgb[0] = rgb[1] = rgb[2] = int(
            (rgb[0] * 0.2126 + rgb[1] * 0.7152 + rgb[2] * 0.0722) * intensity
        )
    return rgb_values
...
def patch_overworld_icon(
    rom: Rom, color: Optional[Sequence[int]], address: int, filename: Optional[str] = None
) -> None:

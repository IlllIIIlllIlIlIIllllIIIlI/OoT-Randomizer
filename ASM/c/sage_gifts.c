...
void give_sage_gifts() {
	...
	if (z64_file.game_mode == 0      // normal gameplay
	...
	if (z64_file.game_mode == 0 && GANON_BOSS_KEY_CONDITION &&
	    !z64_file.dungeon_items[10].boss_key) {
		bool give_boss_key = false;
		switch (GANON_BOSS_KEY_CONDITION) {
		case 1: // Medallions
			if (countSetBitsRec(z64_file.quest_items & 0x3F) >=
			    GANON_BOSS_KEY_CONDITION_COUNT)
				give_boss_key = true;
			break;
		case 2: // Dungeons
			if (countSetBitsRec(z64_file.quest_items & 0x1C003F) >=
			    GANON_BOSS_KEY_CONDITION_COUNT)
				give_boss_key = true;
			break;
		case 3: // Stones
			if (countSetBitsRec(z64_file.quest_items & 0x1C0000) >=
			    GANON_BOSS_KEY_CONDITION_COUNT)
				give_boss_key = true;
			break;
		case 4: // Tokens

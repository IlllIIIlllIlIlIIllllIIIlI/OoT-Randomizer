...
typedef z64_actor_t* (*spawn_actor_fn)(void* actor_ctxt, void* actor_data,
                                       z64_game_t* game_ctxt);
#define spawn_actor ((spawn_actor_fn)0x800255C4)
typedef int (*get_object_index_fn)(z64_obj_ctxt_t* obj_ctxt,
                                   uint16_t object_id);
...
} extern_ctxt = {.version = 2, .speed_multiplier = 1.0};
...
void process_extern_ctxt() {
	...
	if (extern_ctxt.equip_boots != 0) {
		...
		if (extern_ctxt.equip_boots == 1) extern_ctxt.equip_boots = 0;
	}

	/* Fog Distance */
	if (extern_ctxt.fog_distance != 0.0) {
		if (z64_game.fog_distance != extern_ctxt.fog_distance) {
			z64_game.fog_distance =
				interpolate(last_fog_distance, extern_ctxt.fog_distance, -0.1);
		}
	} else {
		if (z64_game.fog_distance != scene_fog_distance) {
			z64_game.fog_distance =
				interpolate(z64_game.fog_distance, scene_fog_distance, 0.1);
		}
	}

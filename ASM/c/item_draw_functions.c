...
typedef Gfx* (*gen_double_tile_fn)(z64_gfx_t* gfx, int32_t tile1, uint32_t x1,
                                   uint32_t y1, int32_t width1, int32_t height1,
                                   int32_t tile2, uint32_t x2, uint32_t y2,
                                   int32_t width2, int32_t height2);
...
void draw_gi_bombchu_and_masks(z64_game_t* game, uint32_t draw_id) {
	...
	gSPMatrix(gfx->poly_opa.p++, append_sys_matrix(gfx),
...
void draw_gi_silver_rupee_pouch(z64_game_t* game, uint32_t draw_id) {
	...
	// Set the wallet color silver by reimplementing the gGiAdultWalletColorDL
	// calls: E7 00 00 00 00 00 00 00 gSPRDPPipeSync FA 00 00 00 A0 82 64 FF
	// gSPSetPrimColor FB 00 00 00 46 3C 32 FF gSPSetEnvColor DF 00 00 00 00 00 00
	// 00
	...
	gSPDisplayList(gfx->poly_opa.p++,
	               item_draw_table[draw_id].args[0].dlist); // gGiWalletDL
	gSPDisplayList(gfx->poly_opa.p++,
	               item_draw_table[draw_id]
	               .args[2]
	               .dlist); // gGiAdultWalletRupeeOuterColorDL
	gSPDisplayList(
		gfx->poly_opa.p++,
		item_draw_table[draw_id].args[3].dlist); // gGiWalletRupeeOuterDL
	gSPDisplayList(
		gfx->poly_opa.p++,
		item_draw_table[draw_id].args[4].dlist); // gGiAdultWalletStringColorDL
	gSPDisplayList(gfx->poly_opa.p++,
	               item_draw_table[draw_id].args[5].dlist); // gGiWalletStringDL
	gSPDisplayList(gfx->poly_opa.p++,
	               item_draw_table[draw_id]
	               .args[6]
	               .dlist); // gGiAdultWalletRupeeInnerColorDL
	gSPDisplayList(
		gfx->poly_opa.p++,
		item_draw_table[draw_id].args[7].dlist); // gGiWalletRupeeInnerDL
}
...
void draw_gi_goron_swords(z64_game_t* game, uint32_t draw_id) {
	...
	gSPSegment(gfx->poly_opa.p++, 0x08,
	           gen_double_tile(gfx, 0, game->common.state_frames, 0, 32, 32, 1, 0,
	                           0, 32, 32));
	gSPMatrix(gfx->poly_opa.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gSPDisplayList(gfx->poly_opa.p++, item_draw_table[draw_id].args[0].dlist);
...
void draw_gi_deku_nut(z64_game_t* game, uint32_t draw_id) {
	...
	append_setup_dl_25_to_opa(gfx);
	gSPSegment(gfx->poly_opa.p++, 0x08,
	           gen_double_tile(gfx, 0, game->common.state_frames * 6,
	                           game->common.state_frames * 6, 32, 32, 1,
	                           game->common.state_frames * 6,
	                           game->common.state_frames * 6, 32, 32));
	gSPMatrix(gfx->poly_opa.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gSPDisplayList(gfx->poly_opa.p++, item_draw_table[draw_id].args[0].dlist);
}
...
void draw_gi_recovery_heart(z64_game_t* game, uint32_t draw_id) {
	...
	gSPSegment(
		gfx->poly_xlu.p++, 0x08,
		gen_double_tile(gfx, 0, 0, -(game->common.state_frames * 3), 32, 32, 1, 0,
		                -(game->common.state_frames * 2), 32, 32));
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[0].dlist);
}
...
void draw_gi_fish_bottle(z64_game_t* game, uint32_t draw_id) {
	...
	gSPSegment(gfx->poly_xlu.p++, 0x08,
	           gen_double_tile(gfx, 0, 0, game->common.state_frames, 32, 32, 1, 0,
	                           game->common.state_frames, 32, 32));
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[0].dlist);
}
...
void draw_gi_magic_spells(z64_game_t* game, uint32_t draw_id) {
	...
	append_setup_dl_25_to_xlu(gfx);
	gSPSegment(gfx->poly_xlu.p++, 0x08,
	           gen_double_tile(gfx, 0, game->common.state_frames * 2,
	                           -(game->common.state_frames * 6), 32, 32, 1,
	                           game->common.state_frames,
	                           -(game->common.state_frames * 2), 32, 32));
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[0].dlist);
...
void draw_gi_scales(z64_game_t* game, uint32_t draw_id) {
	...
	gSPSegment(gfx->poly_xlu.p++, 0x08,
	           gen_double_tile(gfx, 0, game->common.state_frames * 2,
	                           -(game->common.state_frames * 2), 64, 64, 1,
	                           game->common.state_frames * 4,
	                           -(game->common.state_frames * 4), 32, 32));
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[2].dlist);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[3].dlist);
...
void draw_gi_potions(z64_game_t* game, uint32_t draw_id) {
	...
	append_setup_dl_25_to_opa(gfx);
	gSPSegment(gfx->poly_opa.p++, 0x08,
	           gen_double_tile(gfx, 0, -game->common.state_frames,
	                           game->common.state_frames, 32, 32, 1,
	                           -game->common.state_frames,
	                           game->common.state_frames, 32, 32));
	gSPMatrix(gfx->poly_opa.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
...
void draw_gi_mirror_shield(z64_game_t* game, uint32_t draw_id) {
	...
	gSPSegment(
		gfx->poly_opa.p++, 0x08,
		gen_double_tile(gfx, 0, 0, (game->common.state_frames * 2) % 256, 64, 64,
...
void draw_gi_gs_token(z64_game_t* game, uint32_t draw_id) {
	...
	append_setup_dl_25_to_xlu(gfx);
	gSPSegment(gfx->poly_xlu.p++, 0x08,
	           gen_double_tile(gfx, 0, 0, -(game->common.state_frames * 5), 32,
	                           32, 1, 0, 0, 32, 64));
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[1].dlist);
...
void draw_gi_blue_fire_candle(z64_game_t* game, uint32_t draw_id) {
	...
	gSPSegment(
		gfx->poly_xlu.p++, 0x08,
		gen_double_tile(gfx, 0, 0, 0, 16, 32, 1, game->common.state_frames * 1,
		                -(game->common.state_frames * 8), 16, 32));
...
void draw_gi_fairy_lantern(z64_game_t* game, uint32_t draw_id) {
	...
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[1].dlist);
	gSPSegment(gfx->poly_xlu.p++, 0x08,
	           gen_double_tile(gfx, 0, 0, 0, 32, 32, 1, game->common.state_frames,
	                           -(game->common.state_frames * 6), 32, 32));
	duplicate_sys_matrix();
	update_sys_matrix(game->mf_11DA0);
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
...
void draw_gi_fairy(z64_game_t* game, uint32_t draw_id) {
	...
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	// Not sure how much of this is required but these are called from the bottle
	// DL. Not including them causes it to draw weird
	gDPSetRenderMode(gfx->poly_xlu.p++, G_RM_PASS, G_RM_AA_ZB_XLU_SURF2);
	gDPSetTextureLUT(gfx->poly_xlu.p++, G_TT_NONE);
	gSPLoadGeometryMode(gfx->poly_xlu.p++, G_ZBUFFER | G_SHADE | G_CULL_BACK |
	                    G_FOG | G_LIGHTING |
	                    G_SHADING_SMOOTH);
	gSPClearGeometryMode(gfx->poly_xlu.p++, G_CULL_BACK | G_FOG);
	gSPSetGeometryMode(gfx->poly_xlu.p++,
...
void draw_gi_poe_bottles(z64_game_t* game, uint32_t draw_id) {
	...
	gSPSegment(
		gfx->poly_xlu.p++, 0x08,
		gen_double_tile(gfx, 0, 0, 0, 16, 32, 1, game->common.state_frames * 1,
		                -(game->common.state_frames * 6), 16, 32));
	duplicate_sys_matrix();
	update_sys_matrix(game->mf_11DA0);
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
...
void draw_gi_song_notes(z64_game_t* game, uint32_t draw_id) {
	...
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gDPSetEnvColor(gfx->poly_xlu.p++, env_color.r, env_color.g, env_color.b,
	               env_color.a);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[0].dlist);
}
...
void draw_gi_small_keys(z64_game_t* game, uint32_t draw_id) {
	...
	gSPMatrix(gfx->poly_opa.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gDPSetCombineMode(gfx->poly_opa.p++, G_CC_MODULATEI_PRIM,
	                  G_CC_MODULATEI_PRIM);
	gDPSetPrimColor(gfx->poly_opa.p++, 0, 0x80, prim_color.r, prim_color.g,
	                prim_color.b, prim_color.a);
	gDPSetEnvColor(gfx->poly_opa.p++, env_color.r, env_color.g, env_color.b,
	               env_color.a);
	gSPDisplayList(gfx->poly_opa.p++, item_draw_table[draw_id].args[0].dlist);
}
...
void draw_gi_boss_keys(z64_game_t* game, uint32_t draw_id) {
	...
	gSPMatrix(gfx->poly_opa.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gDPSetPrimColor(gfx->poly_opa.p++, 0, 0x80, prim_color_key.r,
	                prim_color_key.g, prim_color_key.b, prim_color_key.a);
	gDPSetEnvColor(gfx->poly_opa.p++, env_color_key.r, env_color_key.g,
	               env_color_key.b, env_color_key.a);
	gSPDisplayList(gfx->poly_opa.p++, item_draw_table[draw_id].args[0].dlist);

	append_setup_dl_25_to_xlu(gfx);
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gDPSetPrimColor(gfx->poly_xlu.p++, 0, 0x80, prim_color.r, prim_color.g,
	                prim_color.b, prim_color.a);
	gDPSetEnvColor(gfx->poly_xlu.p++, env_color.r, env_color.g, env_color.b,
	               env_color.a);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[1].dlist);
...
void draw_gi_chubag(z64_game_t* game, uint32_t draw_id) {
	...
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
	gDPSetPrimColor(gfx->poly_xlu.p++, 0, 0x80, prim_color_band.r,
	                prim_color_band.g, prim_color_band.b, prim_color_band.a);
	gDPSetEnvColor(gfx->poly_xlu.p++, env_color_band.r, env_color_band.g,
	               env_color_band.b, env_color_band.a);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[0].dlist);
	// Front
	append_setup_dl_25_to_xlu(gfx);
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
...
static const uint64_t kInitListMedallion[] = {
	0xe700000000000000, 0xd7000002ffffffff, 0xfc11fe23fffff7fb,
	0xef082c1000552078, 0xd900000000220405, 0xdf00000000000000,
};
...
void draw_gi_stones(z64_game_t* game, uint32_t draw_id) {
	...
	gSPMatrix(gfx->poly_xlu.p++, append_sys_matrix(gfx),
	          G_MTX_PUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPMatrix(gfx->poly_xlu.p++, pushXluMatrix(gfx, kMatrixRot),
	          G_MTX_NOPUSH | G_MTX_MUL | G_MTX_MODELVIEW);
	gSPMatrix(gfx->poly_opa.p++, append_sys_matrix(gfx),
	          G_MTX_PUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
	gSPMatrix(gfx->poly_opa.p++, pushOpaMatrix(gfx, kMatrixRot),
	          G_MTX_NOPUSH | G_MTX_MUL | G_MTX_MODELVIEW);
	...
	append_setup_dl_25_to_xlu(gfx);
	gDPSetPrimColor(gfx->poly_xlu.p++, 0x00, 0x80, prim_color.r, prim_color.g,
	                prim_color.b, prim_color.a);
	gDPSetEnvColor(gfx->poly_xlu.p++, env_color.r, env_color.g, env_color.b,
	               env_color.a);
	gSPDisplayList(gfx->poly_xlu.p++, item_draw_table[draw_id].args[0].dlist);

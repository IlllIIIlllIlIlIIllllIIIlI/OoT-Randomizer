...
void draw_background(z64_disp_buf_t* db, int bg_left, int bg_top, int bg_width,
                     int bg_height) {
	gDPSetCombineMode(db->p++, G_CC_PRIMITIVE, G_CC_PRIMITIVE);
	gDPSetPrimColor(db->p++, 0, 0, 0x00, 0x00, 0x00, 0xD0);
	gSPTextureRectangle(db->p++, bg_left << 2, bg_top << 2,
	                    (bg_left + bg_width) << 2, (bg_top + bg_height) << 2, 0,
	                    0, 0, 1 << 10, 1 << 10);
...
int d_right_dungeon_idx(int i) {
	...
	if (!CFG_DUNGEON_INFO_SILVER_RUPEES || !CFG_DUNGEON_IS_MQ[DODONGO_ID])
		dungeon_idx++;          // skip DC
	if (dungeon_idx >= 2) dungeon_idx++; // skip Jabu
	if (dungeon_idx >= 9 &&
	    (!CFG_DUNGEON_INFO_SILVER_RUPEES || CFG_DUNGEON_IS_MQ[ICE_ID]))
		dungeon_idx++; // skip Ice
	return dungeon_idx;
}
...
void draw_silver_rupee_count(z64_game_t* globalCtx, z64_disp_buf_t* db) {
	...
	for (int i = 0; i < dungeon_count; i++) {
		...
		uint8_t* silver_rupee_puzzles = CFG_DUNGEON_IS_MQ[dungeon.index]
		                        ? dungeon.silver_rupee_puzzles_mq
		                        : dungeon.silver_rupee_puzzles_vanilla;
		for (int puzzle_idx = 0; puzzle_idx < 4; puzzle_idx++) {
			if (silver_rupee_puzzles[puzzle_idx] == (uint8_t)-1) break;
			silver_rupee_data_t silver_rupee_info =
				silver_rupee_vars[silver_rupee_puzzles[puzzle_idx]]
				[CFG_DUNGEON_IS_MQ[dungeon.index]];
			if (silver_rupee_info.room == room) {
				// Draw silver rupee icon
				int scene_index = z64_game.scene_index;
				int voffset = 0;
				if (scene_index < 0x11 &&
				    (z64_file.dungeon_keys[scene_index] >= 0 || bk_display)) {
					voffset -= 17;
				}
				gDPPipeSync(db->p++);
				gDPSetCombineMode(db->p++, G_CC_MODULATEIA_PRIM, G_CC_MODULATEIA_PRIM);
				gDPSetPrimColor(db->p++, 0, 0, 0xFF, 0xFF, 0xFF,
				                globalCtx->hud_alpha_channels.rupees_keys_magic);
				gDPPipeSync(db->p++);
				sprite_texture(db, &key_rupee_clock_sprite, 1, 26, 189 + voffset, 16,
				               16);

				uint8_t count =
					extended_savectx
					.silver_rupee_counts[silver_rupee_puzzles[puzzle_idx]];
				// Draw silver rupee count
				gDPPipeSync(db->p++);

				// Draw the count white if we have less than the required amount
				colorRGBA8_t color = {0xFF, 0xFF, 0xFF,
					              globalCtx->hud_alpha_channels.rupees_keys_magic};

				// Draw the count green (same color as max rupees) if we have the
				// required amount
...
void draw_dungeon_info(z64_disp_buf_t* db) {
	...
	int draw = CAN_DRAW_DUNGEON_INFO && !CAN_DRAW_TRADE_DPAD &&
	           (((pad_held.dl || pad_held.dr || pad_held.dd) &&
	             CFG_DPAD_DUNGEON_INFO_ENABLE) ||
	            ((pad_held.dl || pad_held.dr || pad_held.dd) &&
	             !CFG_DPAD_DUNGEON_INFO_ENABLE && pad_held.a) ||
	            pad_held.a);
	if (!draw) {
		return;
	}
	...
	if (pad_held.a && !((pad_held.dl || pad_held.dr || pad_held.dd) &&
	                    !CFG_DPAD_DUNGEON_INFO_ENABLE)) {
		show_dungeon_info = 1;
		uint16_t altar_flags = z64_file.inf_table[27];
		int show_medals =
			CFG_DUNGEON_INFO_REWARD_ENABLE &&
			(!CFG_DUNGEON_INFO_REWARD_NEED_ALTAR || (altar_flags & 1)) &&
			CFG_DUNGEON_INFO_REWARD_SUMMARY_ENABLE;
		int show_stones =
			CFG_DUNGEON_INFO_REWARD_ENABLE &&
			(!CFG_DUNGEON_INFO_REWARD_NEED_ALTAR || (altar_flags & 2)) &&
			CFG_DUNGEON_INFO_REWARD_SUMMARY_ENABLE;
		int show_keys = 1;
		...
		if (show_medals) {
			...
			for (int i = 0; i < non_chest_game_dungeon_count; i++) {
				...
				sprite_draw(db, &medals_sprite, reward, left, top, icon_size,
				            icon_size);
			}
		}
		...
		if (show_stones) {
			...
			for (int i = 0; i < non_chest_game_dungeon_count; i++) {
				...
				int top = start_top + ((icon_size + padding) * i);
				sprite_draw(db, &stones_sprite, reward, left, top, icon_size,
				            icon_size);
			}
		}
		...
		if (show_keys) {
			...
			for (int i = 0; i < rows; i++) {
				...
				char count[5] = "O(O)"; // we use O instead of 0 because it's easier to
				                        // distinguish from 8
			...
			for (int i = 0; i < rows; i++) {
				...
				if (d->has_boss_key && z64_file.dungeon_items[index].boss_key) {
					int top = start_top + ((icon_size + padding) * i);
					sprite_draw(db, &quest_items_sprite, 0, left, top, icon_size,
					            icon_size);
				}
			}
		...
		if (show_skulls) {
			...
			for (int i = 0; i < rows; i++) {
				dungeon_entry_t* d = &(dungeons[i]);
				if (d->skulltulas &&
				    z64_file.gs_flags[d->index ^ 0x03] == d->skulltulas) {
					int top = start_top + ((icon_size + padding) * i);
					sprite_draw(db, &quest_items_sprite, 0, left, top, icon_size,
					            icon_size);
				}
		...
		if (CFG_DUNGEON_INFO_SILVER_RUPEES) {
			...
			for (int i = 0; i < dungeon_count; i++) {
				...
				uint8_t* silver_rupee_puzzles = CFG_DUNGEON_IS_MQ[d->index]
				            ? d->silver_rupee_puzzles_mq
				            : d->silver_rupee_puzzles_vanilla;
				for (int puzzle_idx = 0; puzzle_idx < 4; puzzle_idx++) {
					if (silver_rupee_puzzles[puzzle_idx] == (uint8_t)-1) break;
					uint8_t count =
						extended_savectx
				...
				if (show_silver_rupees) {
					int top = start_top + ((icon_size + padding) * i);
					sprite_draw(db, &key_rupee_clock_sprite, 0, left, top, icon_size,
					            icon_size);
				}
			}
				...
				if (show_silver_rupees) {
					...
					for (int puzzle_idx = 0; puzzle_idx < 4; puzzle_idx++) {
						...
						silver_rupee_data_t var =
							silver_rupee_vars[silver_rupee_puzzles[puzzle_idx]]
							[CFG_DUNGEON_IS_MQ[d->index]];
						uint8_t rupee_count =
							extended_savectx
							.silver_rupee_counts[silver_rupee_puzzles[puzzle_idx]];
						int puzzle_left =
							left + font_width * (2 * puzzle_idx) + padding * 3 * puzzle_idx;
						// draw text manually instead of going through text_print/text_flush
						// to get the right text colors
						gDPSetPrimColor(db->p++, 0, 0, var.r, var.g, var.b, 0xFF);
						if (rupee_count >= 10) {
							sprite_draw(db, &font_sprite, rupee_count / 10, puzzle_left, top,
						...
						sprite_draw(db, &font_sprite, tile_index, puzzle_left + font_width,
						            top, font_width, font_height);
						if (tile_index == 0) {
							sprite_load(db, &font_sprite, 16,
							            10); // load numbers 0 through 9
						}
					}
				}
			}
		...
		int rows = 9;
		int bg_width =
			(1 * icon_size) + (0x16 * font_sprite.tile_w) + (3 * padding);
		...
		for (int i = 3; i < 9; i++) {
			...
			int top = start_top + ((icon_size + padding) * i);
			sprite_draw(db, &medals_sprite, medal->idx, left, top, icon_size,
			            icon_size);
		}
		...
		for (int i = 0; i < 3; i++) {
			int top = start_top + ((icon_size + padding) * i);
			sprite_draw(db, &stones_sprite, i, left, top, icon_size, icon_size);
		}
			...
			for (int i = 0; i < 9; i++) {
				...
				case 2:
					if (i != 3) { // always display Light Medallion
						dungeon_entry_t* d =
							&(dungeons[i - (i < 3 ? 0 : 1)]); // vanilla location of the
						                                          // reward
				...
				char world_text[5] = "WOOO"; // we use O instead of 0 because it's
				                             // easier to distinguish from 8
				if (world < 100) {
					world_text[0] = ' ';
					world_text[1] = 'W';
				}
		...
		for (int i = 0; i < rows; i++) {
			dungeon_entry_t* d = &(dungeons[d_right_dungeon_idx(
								i)]); // skip Deku/DC/Jabu/Ice dynamically
			int top = start_top + ((icon_size + padding) * i) + 1;
				...
				if (show_silver_rupees) {
					...
					for (int puzzle_idx = 0; puzzle_idx < 4; puzzle_idx++) {
						...
						uint8_t count =
							extended_savectx
							.silver_rupee_counts[silver_rupee_puzzles[puzzle_idx]];
						int puzzle_left = left + font_sprite.tile_w * (2 * puzzle_idx) +
						                  padding * 4 * puzzle_idx;
						// draw text manually instead of going through text_print/text_flush
						// to get the right text colors
						gDPSetPrimColor(db->p++, 0, 0, var.r, var.g, var.b, 0xFF);
						if (count >= 10) {
							sprite_draw(db, &font_sprite, count / 10, puzzle_left, top,
						...
						sprite_draw(db, &font_sprite, tile_index,
						            puzzle_left + font_sprite.tile_w, top,
						            font_sprite.tile_w, font_sprite.tile_h);
						if (tile_index == 0) {
							sprite_load(db, &font_sprite, 16,
							            10); // load numbers 0 through 9
		...
		int padding = 1;
		int rows = 12;
		int mq_width = show_mq ? ((6 * font_sprite.tile_w) + padding) : 0;
		int bg_width =
			(3 * icon_size) + (8 * font_sprite.tile_w) + (8 * padding) + mq_width;
		int bg_height = (rows * icon_size) + ((rows + 1) * padding);
		int bg_left = (Z64_SCREEN_WIDTH - bg_width) / 2;
		int bg_top = (Z64_SCREEN_HEIGHT - bg_height) / 2;

...
void get_current_scene_setup_number() {
	...
	// If the scene_setup_index is > 3 then we're probably in a cutscene so just
	// return -1.
	...
	while (1) {
		...
		if (cmd->code == 0x18) { // Alternate header command
			...
			void** alternate_header =
				(void**)(z64_game.scene_segment + segment_offset);
			uint8_t i = z64_file.scene_setup_index;
			// Starting, at scene_setup_index, scan towards the front of list to find
			// the first non-null entry.
			while (i > 0) {
				if ((alternate_header)[i - 1] !=
				    NULL) { // Found a non-null entry so use this as our setup.
					curr_scene_setup = i;
					return;
				}
				i--;

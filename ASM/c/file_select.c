...
bool is_saved_password_clear(z64_menudata_t* menu_data) {
	...
	for (uint8_t slotFile = 0; slotFile < 2; slotFile++) {
		z64_file_t* file =
			&menu_data->sram_buffer->primary_saves[slotFile].original_save;
		extended_savecontext_static_t* extended =
			&(((extended_sram_file_t*)file)->additional_save_data.extended);
...
void manage_password(z64_disp_buf_t* db, z64_menudata_t* menu_data) {
	...
	// Draw "Password locked" at the place of the Controls texture, and a key on
	// the OK button, to display the lock.
	if (!is_saved_password_clear(menu_data) && !is_buffer_password_clear()) {
		...
		if (cooldown > 0) {
			...
			draw_int_size(db, 1 + cooldown / 60, left - 2 * padding, top, color, 6,
			              12);
		}
		gDPPipeSync(db->p++);
		...
		sprite_draw(db, &quest_items_sprite, 0, left, top - 2, BUTTON_WIDTH,
		            BUTTON_HEIGHT);
		text_print_size("Password locked", left + TEXT_HEIGHT + padding, top,
		                TEXT_WIDTH);
		sprite_draw(db, &quest_items_sprite, 0,
		            left + TEXT_WIDTH + 2 * padding + 15 * font_sprite.tile_w,
		            top - 2, BUTTON_WIDTH, BUTTON_HEIGHT);
	...
	if (menu_data->menu_transition == SM_CONFIRM_FILE) {
		if (password_index < 0) {
			if (menu_data->selected_sub_item == FS_BTN_CONFIRM_YES) {
				if (z64_game.common.input[0].pad_pressed.a ||
				    z64_game.common.input[0].pad_pressed.s) {
					if (is_saved_password_clear(menu_data) ||
					    is_buffer_password_clear()) {
						// Load the game.
		...
		if (menu_data->selected_sub_item ==
		    FS_BTN_CONFIRM_QUIT) { // On the Cancel option, reproduce vanilla
			                   // behaviour and reset the buffer password.
			if (z64_game.common.input[0].pad_pressed.a ||
			    z64_game.common.input[0].pad_pressed.s ||
			    z64_game.common.input[0].pad_pressed.b) {
				z64_PlaySFXID(NA_SE_SY_FSEL_CLOSE);
				menu_data->menu_transition++;
				reset_buffer();
			}
		...
		if (password_index > -1) {
			...
			gDPSetCombineMode(db->p++, G_CC_MODULATEIA_PRIM, G_CC_MODULATEIA_PRIM);
			gDPSetPrimColor(db->p++, 0, 0, 0xFF, 0xFF, 0xFF, 0xFF);
			text_print_size("Enter Password", left_password, top_password,
			                TEXT_WIDTH);
			text_flush_size(db, TEXT_WIDTH, TEXT_HEIGHT, 0, 0);
			...
			if (is_buffer_password_clear()) {
				...
				extended_sram_file_t* file =
					&(menu_data->sram_buffer->primary_saves[menu_data->selected_file]);
				extended_savecontext_static_t* extended =
					&(file->additional_save_data.extended);
				for (uint8_t i = 0; i < PASSWORD_LENGTH; i++) {
			...
			for (uint8_t i = 0; i < password_index + 1; i++) {
				gDPSetPrimColor(db->p++, 0, 0, 0xF4, 0xEC, 0x30,
				                0xFF); // Yellow C buttons
				...
				sprite_draw(db, &ocarina_button_sprite, buffer_password[i] - 1,
				            left_password + i * (BUTTON_WIDTH + 5), top_password + 0x0C,
...
void draw_file_select_hash(uint32_t fade_out_alpha, z64_menudata_t* menu_data) {
	...
	gSPTextureRectangle(db->p++, 0, 0, Z64_SCREEN_WIDTH << 2,
	                    Z64_SCREEN_HEIGHT << 2, 0, 0, 0, 1 << 10, 1 << 10);
}

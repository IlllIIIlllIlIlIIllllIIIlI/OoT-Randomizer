...
int32_t DoorWarp1_PlayerInRange_Overwrite(z64_actor_t* actor,
                                          z64_game_t* game) {
	// Check vanilla range
	if (DoorWarp1_PlayerInRange(actor, game)) {
		// Check if dungeon reward has already been collected
		if (extended_savectx
		    .collected_dungeon_rewards[game->scene_index - 0x0011]) {
			return true;
		}

		// Null out blue warp parent if it is still the dungeon boss
		if (z64_ActorHasParent(actor, game) &&
		    (actor->parent != &z64_link.common)) {
			actor->parent = NULL;
		}

		// Link will attach as the parent actor once the GetItem is accepted. Until
		// then, offer the dungeon reward for Link.
		if (!z64_ActorHasParent(actor, game)) {
			// Put a dummy item value on the blue warp, which will be overwritten by
			// the medallions
		...
		if (z64_MessageGetState(((uint8_t*)(&z64_game)) + 0x20D8) ==
		    TEXT_STATE_CLOSING) {
			if ((game->scene_index != 0x17) && (game->scene_index != 0x18)) {
				extended_savectx.collected_dungeon_rewards[game->scene_index - 0x0011] =
					true;
			}
			return true;
		}

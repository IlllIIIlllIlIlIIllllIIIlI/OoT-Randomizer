...
void draw_illegal_model_text(z64_disp_buf_t* db) {
	...
	// If custom music songs are displayed, only show the text in the inventory
	// screen.
	...
	char text[str_len + 1];
	if (missing_dlist) {
		strncpy(text, "Racing advisory: model skeleton missing dlist\0",
		        str_len + 1);
	} else {
...
z64_mem_obj_t FindModelData() {
	...
	return z64_game.obj_ctxt
	       .objects[0]; // Will be checked and rejected in calling function
}
...
int FindSize(z64_mem_obj_t model, int maxsize) {
	...
	// If the footer was not found, then return max size (should only happen with
	// a vanilla model)
	return maxsize;
}

// Finds the address of the model's hierarchy so we can write the hierarchy
// pointer Based on
...
int FindHierarchy(z64_mem_obj_t model, int size) {
	...
	// the preceeding data and loop until something that's not a segmented pointer
	// is found then return the position of the last segemented pointer.
	char* data = (char*)model.data;
...
bool check_skeleton(z64_mem_obj_t model, int hierarchy, Limb* skeleton) {
	...
	for (int i = 1; i < 21; i++) {
		...
		if (i != 2 && i != 9 && dlistPointer != 0x06) {
			// Set this variable so a special message will be displayed if the xyz
			// coords are correct but a dlist is missing
			missing_dlist = true;
			return false;
		}
	}
...
void check_model_skeletons() {
	...
	// Get the maximum possible size of the object file by checking the object
	// table
	z64_object_table_t obj_file = z64_object_table[model.id];
	int maxsize = obj_file.vrom_end - obj_file.vrom_start;

	// Get the actual length of the model data by checking for the footer (or lack
	// thereof)

...
const exchange_item_t trade_quest_items[] = {
	{0, Z64_ITEM_WEIRD_EGG, Z64_EXCH_ITEM_WEIRD_EGG, PLAYER_AP_WEIRD_EGG,
	 0}, // "Weird Egg"
	{1, Z64_ITEM_CHICKEN, Z64_EXCH_ITEM_CHICKEN, PLAYER_AP_CHICKEN,
	 0}, // "Chicken"
	{2, Z64_ITEM_ZELDAS_LETTER, Z64_EXCH_ITEM_LETTER_ZELDA,
	 PLAYER_AP_LETTER_ZELDA, 0}, // "Zelda's Letter"
	{3, Z64_ITEM_KEATON_MASK, Z64_EXCH_ITEM_MASK_KEATON, PLAYER_AP_MASK_KEATON,
	 0}, // "Keaton Mask"
	{4, Z64_ITEM_SKULL_MASK, Z64_EXCH_ITEM_MASK_SKULL, PLAYER_AP_MASK_SKULL,
	 0}, // "Skull Mask"
	{5, Z64_ITEM_SPOOKY_MASK, Z64_EXCH_ITEM_MASK_SPOOKY, PLAYER_AP_MASK_SPOOKY,
	 0}, // "Spooky Mask"
	{6, Z64_ITEM_BUNNY_HOOD, Z64_EXCH_ITEM_MASK_BUNNY, PLAYER_AP_MASK_BUNNY,
	 0}, // "Bunny Hood"
	{7, Z64_ITEM_GORON_MASK, Z64_EXCH_ITEM_MASK_GORON, PLAYER_AP_MASK_GORON,
	 0}, // "Goron Mask"
	{8, Z64_ITEM_ZORA_MASK, Z64_EXCH_ITEM_MASK_ZORA, PLAYER_AP_MASK_ZORA,
	 0}, // "Zora Mask"
	{9, Z64_ITEM_GERUDO_MASK, Z64_EXCH_ITEM_MASK_GERUDO, PLAYER_AP_MASK_GERUDO,
	 0}, // "Gerudo Mask"
	{10, Z64_ITEM_MASK_OF_TRUTH, Z64_EXCH_ITEM_MASK_TRUTH, PLAYER_AP_MASK_TRUTH,
	 0}, // "Mask of Truth"
	{11, Z64_ITEM_POCKET_EGG, Z64_EXCH_ITEM_POCKET_EGG, PLAYER_AP_POCKET_EGG,
	 0}, // "Pocket Egg"
	{12, Z64_ITEM_POCKET_CUCCO, Z64_EXCH_ITEM_POCKET_CUCCO,
	 PLAYER_AP_POCKET_CUCCO, 0}, // "Pocket Cucco"
	{13, Z64_ITEM_COJIRO, Z64_EXCH_ITEM_COJIRO, PLAYER_AP_COJIRO,
	 0}, // "Cojiro"
	{14, Z64_ITEM_ODD_MUSHROOM, Z64_EXCH_ITEM_ODD_MUSHROOM,
	 PLAYER_AP_ODD_MUSHROOM, 0}, // "Odd Mushroom"
	{15, Z64_ITEM_ODD_POTION, Z64_EXCH_ITEM_ODD_POTION, PLAYER_AP_ODD_POTION,
	 0}, // "Odd Potion"
	{16, Z64_ITEM_POACHERS_SAW, Z64_EXCH_ITEM_SAW, PLAYER_AP_SAW,
	 0}, // "Poacher's Saw"
	{17, Z64_ITEM_BROKEN_GORONS_SWORD, Z64_EXCH_ITEM_SWORD_BROKEN,
	 PLAYER_AP_SWORD_BROKEN, 0}, // "Broken Goron Sword"
	{18, Z64_ITEM_PRESCRIPTION, Z64_EXCH_ITEM_PRESCRIPTION,
	 PLAYER_AP_PRESCRIPTION, 0}, // "Prescription"
	{19, Z64_ITEM_EYEBALL_FROG, Z64_EXCH_ITEM_FROG, PLAYER_AP_FROG,
	 0}, // "Eyeball Frog"
	{20, Z64_ITEM_EYE_DROPS, Z64_EXCH_ITEM_EYEDROPS, PLAYER_AP_EYEDROPS,
	 0}, // "Eye Drops"
	{21, Z64_ITEM_CLAIM_CHECK, Z64_EXCH_ITEM_CLAIM_CHECK, PLAYER_AP_CLAIM_CHECK,
	 0} // "Claim Check"
};
...
uint32_t SaveFile_TradeItemIsTraded(uint16_t itemId) {
	...
	uint16_t tradeItemNum = GetTradeItemIndex(itemId);
	uint32_t traded =
		(z64_file.scene_flags[0x62].unk_00_ & (0x1 << tradeItemNum)) != 0;
	if (itemId >= Z64_ITEM_POCKET_EGG && itemId <= Z64_ITEM_CLAIM_CHECK &&
...
uint16_t SaveFile_NextOwnedTradeItem(uint16_t itemId) {
	if (IsTradeItem(itemId)) {
		...
		while ((potentialItem != itemId) &&
		       !SaveFile_TradeItemIsOwned(potentialItem)) {
			potentialItem++;
...
void UpdateTradeEquips(uint16_t itemId, int16_t tradeSlot) {
	...
	// z64_link.exchange_item_id =
	// trade_quest_items[GetTradeItemIndex(itemId)].exchange_item_id;
	for (int i = 0; i < 3; i++) {
...
void TurnInTradeItem(uint16_t itemId) {
	if (SaveFile_TradeItemIsOwned(itemId) &&
	    !SaveFile_TradeItemIsTraded(itemId)) {
...
z64_actor_t* IsTradeItemTraded(int16_t itemActionParam,
                               z64_actor_t* targetActor) {
	if ((itemActionParam >= PLAYER_AP_LETTER_ZELDA &&
	     itemActionParam <= PLAYER_AP_CHICKEN) ||
	    (itemActionParam >= PLAYER_AP_POCKET_EGG &&
	     itemActionParam < PLAYER_AP_CLAIM_CHECK)) {
...
int32_t Inventory_ReplaceItem_Override(z64_game_t* play, uint16_t oldItem,
                                       uint16_t newItem) {
	int16_t i;
	int32_t result = 0;
...
uint16_t SetupMaskShopHelloDialogOverride(EnOssan* maskShop) {
	...
	if (traded != paid) {
		...
		if (GET_ITEMGETINF(ITEMGETINF_3B)) {
			return 0x70AC;
		} else if (!GET_ITEMGETINF(ITEMGETINF_3A) &&
		           !GET_ITEMGETINF(ITEMGETINF_24) &&
...
void TryPaybackMaskOverride(EnOssan* maskShop, z64_game_t* play) {
	...
	if (z64_file.rupees < price) {
		...
		if (maskShop->happyMaskShopState ==
		    OSSAN_HAPPY_STATE_REQUEST_PAYMENT_KEATON_MASK) {
		...
		if (traded != paid) {
			if (SaveFile_TradeItemIsTraded(Z64_ITEM_SKULL_MASK) &&
			    !SaveFile_MaskIsPaid(Z64_ITEM_SKULL_MASK)) {
				Message_ContinueTextbox(play, 0x70C4);
				maskShop->happyMaskShopState =
					OSSAN_HAPPY_STATE_REQUEST_PAYMENT_SKULL_MASK;
				return;
			}
			if (SaveFile_TradeItemIsTraded(Z64_ITEM_SPOOKY_MASK) &&
			    !SaveFile_MaskIsPaid(Z64_ITEM_SPOOKY_MASK)) {
				Message_ContinueTextbox(play, 0x70C5);
				maskShop->happyMaskShopState =
					OSSAN_HAPPY_STATE_REQUEST_PAYMENT_SPOOKY_MASK;
				return;
			}
			if (SaveFile_TradeItemIsTraded(Z64_ITEM_BUNNY_HOOD) &&
			    !SaveFile_MaskIsPaid(Z64_ITEM_BUNNY_HOOD)) {

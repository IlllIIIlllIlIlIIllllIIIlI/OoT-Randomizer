...
#define BLOCK_DPAD \
	(0x00000001 | 0x00000002 | 0x00000080 | 0x00000400 | 0x10000000 | 0x20000000)
...
#define CAN_DRAW_TRADE_DPAD                                                  \
	(z64_game.pause_ctxt.state == PAUSE_STATE_MAIN &&                          \
	 ...
	 ((z64_game.pause_ctxt.cursor_point[PAUSE_ITEM] == Z64_SLOT_ADULT_TRADE && \
	  ...
	  (z64_game.pause_ctxt.cursor_point[PAUSE_ITEM] == Z64_SLOT_CHILD_TRADE && \
	   CFG_CHILD_TRADE_SHUFFLE)))

#define CAN_DRAW_OCARINA_BUTTONS                                           \
	(z64_game.pause_ctxt.state == PAUSE_STATE_MAIN &&                        \
	 z64_game.pause_ctxt.screen_idx == 0 &&                                  \
	 (!z64_game.pause_ctxt.changing || z64_game.pause_ctxt.changing == 3) && \
	 (z64_game.pause_ctxt.cursor_point[PAUSE_ITEM] == Z64_SLOT_OCARINA &&    \
	  SHUFFLE_OCARINA_BUTTONS))

#define CAN_USE_TRADE_DPAD \
	(CAN_DRAW_TRADE_DPAD && z64_game.pause_ctxt.changing != 3)
...
#define CAN_USE_DPAD                                                    \
	(((z64_link.state_flags_1 & BLOCK_DPAD) == 0) &&                      \
	 ((uint32_t)z64_ctxt.state_dtor == z64_state_ovl_tab[3].vram_dtor) && \
	 (z64_file.game_mode == 0) && ((z64_event_state_1 & 0x20) == 0) &&    \
...
#define CAN_USE_OCARINA                                              \
	(z64_game.pause_ctxt.state == PAUSE_STATE_OFF &&                   \
	 (z64_file.items[Z64_SLOT_OCARINA] == Z64_ITEM_FAIRY_OCARINA ||    \
	  z64_file.items[Z64_SLOT_OCARINA] == Z64_ITEM_OCARINA_OF_TIME) && \
	 !z64_game.restriction_flags.ocarina &&                            \
	 ((z64_link.state_flags_1 & BLOCK_ITEMS) == 0) &&                  \
	 (!(z64_link.state_flags_3 & (1 << 7))) &&                         \
	 z64_game.bombchuBowlingStatus == 0 && z64_game.shootingGalleryStatus == 0)

#define CAN_USE_CHILD_TRADE                                          \
	(z64_game.pause_ctxt.state == PAUSE_STATE_OFF &&                   \
	 z64_file.items[Z64_SLOT_CHILD_TRADE] >= Z64_ITEM_WEIRD_EGG &&     \
	 z64_file.items[Z64_SLOT_CHILD_TRADE] <= Z64_ITEM_MASK_OF_TRUTH && \

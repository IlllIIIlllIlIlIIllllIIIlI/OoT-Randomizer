...
void set_triforce_render() {
	...
	frames =
		frames > TRIFORCE_FRAMES_FADE_INTO ? TRIFORCE_FRAMES_FADE_INTO : frames;
}

void draw_triforce_count(z64_disp_buf_t* db) {
	// Must be triforce hunt and triforce should be drawable, and we should either
	// be on the pause screen or the render triforce flag should be set
	...
	if (z64_game.pause_ctxt.state == PAUSE_STATE_MAIN) {
		...
		if (frames <= TRIFORCE_FRAMES_FADE_INTO) {
			...
			alpha = 255;
		           ...
		           TRIFORCE_FRAMES_FADE_AWAY) {
			alpha = (frames - TRIFORCE_FRAMES_FADE_INTO - TRIFORCE_FRAMES_VISIBLE) *
			        255 / TRIFORCE_FRAMES_FADE_AWAY;
			alpha = 255 - alpha;
		} else {
	...
	int draw_y_triforce =
		Z64_SCREEN_HEIGHT - (triforce_sprite.tile_h * 1.5) + 3 + 1;
	...
	sprite_draw(db, &triforce_sprite, 0, draw_x, draw_y_triforce,
	            triforce_sprite.tile_w, triforce_sprite.tile_h);

...
#define KAKARIKO_WEATHER_REPORT_A_MIX_OF_SUNNY_AND_OVERCAST_SKIES_WITH_A_HIGH_OF_32_DEGREES \
	1
...
void override_weather_state() {
	z64_fog_state = 0;
	KAKARIKO_WEATHER_FORECAST =
		KAKARIKO_WEATHER_REPORT_SUNNY_WITH_A_HIGH_OF_28_DEGREES;
	...
	if (z64_file.forest_medallion && z64_file.fire_medallion &&
	    z64_file.water_medallion &&
	    !(z64_file.scene_flags[24].clear &
	      0x02)) { // have not beaten Bongo Bongo
		switch (z64_game.scene_index) {
		case 82:
		case 83:
			KAKARIKO_WEATHER_FORECAST =
				KAKARIKO_WEATHER_REPORT_SUNNY_WITH_A_CHANCE_OF_SEVERE_THUNDERSTORMS;
			switch (z64_file.entrance_index) {
	...
	if (!(z64_file.event_chk_inf[4] & 0x0200)) { // have not beaten Fire Temple
		...
		case 97: // Death Mountain Crater
			if (!KAKARIKO_WEATHER_FORECAST) {
				KAKARIKO_WEATHER_FORECAST =
					KAKARIKO_WEATHER_REPORT_A_MIX_OF_SUNNY_AND_OVERCAST_SKIES_WITH_A_HIGH_OF_32_DEGREES;
			}
			switch (z64_file.entrance_index) {
			case 0x00DB: // Kakariko Entrance

...
xflag_t drop_collectible_override_flag;  // Flag used by hacks in
                                         // Item_DropCollectible to override the
                                         // item being dropped. Set it to the
                                         // flag for the overridden item.
...
// These tables contain the flag bit offset for a particular scene/room/setup.
// These tables also index into xflag_room_blob to obtain the bit assignment for
// each actor in that room xlflag_room_blob contains a compressed table of actor
// bit assignments for each scene/room/setup. Call get_xflag_bit_offset to
// retrieve the desired offset for a flag.
...
// Pointer to a variable length array that will contain the collectible flags
// for each scene.
...
void item_overrides_init() {
	...
	// Create an actor satisfying the minimum requirements to give the player an
	// item
...
override_key_t get_override_search_key(z64_actor_t* actor, uint8_t scene,
                                       uint8_t item_id) {
	if (actor->actor_id == 0x0A) {
		...
		if (collectible_type == 0x12) { // don't override fairies. Honestly don't
			                        // think this is even necessary
			return (override_key_t){.all = 0};
		}
		EnItem00* item = (EnItem00*)actor;

		if (collectible_type == 0x06 ||
		    collectible_type == 0x11) { // heart pieces and keys
			return (override_key_t){
				       .scene = scene,
...
// Checks for the existence of override_key within the alternative override
// table and returns it override_key: The key to search for in the alternative
// override table Returns: The primary key to use if an alternative override is
// found in the table. Otherwise returns override_key
...
void activate_override(override_t override) {
	...
	active_item_text_id =
		resolve_item_text_id(item_row, active_override_is_outgoing);
	...
	active_item_fast_chest = item_row->chest_type == BROWN_CHEST ||
	                         item_row->chest_type == SILVER_CHEST ||
	                         item_row->chest_type == SKULL_CHEST_SMALL ||
...
void push_outgoing_override(override_t* override) {
	if (override->key.type != OVR_DELAYED ||
	    override->key.flag !=
	    0xFF) { // don't send items received from incoming back to outgoing
		if (OUTGOING_KEY.all == 0) {
			OUTGOING_ITEM = override->value.base.item_id;
			OUTGOING_PLAYER = override->value.base.player;
			// Set the value first and then the key, so a plugin checking whether the
			// key is present is guaranteed to see the value as well
...
inline uint32_t link_is_ready() {
	if ((z64_logo_state != 0x802C5880) && (z64_logo_state != 0) &&
	    (z64_file.game_mode == PAUSE_STATE_OFF) &&
	    (z64_game.pause_ctxt.state == 0) &&
	    // don't receive items in shops to avoid a softlock when buying an item at
	    // the same time as receiving one
	    ((z64_game.scene_index < 0x002C || z64_game.scene_index > 0x0033) &&
	     z64_game.scene_index != 0x0042 && z64_game.scene_index != 0x004B) &&
	    (z64_link.state_flags_1 & 0xFCAC2485) == 0 &&
	    (z64_link.common.unk_flags_00 & 0x0001) &&
	    (z64_link.state_flags_2 & 0x000C0000) == 0 &&
	    (z64_event_state_1 & 0x20) == 0 && (z64_game.camera_2 == 0)) {
...
void try_pending_item() {
	...
	uint16_t resolved_item_id = resolve_upgrades(override);
	item_row_t* item_row = get_item_row(resolved_item_id);
	if (override.value.base.item_id == 0xCA &&
	...
	if (item_row->collectible >= 0 && override.key.flag == 0xFF) {
		// This is an incoming collectible junk item so speed it up by spawning a
		// give immediate collectible
		EnItem00* collectible = (EnItem00*)z64_SpawnActor(
			&z64_game.actor_ctxt, &z64_game, 0x0015, z64_link.common.pos_world.x,
			z64_link.common.pos_world.y, z64_link.common.pos_world.z, 0, 0, 0,
			0x8000 | item_row->collectible);
		collectible->override = override;
		collectible->scale = collectible->actor.scale.x =
			collectible->actor.scale.y = collectible->actor.scale.z = 0.015f;
...
void handle_pending_items() {
	...
	if (link_is_ready()) {
		pop_ice_trap();
		// don't apply ice traps while playing the treasure chest game, since that
		// would allow cheesing it (dying there lets you buy another key but doesn't
		// lock already unlocked doors)
		if (ice_trap_is_pending() &&
		    (z64_game.scene_index != 0x0010 ||
		     (!SHUFFLE_CHEST_GAME && z64_game.chest_flags & 0x00000400))) {
			give_ice_trap();
		} else {
			try_pending_item();
		}
...
void get_item(z64_actor_t* from_actor, z64_link_t* link,
              int8_t incoming_item_id) {
	override_t override = {0};
	...
	if (from_actor && incoming_item_id != 0) {
		...
		if (row) {
			...
			// Only necessary for full adult trade shuffle, except for Biggoron who
			// always sets his flag.
			if ((CFG_ADULT_TRADE_SHUFFLE ||
			     action_id == Z64_ITEM_BIGGORON_SWORD) // full adult trade shuffle
			                                           // on, or vanilla incoming
			                                           // item is Biggoron Sword
			    && action_id > 0      // filter invalid items
			    && from_actor->actor_id != 0x000A &&
			    from_actor->actor_id !=
			    0x013D // filter chests (0x000A) and Medigoron (0x013D) as they
			           // aren't trading actors
			    // Fun fact, Medigoron's incoming item GI_GIANTS_KNIFE (0x0028) has
			    // the same action ID as Biggoron Sword, which could prevent trading
			    // to Biggoron if you talked to Medigoron first without this filter.
			    // Same thing happens without full adult trade shuffle and the
			    // Biggoron dupe fix above
			    && IsAdultTradeItem(
				    action_id)) { // Only set traded flags for adult trade items.
				                  // Child is handled separately.
				if (action_id ==
				    Z64_ITEM_BIGGORON_SWORD) { // special case for biggoron sword as
					                       // its ID isn't contiguous with the
					                       // other trade items
	...
	if (override.key.all == 0) {
		...
		// a shop item once.
		if (row) {
			if (row->action_id >= Z64_ITEM_KEATON_MASK &&
			    row->action_id <= Z64_ITEM_MASK_OF_TRUTH && CFG_CHILD_TRADE_SHUFFLE) {
	...
	if (from_actor->actor_id == 0x0A) {
		...
		if (override.value.base.item_id == 0x7C &&
		    override.value.base.player == PLAYER_ID &&
		    (FAST_CHESTS || active_item_fast_chest) &&
		    z64_game.scene_index != 0x0010) {
			// Use ice trap base item ID to freeze Link as the chest opens rather than
			// playing the full item get animation
			// HACK: Not used in treasure box shop since it causes crashes that seem
			// to be related to a timer being shared between ice traps and something
			// in the minigame
			base_item_id = 0x7C;
		}
		from_actor->variable =
			(from_actor->variable & 0xF01F) | (base_item_id << 5);
	}
...
// New EnItem00 function that freezes Link until the messagebox is closed.
// Similar to how skulls work.
void Collectible_WaitForMessageBox(EnItem00* this, z64_game_t* game) {
	// Put the item above Link's head and keep it spinning like the normal action
	// function
...
uint32_t loaded_scene_room_setup =
	-1; // Stores the currently cached scene/room/setup
uint16_t loaded_room_bit_offset =
	-1; // Stores the bit offset of the cached scene/room/setup
uint8_t room_flags[256];  // Stores the bit offset for each actor in the
                          // currently cached scene/room/setup

// Determine the bit offset into the collectible_override_flags for a flag
// Caches the bit offets for every actor in the current scene/room/setup into
// room_flags so that we don't have to search the table over and over.
uint16_t get_xflag_bit_offset(xflag_t* flag) {
	...
	// Index xflag_scene_table to get the offset into the room table for the
	// current scene
	uint32_t test_scene_room_setup;

	// Check if we're in a grotto because we calculate grotto scene/room/setup
	// differently because grottos are dumb
	if (is_grotto) {
		test_scene_room_setup = (flag->scene << 24) +
		                        (flag->grotto.grotto_id << 8) + (flag->grotto.room);
	} else {
		test_scene_room_setup =
			(flag->scene << 24) + (flag->setup << 6) + (flag->room);
	}
	...
	if (test_scene_room_setup != loaded_scene_room_setup) {
		...
		// First byte in the xflag_room_table block for this scene is the number of
		// rooms/setups
		...
		for (i = 0; i < room_setup_count; i++) {
			...
			if (flag->scene == 0x3E) {
				// If we're in a the room/setup entries are stored differently because
				// we need 2 bytes to represent them
			...
			if ((room_id_temp == room_id) && setup_id_temp == setup_id) {
				// Match. Read the next 2 bytes which contains the byte offset into the
				// xflag_room_blob table
				room_byte_offset = (xflag_room_table[room_table_index] << 8) +
				                   xflag_room_table[room_table_index + 1];
				break;
			}
			// Doesn't match so skip to the next entry
			room_table_index += 2;
		}
		// If we get here and room_byte_offset is still 0xFFFF then the room/setup
		// combination for this flag wasn't found in the table. Just return 0xFFFF
		if (room_byte_offset == 0xFFFF) {
			return 0xFFFF;
		}

		// Now load the actor flags from the compressed data starting at
		// room_byte_offset in xflag_room_blob

		// Read the bit offset for the current room/setup. First uint16_t in the
		// blob
		loaded_room_bit_offset = (xflag_room_blob[room_byte_offset] << 8) +
		                         (xflag_room_blob[room_byte_offset + 1]);
		room_byte_offset += 2;

		// Read the next byte from the blob. This contains the size, in bytes, of
		// the run-length coded data to follow
	...
	// Substract 1 from the value in room_flags because we use 0 to indicate that
	// there is no flag for that actor.
	if (loaded_room_bit_offset != -1) {
		if (is_grotto) {
			if (room_flags[flag->grotto.flag])
				return loaded_room_bit_offset + room_flags[flag->grotto.flag] - 1 +
				       flag->grotto.subflag;
		} else if (room_flags[flag->flag]) {
			return loaded_room_bit_offset + room_flags[flag->flag] - 1 +
			       flag->subflag;
		}
	}
	return 0xFFFF;
...
bool Get_NewFlag(xflag_t* flag) {
	if (flag->all) { // Check if this is one of our collectibles
		...
		if (flag_bit_offset != 0xFFFF) { // get_xflag_bit_offset will return 0xFF
			                         // is the flag is not found in the tables
			return collectible_override_flags[flag_bit_offset / 8] &
			       (0x80 >> (flag_bit_offset % 8));
		}
	}
	return true;
}
...
void Set_NewFlag(xflag_t* flag) {
	uint16_t flag_bit_offset = get_xflag_bit_offset(flag);
	if (flag_bit_offset != 0xFFFF) { // get_xflag_bit_offset will return 0xFF is
		                         // the flag is not found in the tables
		collectible_override_flags[flag_bit_offset / 8] |=
			(0x80 >> (flag_bit_offset % 8));
	}
}

// Hack at the end of Item_DropCollectible to not set the time to live, or clear
// the "room_index" if the collectible is being overridden. This allows the the
// item to not despawn after a few seconds like normal dropped collectibles. Not
// clearing room_index to -1 causes collectible items to despawn upon switching
// rooms.
void Item_DropCollectible_Room_Hack(EnItem00* spawnedActor) {
	if (spawnedActor->override.key.all &&
	    !Get_NewFlag(
		    &(Actor_GetAdditionalData(&(spawnedActor->actor))
		      ->flag))) { // Check if we should override the collectible
...
// Prevent overridden collectible items from despawning when changing to a room
// where they are still being drawn.
...
// Hack in EnItem00_Init where it checks whether or not to kill the actor based
// on the collectible flag. We use this point to determine if this is an
// overriden collectible and store that information in the actor.
bool Item00_KillActorIfFlagIsSet(z64_actor_t* actor) {
	...
	if (this->override.key
	    .all) { // If an override exists and we haven't already collected it
		extra->flag = flag;
		return 0; // Return 0 to continue spawning the actor
	}

	// If we get here than we either don't have an override, or the override has
	// already been collected. Perform normal collectible flag check.
...
// This function returns the provided drop ID if bombs should be dropped, or
// ITEM00_ARROWS_SINGLE for chu drops.
...
// Override the drop ID (what item to spawn) in the call to
// Item_DropCollectible/Item_DropCollectible2. Drops all overridden items as
// green rupees for consistency (so they don't float like hearts do). The rest
// of the code is just the rewrite of the vanilla code for converting drops
// based on age/health.
int16_t get_override_drop_id(int16_t dropId) {
	...
	if (!Get_NewFlag(&drop_collectible_override_flag) &&
	    dropId != ITEM00_HEART_PIECE && dropId != ITEM00_SMALL_KEY &&
	    dropId != ITEM00_HEART_CONTAINER && dropId != ITEM00_SHIELD_DEKU &&
	    dropId != ITEM00_SHIELD_HYLIAN && dropId != ITEM00_TUNIC_ZORA &&
	...
	if (LINK_IS_ADULT) {
		...
		if (dropId == ITEM00_ARROWS_SMALL || dropId == ITEM00_ARROWS_MEDIUM ||
		    dropId == ITEM00_ARROWS_LARGE) {
			dropId = ITEM00_SEEDS;
		}
	}

	// Chu bag drops, convert bomb drop to bombchu drop under certain
	// circumstances
	if (FREE_BOMBCHU_DROPS &&
	    (dropId == ITEM00_BOMBS_A || dropId == ITEM00_BOMBS_SPECIAL ||
	     dropId == ITEM00_BOMBS_B)) {
		if (z64_file.items[Z64_SLOT_BOMB] != ITEM_NONE &&
		    z64_file.items[Z64_SLOT_BOMBCHU] !=
		    ITEM_NONE) { // we have bombs and chus
			return drop_bombs_or_chus(dropId);
		} else if (z64_file.items[Z64_SLOT_BOMB] != ITEM_NONE) { // only have bombs
			return dropId; // don't do anything because this is already the right
			               // drop ID
		} else if (z64_file.items[Z64_SLOT_BOMBCHU] !=
		           ITEM_NONE) { // only have chus
	...
	if ((dropId == ITEM00_BOMBS_A || dropId == ITEM00_BOMBS_SPECIAL ||
	     dropId == ITEM00_BOMBS_B) &&
	    z64_file.items[ITEM_BOMB] == ITEM_NONE) {
		return -1;
	}
	if ((dropId == ITEM00_ARROWS_SMALL || dropId == ITEM00_ARROWS_MEDIUM ||
	     dropId == ITEM00_ARROWS_LARGE) &&
	    z64_file.items[ITEM_BOW] == ITEM_NONE) {
		return -1;
	}
	if ((dropId == ITEM00_MAGIC_LARGE || dropId == ITEM00_MAGIC_SMALL) &&
	...
	if (dropId == ITEM00_HEART &&
	    (z64_file.energy_capacity == z64_file.energy || NO_COLLECTIBLE_HEARTS)) {
...
void dispatch_item(uint16_t resolved_item_id, uint8_t player,
                   override_t* override, item_row_t* item_row) {
...
// Override hack for freestanding collectibles (rupees, recovery hearts, sticks,
// nuts, seeds, bombs, arrows, magic jars. Pieces of heart, heart containers,
// small keys handled by the regular get_item function)
uint8_t item_give_collectible(uint8_t item, z64_link_t* link,
                              z64_actor_t* from_actor) {
	EnItem00* pItem = (EnItem00*)from_actor;

	override_t override = pItem->override;
	xflag_t flag = Actor_GetAdditionalData(from_actor)->flag;

	// Check if we should override the item. We have logic in the randomizer to
	// not include excluded items in the override table.
	if (override.key.all == 0 || Get_NewFlag(&flag)) {
		z64_GiveItem(&z64_game,
		             items[item]); // Give the regular item (this is what is
		                           // normally called by the non-hacked function)
		return 0;
	}

	if (!collectible_mutex &&
	    pItem->actor.main_proc !=
	    NULL) { // Check our mutex so that only one collectible can run at a
		    // time (if 2 run on the same frame you lose the message).
		    // Also make sure that this actor hasn't already been killed.
		...
		Set_NewFlag(&flag);
		// if (item == ITEM00_HEART_PIECE || item == ITEM00_SMALL_KEY) { // Don't
		// allow heart pieces or small keys to be collected a second time. This is
		// really just for the "Drop" types.
		...
		if (item_row->collectible >= 0) { // Item is one of our base collectibles
			collectible_mutex = NULL;
			pItem->actor.dropFlag = 1; // Store this so the draw function knows to
			                           // keep drawing the override.
			...
			if (item_row->collectible <= ITEM00_RUPEE_RED ||
			    item_row->collectible == ITEM00_RUPEE_PURPLE ||
			    item_row->collectible == ITEM00_RUPEE_ORANGE) {
				sfxId = NA_SE_SY_GET_RUPY;
			}

			// If the item is for another player, use a custom action to make the item
			// fly off the screen
			if (player != PLAYER_ID) {
				pItem->timeToLive = 15; // same time to live as regular bounce effect.
				pItem->unk_154 =
					35; // not quite sure but this is what the vanilla game does.
				...
				sfxId = NA_SE_SY_FSEL_DECIDE_L; // Play a different sound effect for
				                                // outgoing items. This is one from the
				                                // file select screen.
				z64_Audio_PlaySoundGeneral(sfxId, (void*)0x80104394, 4,
				                           (float*)0x801043A0, (float*)0x801043A0,
				                           (uint8_t*)0x801043A8);
			...
			z64_Audio_PlaySoundGeneral(sfxId, (void*)0x80104394, 4,
			                           (float*)0x801043A0, (float*)0x801043A0,
			                           (uint8_t*)0x801043A8);
			return 3; // Return to the original function so it can draw the
			          // collectible above our head.
		}

		// draw message box and play get item sound (like when a skull is picked up)
		z64_Audio_PlayFanFare(NA_BGM_SMALL_ITEM_GET);

		z64_DisplayTextbox(&z64_game,
		                   resolve_item_text_id(item_row, player != PLAYER_ID), 0);

		// Set up
		pItem->timeToLive = 15; // unk_15A is a frame timer that is decremented
		                        // each frame by the main actor code.
		pItem->unk_154 =
			35; // not quite sure but this is what the vanilla game does.
		pItem->getItemId = 0;
		pItem->actor.rot_2.z = 0;
		...
		pItem->actionFunc =
			Collectible_WaitForMessageBox; // Set up the EnItem00 action function
		                                       // to wait for the message box to close.
...
int give_sarias_gift() {
	...
	if (received_sarias_gift == 0) {
		if (OCARINAS_SHUFFLED) push_delayed_item(0x02);
...
void fairy_ocarina_getitem() {
	override_key_t lw_gift_from_saria = {
		.scene = 0xFF, .type = OVR_DELAYED, .flag = 0x02
	};
	...
	PLAYER_NAME_ID = override.value.base.player;
	z64_DisplayTextbox(
		&z64_game, resolve_item_text_id(item_row, PLAYER_NAME_ID != PLAYER_ID),
		0);
}

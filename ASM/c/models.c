...
void load_object_file(uint32_t object_id, uint8_t* buf) {
	...
	if (object_id <= num_vanilla_objects) {
		...
		z64_object_table_t* extended_table =
			(z64_object_table_t*)(&EXTENDED_OBJECT_TABLE);
...
typedef void (*pre_draw_fn)(z64_actor_t* actor, z64_game_t* game,
                            uint32_t unknown);
...
void draw_model_low_level(uint8_t graphic_id_minus_1, z64_actor_t* actor,
                          z64_game_t* game) {
...
void lookup_model(model_t* model, z64_actor_t* actor, z64_game_t* game,
...
// Shop draw function for each shelf slot, replaces GetItem_Draw inside of
// EnGirlA_Draw
void shop_draw(z64_actor_t* actor, z64_game_t* game) {
	...
	override_t override = lookup_override((z64_actor_t*)this,
	                                      z64_game.scene_index, this->getItemId);
	...
	/*
	    ...
	    the override model.
	 */
	if (override.key.all && this->getItemId &&
	    game->obj_ctxt.objects[this->objBankIndex].id != 0x148) {
		lookup_model_by_override(&model, override);
		if (model.object_id != 0x0000) {
			draw_model(model, actor, game, 0.0);
		}
	} else {
		// vanilla draw function if the slot is a regular shop item, shuffled or
		// unshuffled
...
// collectible draw function for common items (sticks, nuts, arrows/seeds/etc.
// and keys)

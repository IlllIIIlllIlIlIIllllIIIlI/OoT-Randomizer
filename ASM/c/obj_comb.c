...
// Hack beehives to drop a collectible w / an extended flag, based on the grotto
// param
...
void ObjComb_Update(z64_actor_t* thisx, z64_game_t* game) {
	...
	this->actor.rot_2.x =
		z64_Math_SinS(this->unk_1B2) * this->unk_1B0 + this->actor.rot_init.x;
...
/*
   ...
   texture); gSPEndDisplayList(gfx->poly_opa.d + 1); gMoveWd(gfx->poly_opa.p++,
   G_MW_SEGMENT, 9 * sizeof(int), gfx->poly_opa.d);
 */

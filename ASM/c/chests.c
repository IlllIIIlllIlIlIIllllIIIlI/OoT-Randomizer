...
#define box_obj_idx(actor) \
	((int8_t*)actor)[0x015A] // forest_hallway_actor->box_obj_idx
		...
		if (override.value.base.item_id != 0) {
			...
			if (CHEST_SIZE_MATCH_CONTENTS || CHEST_SIZE_TEXTURE) {
				if (item_row->chest_type == BROWN_CHEST ||
					...
					size = (scene == 0x10 && actor->variable != 0x4ECA &&
					        !SHUFFLE_CHEST_GAME)
		     ? BROWN_CHEST
		     : SMALL_CHEST;
				} else {
...
void set_chest_texture(z64_gfx_t* gfx, uint8_t chest_type, Gfx** opa_ptr) {
	...
	gfx->poly_opa.d -= 4;
	gDPSetTextureImage(gfx->poly_opa.d, G_IM_FMT_RGBA, G_IM_SIZ_16b, 1,
	                   frontTexture);
	gSPEndDisplayList(gfx->poly_opa.d + 1);
	gDPSetTextureImage(gfx->poly_opa.d + 2, G_IM_FMT_RGBA, G_IM_SIZ_16b, 1,
	                   baseTexture);
...
void draw_chest_base(z64_game_t* game, z64_actor_t* actor, Gfx** opa_ptr) {
	...
	gSPMatrix((*opa_ptr)++, write_matrix_stack_top(gfx),
...
void draw_chest(z64_game_t* game, int32_t part, void* unk, void* unk2,
...
_Bool should_draw_forest_hallway_chest(z64_actor_t* actor, z64_game_t* game) {
	...
	if (CHEST_LENS_ONLY && !(game->chest_flags & 0x4000) &&
	    !game->actor_ctxt.lens_active) {
		return false;
	}

	// Vanilla code
	return (box_obj_idx(actor) > 0) &&
	       ((box_obj_idx(actor) = z64_ObjectIndex(&game->obj_ctxt, OBJECT_BOX)) >
	        0) &&
...
void draw_forest_hallway_chest_base() {
	...
	draw_chest_base(&z64_game, (z64_actor_t*)&dummy_chest,
	                &z64_game.common.gfx->poly_opa.p);
}
...
void draw_forest_hallway_chest_lid() {
	...
	draw_chest_lid(&z64_game, (z64_actor_t*)&dummy_chest,
	               &z64_game.common.gfx->poly_opa.p);
}

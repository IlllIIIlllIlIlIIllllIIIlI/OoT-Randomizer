...
#define ITEM_ICON_SIZE \
	(ITEM_ICON_WIDTH * ITEM_ICON_HEIGHT * 4) // The size in bytes of an item icon
...
typedef struct {
	/* 0x00 */ int16_t id;
	/* 0x02 */ uint8_t
	        category; // Classifies actor and determines when it will update or draw
	/* 0x04 */ uint32_t flags;
...
typedef struct {
	...
	/* 0xE306 */ uint8_t
	        msgBufDecoded[200]; // decoded message buffer, may be smaller than this
	/* 0xE3CE */ uint16_t msgBufPos; // original name : "rdp"
	/* 0xE3D0 */ uint16_t unk_E3D0; // unused, only ever set to 0
	/* 0xE3D2 */ uint16_t
	        textDrawPos; // draw all decoded characters up to this buffer position
	...
	/* 0xE3E6 */ uint8_t choiceNum; // textboxes that are not choice textboxes
	                                // have a choiceNum of 1
	/* 0xE3E7 */ uint8_t stateTimer;
	/* 0xE3E8 */ uint16_t textDelayTimer;
	/* 0xE3EA */ uint16_t textDelay;
	/* 0xE3EA */ uint16_t
	        lastPlayedSong; // original references : "Ocarina_Flog" , "Ocarina_Free"
	/* 0xE3EE */ uint16_t ocarinaMode; // original name : "ocarina_mode"
	/* 0xE3F0 */ uint16_t ocarinaAction; // original name : "ocarina_no"
	/* 0xE3F2 */ uint16_t unk_E3F2; // this is like "lastPlayedSong" but set less
	                                // often, original name : "chk_ocarina_no"
	...
	/* 0xE3F8 */ uint8_t
	        textboxBackgroundUnkArg; // unused, set by the textbox background control
	                                 // character arguments
	...
	/* 0xE40C */ int16_t
	        disableWarpSongs; // warp song flag set by scene commands
...
typedef enum {
	...
	/* 0x01 */ CAM_MODE_Z_PARALLEL, // Holding Z but with no target, keeps the
	                                // camera aligned
	...
	/* 0x07 */ CAM_MODE_AIM_ADULT, // First person aiming as adult: bow and
	                               // hookshot
	/* 0x08 */ CAM_MODE_Z_AIM, // Third person aiming for all items, child and
	                           // adult
	/* 0x09 */ CAM_MODE_HOOKSHOT_FLY, // Player being pulled by the hookshot to a
	                                  // target
	/* 0x0A */ CAM_MODE_AIM_BOOMERANG, // Aiming the boomerang
	/* 0x0B */ CAM_MODE_AIM_CHILD, // First person aiming as child: slingshot
	/* 0x0C */ CAM_MODE_Z_WALL_CLIMB, // Climbing a wall with Z pressed: ladders
	                                  // and vines
	/* 0x0D */ CAM_MODE_JUMP,     // Falling in air from a ledge jump
	/* 0x0E */ CAM_MODE_LEDGE_HANG, // Hanging from and climbing a ledge
	/* 0x0F */ CAM_MODE_Z_LEDGE_HANG, // Hanging from and climbing a ledge with Z
	                                  // pressed
	/* 0x10 */ CAM_MODE_FREE_FALL, // Falling in air except for a ledge jump or
	                               // knockback
	/* 0x11 */ CAM_MODE_CHARGE, // Charging a spin attack
	/* 0x12 */ CAM_MODE_STILL, // Attacks without Z pressed, falling in air from
	                           // knockback
	/* 0x13 */ CAM_MODE_PUSH_PULL,
	/* 0x14 */ CAM_MODE_FOLLOW_BOOMERANG, // Boomerang has been thrown,
	                                      // force-target the boomerang as it
	                                      // flies
	/* 0x15 */ CAM_MODE_MAX,
} CameraModeType;
...
typedef struct EnGSwitch {
	...
	/* 0x0162 */ int16_t
	        delayTimer; // delay between the two blue rupees appearing
	...
	/* 0x0168 */ int16_t moveState; // Appear or retreat (for blue rupees and the
	                                // stationary green one)
...
typedef enum {
	/*  0 */ PAUSE_STATE_OFF,
	/*  1 */ PAUSE_STATE_WAIT_LETTERBOX, // Request no letterboxing and wait for
	                                     // it.
	/*  2 */ PAUSE_STATE_WAIT_BG_PRERENDER, // Wait for the pause background
	                                        // prerender to be done.
	/*  3 */ PAUSE_STATE_INIT, // Load data and initialize/setup various things.
	/*  4 */ PAUSE_STATE_OPENING_1, // Animate the pause menu coming together
	                                // with rotations and other animations.
	/*  5 */ PAUSE_STATE_OPENING_2, // Finish some animations for opening the
	                                // menu.
	...
	/* 19 */ PAUSE_STATE_RESUME_GAMEPLAY // Handles returning to normal gameplay
	                                     // once the pause menu is visually
	                                     // closed
} PauseState;
...
typedef enum {
	...
	/* 1 */ PAUSE_BG_PRERENDER_SETUP, // The current frame is only drawn for the
	                                  // purpose of serving as the pause
	                                  // background.
	/* 2 */ PAUSE_BG_PRERENDER_PROCESS, // The previous frame was
	                                    // PAUSE_BG_PRERENDER_SETUP, now apply
	                                    // prerender filters.
	/* 3 */ PAUSE_BG_PRERENDER_READY, // The pause background is ready to be
	                                  // used.
...
typedef int32_t(*z64_Flags_GetCollectibleFunc)(z64_game_t* game,
                                               uint32_t flag);
typedef void(*z64_Audio_PlaySoundGeneralFunc)(uint16_t sfxId,void* pos,
                                              uint8_t token,float* freqScale,
                                              float* a4,uint8_t* reverbAdd);
...
typedef void(*z64_LoadRoom_proc)(z64_game_t* game,void* p_ctxt_room_index,
                                 uint8_t room_index);
typedef void(*z64_UnloadRoom_proc)(z64_game_t* game,void* p_ctxt_room_index);
typedef void(*z64_Io_proc)(uint32_t dev_addr,void* dram_addr,uint32_t size,
                           int32_t direction);
...
typedef void(*z64_LinkDamage_proc)(z64_game_t* ctxt,z64_link_t* link,
                                   uint8_t damage_type,float unk_00,
                                   uint32_t unk_01,uint16_t unk_02);
typedef void(*z64_LinkInvincibility_proc)(z64_link_t* link,uint8_t frames);
typedef float* (*z64_GetMatrixStackTop_proc)();
typedef void(*SsSram_ReadWrite_proc)(uint32_t addr,void* dramAddr,
...
typedef void(*z64_EffectSsKiraKira_SpawnSmall_proc)(
	z64_game_t* globalCtx,z64_xyzf_t* pos,z64_xyzf_t* velocity,
	z64_xyzf_t* accel,colorRGBA8_t* primColor,colorRGBA8_t* envColor);

typedef int32_t(*z64_ObjectSpawn_proc)(z64_obj_ctxt_t* object_ctx,
                                       int16_t object_id);
typedef int32_t(*z64_ObjectIndex_proc)(z64_obj_ctxt_t* object_ctx,
                                       int16_t object_id);
typedef int32_t(*z64_ObjectIsLoaded_proc)(z64_obj_ctxt_t* object_ctx,
                                          int32_t bank_index);

typedef int32_t(*z64_ActorHasParent_proc)(z64_actor_t* actor,
                                          z64_game_t* game);
typedef int32_t(*z64_ActorOfferGetItem_proc)(z64_actor_t* actor,
                                             z64_game_t* game,
                                             int32_t get_item_id,
                                             float xz_range,float y_range);
typedef void(*z64_RandSeed_proc)(uint32_t seed);
typedef float(*z64_Rand_ZeroOne_proc)();
typedef void(*Font_LoadChar_proc)(void* font,uint8_t character,
                                  uint16_t codePointIndex);
...
typedef void(*z64_ScalePitchAndTempo_proc)(float scaleTempoAndFreq,
                                           uint8_t duration);
...
typedef void(*z64_Play_SetupRespawnPoint_proc)(z64_game_t* game,
                                               int32_t respawnMode,
...
#define z64_scene_config_table \
	((z64_SceneConfig_proc*)z64_scene_config_table_addr)
...
#define z64_MessageGetState \
	((z64_Message_GetStateFunc)z64_Message_GetState_addr)
#define z64_SetCollectibleFlags \
	((z64_Flags_SetCollectibleFunc)z64_SetCollectibleFlags_addr)
#define z64_Flags_GetCollectible \
	((z64_Flags_GetCollectibleFunc)z64_GetCollectibleFlags_addr)
...
#define z64_Audio_PlaySoundGeneral \
	((z64_Audio_PlaySoundGeneralFunc)z64_Audio_PlaySoundGeneral_addr)
#define z64_Audio_PlayFanFare \
	((z64_Audio_PlayFanFareFunc)z64_Audio_PlayFanFare_addr)
...
#define z64_LinkInvincibility \
	((z64_LinkInvincibility_proc)z64_LinkInvincibility_addr)
#define z64_GetMatrixStackTop \
	((z64_GetMatrixStackTop_proc)z64_GetMatrixStackTop_addr)
...
#define z64_ActorOfferGetItem \
	((z64_ActorOfferGetItem_proc)z64_ActorOfferGetItem_addr)
...
#define z64_Item_DropCollectible \
	((z64_Item_DropCollectible_proc)z64_Item_DropCollectible_addr)
#define z64_Item_DropCollectible2 \
	((z64_Item_DropCollectible_proc)z64_Item_DropCollectible2_addr)
#define z64_Item_DropCollectibleRandom \
	((z64_Item_DropCollectibleRandom_proc)z64_Item_DropCollectibleRandom_addr)
#define z64_Gfx_DrawDListOpa \
	((z64_Gfx_DrawDListOpa_proc)z64_Gfx_DrawDListOpa_addr)
#define z64_Math_SinS ((z64_Math_SinS_proc)z64_Math_SinS_addr)
#define z64_Rand_ZeroOne ((z64_Rand_ZeroOne_proc)z64_Rand_ZeroOne_addr)
#define z64_EffectSsKiraKira_SpawnSmall \
	((z64_EffectSsKiraKira_SpawnSmall_proc)z64_EffectSsKiraKira_SpawnSmall_addr)
#define Interface_LoadItemIcon1 \
	((Interface_LoadItemIcon1_proc)Interface_LoadItemIcon1_addr)

#define Rupees_ChangeBy ((Rupees_ChangeBy_proc)Rupees_ChangeBy_addr)

#define Message_ContinueTextbox \
	((Message_ContinueTextbox_proc)Message_ContinueTextbox_addr)
#define z64_ScalePitchAndTempo \
	((z64_ScalePitchAndTempo_proc)z64_ScalePitchAndTempo_addr)
#define z64_Audio_GetActiveSeqId \
	((z64_Audio_GetActiveSeqId_proc)z64_Audio_GetActiveSeqId_addr)
#define z64_Play_SetupRespawnPoint \
	((z64_Play_SetupRespawnPoint_proc)z64_Play_SetupRespawnPoint_addr)
...
/* macros */
#define GET_ITEMGETINF(flag) \
	(z64_file.item_get_inf[(flag) >> 4] & (1 << ((flag) & 0xF)))
#define GET_EVENTCHKINF(flag) \
	(z64_file.event_chk_inf[(flag) >> 4] & (1 << ((flag) & 0xF)))
#define SET_EVENTCHKINF(flag) \
	(z64_file.event_chk_inf[(flag) >> 4] |= (1 << ((flag) & 0xF)))
...
#define EVENTCHKINF_CARPENTERS_FREE_MASK(n) \
	(1 << EVENTCHKINF_CARPENTERS_FREE_SHIFT(n))
#define EVENTCHKINF_CARPENTERS_FREE(n)        \
	((EVENTCHKINF_CARPENTERS_FREE_INDEX << 4) | \
	 EVENTCHKINF_CARPENTERS_FREE_SHIFT(n))
#define EVENTCHKINF_CARPENTERS_FREE_MASK_ALL                                   \
	(EVENTCHKINF_CARPENTERS_FREE_MASK(0) | EVENTCHKINF_CARPENTERS_FREE_MASK(1) | \
	 EVENTCHKINF_CARPENTERS_FREE_MASK(2) | EVENTCHKINF_CARPENTERS_FREE_MASK(3))
#define GET_EVENTCHKINF_CARPENTERS_FREE_ALL()                               \
	CHECK_FLAG_ALL(z64_file.event_chk_inf[EVENTCHKINF_CARPENTERS_FREE_INDEX], \
...
#define EVENTCHKINF_SONGS_FOR_FROGS_CHOIR_MASK \
	(1 << EVENTCHKINF_SONGS_FOR_FROGS_CHOIR_SHIFT)
#define EVENTCHKINF_SONGS_FOR_FROGS_ZL_MASK \
	(1 << EVENTCHKINF_SONGS_FOR_FROGS_ZL_SHIFT)
#define EVENTCHKINF_SONGS_FOR_FROGS_EPONA_MASK \
	(1 << EVENTCHKINF_SONGS_FOR_FROGS_EPONA_SHIFT)
#define EVENTCHKINF_SONGS_FOR_FROGS_SUNS_MASK \
	(1 << EVENTCHKINF_SONGS_FOR_FROGS_SUNS_SHIFT)
#define EVENTCHKINF_SONGS_FOR_FROGS_SARIA_MASK \
	(1 << EVENTCHKINF_SONGS_FOR_FROGS_SARIA_SHIFT)
#define EVENTCHKINF_SONGS_FOR_FROGS_SOT_MASK \
	(1 << EVENTCHKINF_SONGS_FOR_FROGS_SOT_SHIFT)
#define EVENTCHKINF_SONGS_FOR_FROGS_STORMS_MASK \
	(1 << EVENTCHKINF_SONGS_FOR_FROGS_STORMS_SHIFT)
#define EVENTCHKINF_SONGS_FOR_FROGS_CHOIR     \
	((EVENTCHKINF_SONGS_FOR_FROGS_INDEX << 4) | \
	 EVENTCHKINF_SONGS_FOR_FROGS_CHOIR_SHIFT)
#define EVENTCHKINF_SONGS_FOR_FROGS_ZL        \
	((EVENTCHKINF_SONGS_FOR_FROGS_INDEX << 4) | \

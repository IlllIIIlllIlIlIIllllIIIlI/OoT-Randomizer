...
void EnWonderItem_Multitag_DrawHack(z64_xyzf_t* tags, uint32_t index,
	...
	if (this->overridden) {
		...
		z64_EffectSsKiraKira_SpawnSmall(&z64_game, &pos, &sEffectVelocity,
		                                &sEffectAccel, color, &sEffectEnvColor);
	}
}

void EnWonderItem_DropCollectible_Hack(EnWonderItem* this,
                                       z64_game_t* globalCtx,
                                       int32_t autoCollect) {
	static int16_t dropTable[] = {
		ITEM00_NUTS,          ITEM00_HEART_PIECE,  ITEM00_MAGIC_LARGE,
		ITEM00_MAGIC_SMALL,   ITEM00_HEART,        ITEM00_ARROWS_SMALL,
		ITEM00_ARROWS_MEDIUM, ITEM00_ARROWS_LARGE, ITEM00_RUPEE_GREEN,
		ITEM00_RUPEE_BLUE,    ITEM00_RUPEE_RED,    ITEM00_FLEXIBLE,
	};
	int16_t i;
	int16_t randomDrop;
	...
	if (this->overridden) {
		...
		z64_bzero(&drop_collectible_override_flag,
	...
	for (i = this->dropCount; i > 0; i--) {
		if (this->itemDrop < WONDERITEM_DROP_RANDOM) {
			if ((this->itemDrop == WONDERITEM_DROP_FLEXIBLE) || !autoCollect) {
				z64_Item_DropCollectible(globalCtx, &this->actor.pos_world,
				                         dropTable[this->itemDrop]);
			} else {
				z64_Item_DropCollectible(globalCtx, &this->actor.pos_world,
				                         dropTable[this->itemDrop] | 0x8000);
			}
		} else {
			randomDrop = this->itemDrop - WONDERITEM_DROP_RANDOM;
			if (!autoCollect) {
				z64_Item_DropCollectibleRandom(globalCtx, NULL, &this->actor.pos_world,
				                               randomDrop);
			} else {
				z64_Item_DropCollectibleRandom(globalCtx, NULL, &this->actor.pos_world,
				                               randomDrop | 0x8000);
...
void EnWonderItem_Update_Hack(EnWonderItem* this) {
	...
	if (this->overridden) {
		...
		if (color) {
			z64_EffectSsKiraKira_SpawnSmall(&z64_game, &(this->actor.pos_world),
			                                &sEffectVelocity, &sEffectAccel, color,
			                                &sEffectEnvColor);
		}
	}
}

// Hack to not kill wonderitem when switch flag is set if we need to override
// still
uint32_t EnWonderItem_Kill_Hack(EnWonderItem* this) {
	...
	if ((this->switchFlag >= 0) &&
	    z64_Flags_GetSwitch(&z64_game, this->switchFlag))
		return 1;
	return 0;
}

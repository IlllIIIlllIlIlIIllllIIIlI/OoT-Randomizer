...
void update_sword_trail_colors() {
	...
	if (CFG_RAINBOW_SWORD_INNER_ENABLED) {
		colorRGB8_t color_inner =
			get_rainbow_color(frames, SWORD_CYCLE_FRAMES_INNER);
		sword_trail_colors[1].color = color_inner;
		sword_trail_colors[3].color = color_inner;
	}

	if (CFG_RAINBOW_SWORD_OUTER_ENABLED) {
		colorRGB8_t color_outer =
			get_rainbow_color(frames, SWORD_CYCLE_FRAMES_OUTER);
	...
	if (boomerang != NULL && boomerang->main_proc != NULL) {
		...
		if (effect_index < 31) {
			...
			if (CFG_RAINBOW_BOOM_TRAIL_INNER_ENABLED) {
				trail->fx.p1_start.color =
					get_rainbow_color(frames, BOOM_CYCLE_FRAMES_INNER);
				trail->fx.p1_end.color = get_rainbow_color(
					frames + 2 * BOOM_CYCLE_FRAMES_INNER, BOOM_CYCLE_FRAMES_INNER);
			} else {
			...
			if (CFG_RAINBOW_BOOM_TRAIL_OUTER_ENABLED) {
				trail->fx.p2_start.color =
					get_rainbow_color(frames, BOOM_CYCLE_FRAMES_OUTER);
				trail->fx.p2_end.color = get_rainbow_color(
					frames + 2 * BOOM_CYCLE_FRAMES_OUTER, BOOM_CYCLE_FRAMES_OUTER);
			} else {
...
void update_bombchu_trail_colors() {
	...
	colorRGB8_t rainbow_color_inner_start =
		get_rainbow_color(frames, BOMBCHU_CYCLE_FRAMES_INNER);
	colorRGB8_t rainbow_color_inner_end = get_rainbow_color(
		frames + 2 * BOMBCHU_CYCLE_FRAMES_INNER, BOMBCHU_CYCLE_FRAMES_INNER);
	colorRGB8_t rainbow_color_outer_start =
		get_rainbow_color(frames, BOMBCHU_CYCLE_FRAMES_OUTER);
	colorRGB8_t rainbow_color_outer_end = get_rainbow_color(
		frames + 2 * BOMBCHU_CYCLE_FRAMES_OUTER, BOMBCHU_CYCLE_FRAMES_OUTER);

	z64_actor_t* explosive = z64_game.actor_list[ACTORTYPE_EXPLOSIVES].first;
	while (explosive != NULL) {
		if (explosive->main_proc != NULL &&
		    explosive->actor_id == 0xDA) // En_Bom_Chu
		{
...
void update_navi_colors() {
	...
	for (int i = 0; i < 3; i++) {
		if (rainbow_inner_enabled) z64_game.target_arr[i].color = color_inner;
	}

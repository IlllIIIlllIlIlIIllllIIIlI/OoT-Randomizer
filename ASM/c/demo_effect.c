...
extern void DemoEffect_DrawJewel(DemoEffect* this, z64_game_t* globalCtx,
                                 void* func);
extern void* DemoEffect_DrawJewel_AfterHook;

extern override_key_t CFG_BIGOCTO_OVERRIDE_KEY;

// Overrides the item model for the dungeon reward shown before the Big Octo
// fight
void DemoEffect_DrawJewel_Hook(DemoEffect* this, z64_game_t* globalCtx) {
	...
	if (this->override.key.all) {
		...
		DemoEffect_DrawJewel(
			this, globalCtx,
			resolve_overlay_addr(&DemoEffect_DrawJewel_AfterHook, 0x008B));
	}
}

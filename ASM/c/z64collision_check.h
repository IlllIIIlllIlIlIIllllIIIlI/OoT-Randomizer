...
typedef struct {
	...
	/* 0x04 */ struct z64_actor_t*
	        at; // Actor attached to what it collided with as an AT collider.
	/* 0x08 */ struct z64_actor_t*
	        ac; // Actor attached to what it collided with as an AC collider.
	/* 0x0C */ struct z64_actor_t*
	        oc; // Actor attached to what it collided with as an OC collider.
	...
	/* 0x13 */ uint8_t
	        ocFlags2; // Flags related to which colliders it can OC collide with.
	/* 0x14 */ uint8_t
	        colType; // Determines hitmarks and sound effects during AC collisions.
...
typedef struct ColliderInfo {
	/* 0x00 */ ColliderTouch
	        toucher; // Damage properties when acting as an AT collider
	/* 0x08 */ ColliderBump
	        bumper; // Damage properties when acting as an AC collider
	/* 0x14 */ uint8_t elemType; // Affects sfx reaction when attacked by Link
	                             // and hookability. Full purpose unknown.
	...
	/* 0x20 */ struct ColliderInfo*
	        atHitInfo; // element that hit the AT collider
	/* 0x24 */ struct ColliderInfo*
	        acHitInfo; // element that hit the AC collider
} ColliderInfo;   // size = 0x28

typedef struct {
	/* 0x00 */ uint8_t elemType; // Affects sfx reaction when attacked by Link
	                             // and hookability. Full purpose unknown.
	/* 0x04 */ ColliderTouch
	        toucher; // Damage properties when acting as an AT collider
	/* 0x0C */ ColliderBumpInit
	        bumper; // Damage properties when acting as an AC collider
...
#define AT_SELF \
	(1 << 6) // Can have AT collisions with colliders attached to the same actor
#define AT_TYPE_ALL                 \
	(AT_TYPE_PLAYER | AT_TYPE_ENEMY | \
	 AT_TYPE_OTHER) // Has all three damage alignments
...
#define AC_TYPE_ALL                 \
	(AC_TYPE_PLAYER | AC_TYPE_ENEMY | \
	 AC_TYPE_OTHER) // Takes damage from all three alignments
...
#define OC1_NO_PUSH \
	(1 << 2) // Does not push other colliders away during OC collisions
...
#define OC1_TYPE_ALL              \
	(OC1_TYPE_PLAYER | OC1_TYPE_1 | \
	 OC1_TYPE_2) // Can have collisions with all three OC types

#define OC2_NONE 0               // No flags set. Has no OC type
#define OC2_HIT_PLAYER (1 << 0)  // Had an OC collision with OC type player
#define OC2_UNK1 \
	(1 << 1) // Prevents OC collisions with OC2_UNK2. Some horses and toki_sword
                 // have it.
#define OC2_UNK2 \
	(1 << 2) // Prevents OC collisions with OC2_UNK1. Nothing has it.
...
#define OC2_FIRST_ONLY \
	(1 << 6) // Skips AC checks on elements after the first collision. Only used
                 // by Ganon
...
#define TOUCH_NEAREST \
	(1 << 2) // If a Quad, only collides with the closest bumper
#define TOUCH_SFX_NORMAL \
	(0 << 3)                 // Hit sound effect based on AC collider's type
...
#define BUMP_HOOKABLE \
	(1 << 2) // Can be hooked if actor has hookability flags set.
#define BUMP_NO_AT_INFO \
	(1 << 3) // Does not give its info to the AT collider that hit it.
#define BUMP_NO_DAMAGE (1 << 4)  // Does not take damage.
#define BUMP_NO_SWORD_SFX \
	(1 << 5) // Does not have a sound when hit by player-attached AT colliders.
...
#define OCELEM_UNK3 \
	(1 << 3) // Unknown purpose. Used by Dead Hand element 0 and Dodongo element
                 // 5
...
// These flags are not to be used in code until we figure out how we want to
// format them. They are only here for reference
...
#define DMG_ARROW                                                        \
	(DMG_ARROW_NORMAL | DMG_ARROW_FIRE | DMG_ARROW_ICE | DMG_ARROW_LIGHT | \

...
menu_category_t menu_categories[] = {
	{0, "Dungeons"}, {1, "Overworld"},  {2, "Bosses"},
	{3, "Items"},    {4, "Switch Age"}, {5, "Bunny Hood"},
};

warp_t dungeon_warps[] = {
	{0, 0x00, "Deku"},           {1, 0x04, "Dodongo"},
	{2, 0x28, "Jabu"},           {3, 0x169, "Forest"},
	{4, 0x165, "Fire"},          {5, 0x10, "Water"},
	{6, 0x37, "Shadow"},         {7, 0x82, "Spirit"},
	{8, 0x98, "BotW"},           {9, 0x88, "Ice"},
	{10, 0x486, "Hideout"},      {11, 0x08, "GTG"},
	{12, 0x467, "Ganon Castle"},
};

warp_t overworld_warps[] = {
	{0, 0x443, "KF"},          {1, 0x11E, "LW"},
	{2, 0xFC, "SFM"},          {3, 0x157, "LLR"},
	{4, 0xB1, "Market"},       {5, 0x138, "HC/OGC"},
	{6, 0xDB, "Kak"},          {7, 0xE4, "GY"},
	{8, 0x13D, "DMT (Kak)"},   {9, 0x1BD, "DMT (Crater)"},
	{10, 0x14D, "Goron City"}, {11, 0xEA, "ZR"},
	{12, 0x108, "ZD"},         {13, 0x225, "ZF"},
	{14, 0x102, "LH"},         {15, 0x117, "GV"},
	{16, 0x129, "GF"},         {17, 0x130, "HW"},
	{18, 0x123, "Colossus"},
};

warp_t bosses_warps[] = {
	{0, 0x40F, "Gohma"}, {1, 0x40B, "KD"},   {2, 0x301, "Bari"},
	{3, 0x0C, "PG"},     {4, 0x305, "Volv"}, {5, 0x417, "Morph"},
	{6, 0x413, "Bongo"}, {7, 0x8D, "Twin"},  {8, 0x41F, "Ganondorf"},
	{9, 0x517, "Ganon"},
};
...
void draw_debug_menu(z64_disp_buf_t* db) {
	if (menu_not_on_dup) {
		if (z64_game.common.input[0].pad_pressed.l &&
		    z64_game.common.input[0].pad_pressed.r) {
	...
	if (show_warp_menu) {
		if (current_menu_indexes.sub_menu_index == 0) {
			...
			case 1: // Overworld
				...
				if (z64_game.common.input[0].pad_pressed.a) {
					warp_t* d =
						&(overworld_warps[current_menu_indexes.overworld_index]);
					z64_file.entrance_index = d->entrance_index;
					z64_game.entrance_index = d->entrance_index;
					z64_game.scene_load_flag = 0x14;
					z64_game.fadeout_transition = 0x02;
			...
			case 3: // Items
				...
				if (z64_game.common.input[0].pad_pressed.a) {
					...
					if ((d->item_index >= ITEM_SONG_MINUET) &&
					    (d->item_index <= ITEM_SONG_STORMS)) {
		...
		int padding = 1;
		int rows = 13;
		int bg_width = (6 * icon_size) + (13 * font_width) + (8 * padding);
		...
		if (current_menu_indexes.sub_menu_index == 0) {
			...
			int top = start_top +
			          ((icon_size + padding) * current_menu_indexes.main_index) + 1;
			text_print_size(d->name, left, top, font_width);
			text_flush_size(db, font_width, font_height, 0, 0);
		} else {
			switch (current_menu_indexes.main_index) {
			case 0: // Dungeons
				...
				int top =
					start_top +
					((icon_size + padding) * current_menu_indexes.dungeon_index) + 1;
				text_print_size(d->name, left, top, font_width);
				text_flush_size(db, font_width, font_height, 0, 0);
				break;
			...
			case 1: // Overworld
				...
				if (current_menu_indexes.overworld_index < 10) {
					...
					top =
						start_top +
						((icon_size + padding) * current_menu_indexes.overworld_index) +
						1;
					text_print_size(d->name, left, top, font_width);
					text_flush_size(db, font_width, font_height, 0, 0);
				} else {
					...
					top = start_top +
					      ((icon_size + padding) *
					       (current_menu_indexes.overworld_index - 10)) +
					      1;
					text_print_size(d->name, left, top, font_width);
					text_flush_size(db, font_width, font_height, 0, 0);
				}
			...
			case 2: // Bosses
				...
				top = start_top +
				      ((icon_size + padding) * current_menu_indexes.boss_index) + 1;
				text_print_size(d->name, left, top, font_width);
				text_flush_size(db, font_width, font_height, 0, 0);
				break;
			case 3: // Items
				...
				if (current_menu_indexes.item_index < 10) {
					...
					top = start_top +
					      ((icon_size + padding) * current_menu_indexes.item_index) + 1;
					text_print_size(dd->name, left, top, font_width);
					text_flush_size(db, font_width, font_height, 0, 0);
				}
				if (current_menu_indexes.item_index > 9 &&
				    current_menu_indexes.item_index < 20) {
					for (int i = 0; i < 10; i++) {
					...
					top = start_top +
					      ((icon_size + padding) *
					       (current_menu_indexes.item_index - 10)) +
					      1;
					text_print_size(dd->name, left, top, font_width);
					text_flush_size(db, font_width, font_height, 0, 0);
				}
				if (current_menu_indexes.item_index > 19 &&
				    current_menu_indexes.item_index < 28) {
					for (int i = 0; i < 9; i++) {
...
int draw_int_helper(z64_disp_buf_t* db, int32_t number, int16_t left,
                    int16_t top, colorRGBA8_t color) {
	int isNegative = 0;
	if (number < 0) {
		number *= -1;
		isNegative = 1;
	...
	gDPSetCombineLERP(db->p++, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, 0, 0,
	                  0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0);
...
void draw_debug_numbers(z64_disp_buf_t* db) {
	for (int i = 0; i < 10; i++) {
		...
		colorRGBA8_t color = {debug_text_color.r, debug_text_color.g,
			              debug_text_color.b, 0xFF};
		draw_int_helper(db, numberToShow, debug_text_x_placement, height + offsetY,
		                color);
	}
	...
	for (int i = 0; i < 10; i++) {
		...
		colorRGBA8_t color = {debug_text_color.r, debug_text_color.g,
			              debug_text_color.b, 0xFF};
		int numberDigit = draw_int_helper(db, entireValue, debug_text_x_placement,
		                                  height + offsetY, color);
		text_print_size(
			".", debug_text_x_placement + numberDigit * rupee_digit_sprite.tile_w,
			height + offsetY, rupee_digit_sprite.tile_w);
		text_flush_size(db, rupee_digit_sprite.tile_w, rupee_digit_sprite.tile_h, 0,
		                0);
		draw_int_helper(db, decimalValue,
		                debug_text_x_placement +
		                numberDigit * rupee_digit_sprite.tile_w +
		                font_sprite.tile_w,
		                height + offsetY, color);
	}
}

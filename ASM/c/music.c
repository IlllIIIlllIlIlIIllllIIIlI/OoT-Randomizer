...
int bgm_sequence_ids[47] = {
	0x02, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x26, 0x27, 0x28,
	0x29, 0x2A, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x38, 0x3A, 0x3C, 0x3E, 0x3F,
	0x40, 0x42, 0x4A, 0x4B, 0x4C, 0x4E, 0x4F, 0x50, 0x55, 0x56, 0x58, 0x5A,
	0x5B, 0x5C, 0x5F, 0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x6B, 0x6C,
};
...
void display_song_name(z64_disp_buf_t* db) {
	if (are_song_displayed() &&
	    !dungeon_info_is_drawn() && // Don't display if the custom rando pause
	                                // screen if displayed.
	    !(z64_link.state_flags_2 &
	      0x8000000)) { // Don't display if Link is playing the Ocarina.
		...
		if (z64_game.pause_ctxt.state == PAUSE_STATE_MAIN) {
			...
			if (frames <= SONG_NAME_FRAMES_FADE_INTO) {
				alpha = frames * 255 / SONG_NAME_FRAMES_FADE_INTO;
			           ...
			           SONG_NAME_FRAMES_FADE_INTO + SONG_NAME_FRAMES_VISIBLE) {
				alpha = 255;
			} else if (frames <= SONG_NAME_FRAMES_FADE_INTO +
			           SONG_NAME_FRAMES_VISIBLE +
			           SONG_NAME_FRAMES_FADE_AWAY) {
				alpha =
					(frames - SONG_NAME_FRAMES_FADE_INTO - SONG_NAME_FRAMES_VISIBLE) *
					255 / SONG_NAME_FRAMES_FADE_AWAY;
				alpha = 255 - alpha;
			} else {
				frames = 0;

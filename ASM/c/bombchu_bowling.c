	...
	if (!(z64_file.scene_flags[0x4B].unk_00_ & prizeFlag)) {
		...
		case 2:
			...
			if (!EXTRA_BOWLING_SHUFFLE &&
			    z64_file.items[Z64_SLOT_BOMBCHU] == ITEM_NONE &&
			    FREE_BOMBCHU_DROPS) {
				prizeTemp = z64_file.bomb_bag ? EXITEM_BOMBS_BOWLING
				        : EXITEM_PURPLE_RUPEE_BOWLING;
			} else {
		...
		if (!EXTRA_BOWLING_SHUFFLE) {
			switch (prizeSelect) {
			case 2:
				if (z64_file.items[Z64_SLOT_BOMBCHU] == ITEM_NONE &&
				    FREE_BOMBCHU_DROPS) {
					prizeTemp = z64_file.bomb_bag ? EXITEM_BOMBS_BOWLING
					  : EXITEM_PURPLE_RUPEE_BOWLING;
				} else {

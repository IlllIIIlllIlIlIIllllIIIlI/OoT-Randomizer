...
void draw_pot(z64_actor_t* actor, z64_game_t* game) {
	...
	if ((actor->actor_id == 0x111 && (actor->variable >> 8) & 1) ||
	    actor->actor_id == 0x117) {
		dlist = POT_DLIST;
	...
	gMoveWd(gfx->poly_opa.p++, G_MW_SEGMENT, 0x0A * sizeof(int),
	        gfx->poly_opa.d + 2);

	// draw the original dlist that has been hacked in ASM to jump to the custom
	// dlist
...
void ObjTsubo_SpawnCollectible_Hack(z64_actor_t* this, z64_game_t* game) {
	...
	if (flag->all && !Get_NewFlag(flag)) {
		...
		EnItem00* spawned = z64_Item_DropCollectible(
			game, &this->pos_world, ((((this->variable >> 9) & 0x3F) << 8)));
	...
	if ((dropParams >= ITEM00_RUPEE_GREEN) &&
	    (dropParams <= ITEM00_BOMBS_SPECIAL)) {
		EnItem00* spawned = z64_Item_DropCollectible(
			game, &this->pos_world,
			(dropParams | (((this->variable >> 9) & 0x3F) << 8)));
	}
}
...
void EnTuboTrap_DropCollectible_Hack(z64_actor_t* this, z64_game_t* game) {
	...
	if (param3FF >= 0 && param3FF < 0x1A) {
		EnItem00* spawned = z64_Item_DropCollectible(
			game, &this->pos_world, param3FF | ((params & 0x3F) << 8));
	}
}

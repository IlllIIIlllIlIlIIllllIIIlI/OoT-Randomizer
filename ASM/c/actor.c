...
// Get a pointer to the additional data that is stored at the beginning of every
// actor This is calculated as the actor's address + the actor instance size
// from the overlay table.
...
void Actor_BuildFlag(z64_actor_t* actor, xflag_t* flag, uint16_t actor_index,
                     uint8_t subflag) {
...
// Called from Actor_UpdateAll when spawning the actors in the scene's/room's
// actor list to store flags in the new space that we added to the actors. Prior
// to being called, CURR_ACTOR_SPAWN_INDEX is set to the current position in the
// actor spawn list.
void Actor_After_UpdateAll_Hack(z64_actor_t* actor, z64_game_t* game) {
	...
	// Add additional actor hacks here. These get called shortly after the call to
	// actor_init Hacks are responsible for checking that they are the correct
	// actor.
	EnWonderitem_AfterInitHack(actor, game);
	CURR_ACTOR_SPAWN_INDEX = 0; // reset CURR_ACTOR_SPAWN_INDEX
}

// For pots/crates/beehives, store the flag in the new space in the actor
// instance. Flag consists of the room #, scene setup, and the actor index
void Actor_StoreFlag(z64_actor_t* actor, z64_game_t* game, xflag_t flag) {
	...
	// For the following actors we store the flag in the new space added to the
	// actor.
	case OBJ_TSUBO:
	case EN_TUBO_TRAP:
	case OBJ_KIBAKO:
	...
	case OBJ_MURE3:
	case BG_HAKA_TUBO: {
		// For these actors which use subflags, always store the flag which serves
		// as the base when calculating the subflag. Don't really need to do this
		// since we will recalculate the flag and check the override when spawning
		// the subflags.
		extra->flag = flag;
	}
	default: {
...
// For pots/crates/beehives, store the flag in the new space in the actor
// instance. Flag consists of the room #, scene setup, and the actor index
void Actor_StoreFlagByIndex(z64_actor_t* actor, z64_game_t* game,
...
// Get an override for new flag. If the override doesn't exist, or flag has
// already been set, return 0.
...
// For pots/crates/beehives match contents, determine the override and store the
// chest type in new space in the actor instance So we don't have to hit the
// override table every frame.
void Actor_StoreChestType(z64_actor_t* actor, z64_game_t* game) {
	...
	if (override.key.all != 0 &&
	    pChestType != NULL) { // If we don't have an override key, then either
		                  // this item doesn't have an override entry, or it
		                  // has already been collected.
		if (POTCRATE_TEXTURES_MATCH_CONTENTS == PTMC_UNCHECKED &&
		    override.key.all >
		    0) { // For "unchecked" PTMC setting: Check if we have an override
			 // which means it wasn't collected.
...
z64_actor_t* Actor_SpawnEntry_Hack(void* actorCtx, ActorEntry* actorEntry,
                                   z64_game_t* globalCtx) {
	...
	case EN_G_SWITCH: {
		continue_spawn =
			spawn_override_silver_rupee(actorEntry, globalCtx, &overridden);
	...
	if (continue_spawn) {
		spawned = z64_SpawnActor(
			actorCtx, globalCtx, actorEntry->id, actorEntry->pos.x,
			actorEntry->pos.y, actorEntry->pos.z, actorEntry->rot.x,
			actorEntry->rot.y, actorEntry->rot.z, actorEntry->params);
...
bool spawn_override_silver_rupee(ActorEntry* actorEntry, z64_game_t* globalCtx,
	...
	*overridden = false;
	if (SHUFFLE_SILVER_RUPEES) { // Check if silver rupee shuffle is enabled.
		...
		if (type !=
		    1) { // only override actual silver rupees, not the switches or pots.
			return true;
		}
		override_t override = lookup_override_by_newflag(&flag);
		if (override.key.all != 0) {
			if (type == 1 && !Get_NewFlag(&flag)) {
				// Spawn a green rupee which will be overridden using the collectible
				// hacks.
				actorEntry->params = 0;
...
void after_spawn_override_silver_rupee(z64_actor_t* spawned, bool overridden) {
	if (overridden) {
		...
		this->collider.info.bumper.dmgFlags =
			0; // Remove clear the bumper collider flags so it doesn't interact w/
		           // boomerang
	}
}

z64_actor_t* Player_SpawnEntry_Hack(void* actorCtx, ActorEntry* playerEntry,
                                    z64_game_t* globalCtx) {
	...
	return z64_SpawnActor(
		actorCtx, globalCtx, playerEntry->id, playerEntry->pos.x,
		playerEntry->pos.y, playerEntry->pos.z, playerEntry->rot.x,
		playerEntry->rot.y, playerEntry->rot.z, playerEntry->params);
}

// This is our entrypoint back into Actor_Spawn. Call/return this to spawn the
// actor
extern z64_actor_t* Actor_Spawn_Continue(void* actorCtx, z64_game_t* globalCtx,
                                         int16_t actorId, float posX,
                                         float posY, float posZ, int16_t rotX,
                                         int16_t rotY, int16_t rotZ,
                                         int16_t params);

z64_actor_t* Actor_Spawn_Hook(void* actorCtx, z64_game_t* globalCtx,
                              int16_t actorId, float posX, float posY,
                              float posZ, int16_t rotX, int16_t rotY,
                              int16_t rotZ, int16_t params) {
	bool continue_spawn = true;
	...
	if (continue_spawn) {
		z64_actor_t* spawned =
			Actor_Spawn_Continue(actorCtx, globalCtx, actorId, posX, posY, posZ,

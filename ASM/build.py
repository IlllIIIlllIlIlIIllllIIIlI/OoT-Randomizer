...
parser.add_argument(
    "--compile-c", action="store_true", help="Recompile C modules. This is the default"
)
parser.add_argument("--no-compile-c", action="store_true", help="Do not recompile C modules")
parser.add_argument(
    "--dump-obj",
    action="store_true",
    help="Dumps extra object info for debugging purposes. Does nothing with --no-compile-c",
)
parser.add_argument(
    "--diff-only", action="store_true", help="Creates diff output without running armips"
)
...
if base_rom_size != 0x400_0000:
    sys.exit(
        f"build.py: roms/base.z64 should be 0x4000000 bytes (64 MiB), but yours is 0x{base_rom_size:x} bytes ({base_rom_size / (1024 ** 2)} MiB). Make sure you have an uncompressed base ROM (see ../bin/Decompress)."
    )

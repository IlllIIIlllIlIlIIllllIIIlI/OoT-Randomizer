...
def dump_list(obj: list, current_indent: str = "", ensure_ascii: bool = False) -> str:
    ...
    output = output_format.format(
        ...
        values=join_format.join(
            [values_format.format(value=entry, indent=current_indent + INDENT) for entry in entries]
        ),
    )
...
def dump_dict(
    obj: dict,
    current_indent: str = "",
    sub_width: Optional[Sequence[int, int]] = None,
    ...
    for key, value in obj.items():
        entries.append(
            (
                dump_scalar(str(key), ensure_ascii),
                dump_obj(value, current_indent + INDENT, sub_width, ensure_ascii),
            )
        )

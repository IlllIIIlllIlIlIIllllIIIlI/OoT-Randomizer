...
class World:
    def __init__(
        self, world_id: int, settings: Settings, resolve_randomized_settings: bool = True
    ) -> None:
        ...
        self.keysanity: bool = settings.shuffle_smallkeys in (
            "keysanity",
            "remove",
            "any_dungeon",
            "overworld",
        ...
        self.shuffle_interior_entrances: bool = settings.shuffle_interior_entrances in (
            "simple",
            "all",
        )

        self.shuffle_special_dungeon_entrances: bool = settings.shuffle_dungeon_entrances == "all"
        self.shuffle_dungeon_entrances: bool = settings.shuffle_dungeon_entrances in (
            "simple",
            "all",
        )

        self.entrance_shuffle: bool = bool(
            self.shuffle_interior_entrances
            or settings.shuffle_grotto_entrances
            or self.shuffle_dungeon_entrances
            or settings.shuffle_overworld_entrances
            or settings.shuffle_gerudo_valley_river_exit
            or settings.owl_drops
            or settings.warp_songs
            or settings.spawn_positions
            or (settings.shuffle_bosses != "off")
        )

        self.mixed_pools_bosses = False  # this setting is still in active development at https://github.com/Roman971/OoT-Randomizer

        self.ensure_tod_access: bool = bool(
            self.shuffle_interior_entrances
            or settings.shuffle_overworld_entrances
            or settings.spawn_positions
        )
        self.disable_trade_revert: bool = (
            self.shuffle_interior_entrances
            or settings.shuffle_overworld_entrances
            or settings.adult_trade_shuffle
        )
        self.skip_child_zelda: bool = (
            "Zeldas Letter" not in settings.shuffle_child_trade
            and "Zeldas Letter" in self.distribution.starting_items
        )
        ...
        if not settings.adult_trade_shuffle and settings.adult_trade_start:
            ...
            plando_adult_trade = list(
                filter(
                    lambda location_record_pair: adult_trade_matcher(location_record_pair[1].item),
                    self.distribution.pattern_dict_items(self.distribution.locations),
                )
            )
        ...
        if settings.open_forest == "closed" and (
            self.shuffle_special_interior_entrances
            or settings.shuffle_hideout_entrances
            or settings.shuffle_overworld_entrances
            or settings.warp_songs
            or settings.spawn_positions
        ):
        ...
        self.song_notes: dict[str, Song] = generate_song_list(
            self,
        ...
        for hint_type in hint_dist_keys:
            if (
                ...
                and hint_type not in self.hint_dist_user["distribution"]
            ):
                self.hint_dist_user["distribution"][hint_type] = {
                    "order": 0,
                    "weight": 0.0,
                    "fixed": 0,
                    "copies": 0,
        ...
        if all(key in self.hint_dist_user["distribution"] for key in hint_dist_keys):
            ...
            for key in self.hint_dist_user["distribution"]:
                if not all(
                    sub_key in sub_keys for sub_key in self.hint_dist_user["distribution"][key]
                ):
                    hint_dist_valid = False
        ...
        self.item_hints: list[str] = (
            self.settings.item_hints + self.item_added_hint_types["named-item"]
        )
        ...
        self.dungeon_rewards_hinted: bool = (
            settings.shuffle_mapcompass != "remove"
            if settings.enhance_map_compass
            else "altar" in settings.misc_hints
        )
        self.misc_hint_items: dict[str, str] = {
            hint_type: self.hint_dist_user.get("misc_hint_items", {}).get(
                hint_type, data["default_item"]
            )
            for hint_type, data in misc_item_hint_table.items()
        }
        self.misc_hint_locations: dict[str, str] = {
            hint_type: self.hint_dist_user.get("misc_hint_locations", {}).get(
                hint_type, data["item_location"]
            )
        ...
        self.max_progressions: dict[str, int] = {
            name: item.special.get("progressive", 1) for name, item in ItemInfo.items.items()
        }
        max_tokens = 0
        ...
        if (
            "distribution" in self.hint_dist_user
            and "goal" in self.hint_dist_user["distribution"]
            and (
                self.hint_dist_user["distribution"]["goal"]["fixed"] != 0
                or self.hint_dist_user["distribution"]["goal"]["weight"] != 0
            )
        ):
            ...
            for category in self.hint_dist_user["custom_goals"]:
                if category["category"] in self.goal_categories:
                    ...
                    cat = GoalCategory(
                        category["category"],
                        category["priority"],
                        minimum_goals=category["minimum_goals"],
                    )
        ...
        self.goal_categories = OrderedDict(
            {
                ...
                for (name, category) in sorted(
                    self.goal_categories.items(), key=lambda kv: kv[1].priority
                )
            }
        )
        ...
        if len(self.goal_categories) > 0:
            ...
            for category in self.goal_categories.values():
                if (
                    goal_list1 != [goal.name for goal in category.goals]
                    and category.name not in minor_goal_categories
                ):
        ...
        self.locked_goal_categories: dict[str, GoalCategory] = {
            name: category
            for (name, category) in self.goal_categories.items()
            if category.lock_entrances
        }
        self.unlocked_goal_categories: dict[str, GoalCategory] = {
            name: category
            for (name, category) in self.goal_categories.items()
            if not category.lock_entrances
        }
    ...
    def resolve_random_settings(self) -> None:
        ...
        if self.settings.randomize_settings:
            ...
            for setting in list(self.randomized_list):
                if (
                    (setting == "bridge_medallions" and self.settings.bridge != "medallions")
                    ...
                    or (
                        setting == "lacs_medallions"
                        and self.settings.lacs_condition != "medallions"
                    )
        ...
        if self.settings.dungeon_shortcuts_choice == "random":
            self.settings.dungeon_shortcuts = random.sample(
                dungeons, random.randint(0, len(dungeons))
            )
        ...
        if self.settings.bridge == "random" and (
            "bridge" not in dist_keys
        ...
        if num_trials < dist_num_chosen:
            raise RuntimeError(
                "%d trials set to active on world %d, but only %d active trials allowed."
                % (dist_num_chosen, self.id, num_trials)
            )
        ...
        dist_num_mq, dist_num_empty = self.distribution.configure_dungeons(
            self, mq_dungeon_pool, empty_dungeon_pool
        )
        ...
        if self.settings.empty_dungeons_mode == "count":
            ...
            if nb_to_pick < 0:
                raise RuntimeError(
                    f"{dist_num_empty} dungeons are set to empty on world {self.id+1}, but only {self.settings.empty_dungeons_count} empty dungeons allowed"
                )
            if len(empty_dungeon_pool) < nb_to_pick:
                non_empty = 8 - dist_num_empty - len(empty_dungeon_pool)
                raise RuntimeError(
                    f"On world {self.id+1}, {dist_num_empty} dungeons are set to empty and {non_empty} to non-empty. Can't reach {self.settings.empty_dungeons_count} empty dungeons."
                )
        ...
        if self.settings.mq_dungeons_mode == "random" and "mq_dungeons_count" not in dist_keys:
            ...
            if nb_to_pick < 0:
                raise RuntimeError(
                    "%d dungeons are set to MQ on world %d, but only %d MQ dungeons allowed."
                    % (dist_num_mq, self.id + 1, self.settings.mq_dungeons_count)
                )
            if len(mq_dungeon_pool) < nb_to_pick:
                non_mq = 8 - dist_num_mq - len(mq_dungeon_pool)
                raise RuntimeError(
                    f"On world {self.id+1}, {dist_num_mq} dungeons are set to MQ and {non_mq} to non-MQ. Can't reach {self.settings.mq_dungeons_count} MQ dungeons."
                )
        ...
        if self.settings.silver_rupee_pouches_choice == "random":
            ...
            self.settings.silver_rupee_pouches = random.sample(
                puzzles, random.randint(0, len(puzzles))
            )
                ...
                for event, rule in region["events"].items():
                    ...
                    if new_location.never:
                        logging.getLogger("").debug(
                            "Dropping unreachable event: %s", new_location.name
                        )
                    else:
                        new_location.world = self
                        new_region.locations.append(new_location)
    ...
    def create_dungeons(self) -> list[tuple[Entrance, str]]:
        ...
        for hint_area in HintArea:
            if (name := hint_area.dungeon_name) is not None:
                logic_folder = (
                    "Glitched World" if self.settings.logic_rules == "glitched" else "World"
                )
                file_name = name + (" MQ.json" if self.dungeon_mq[name] else ".json")
                savewarps_to_connect += self.load_regions_from_json(
                    os.path.join(data_path(logic_folder), file_name)
                )
    ...
    def create_internal_locations(self) -> None:
        ...
        assert self.parser.events <= self.event_items, "Parse error: undefined items %r" % (
            self.parser.events - self.event_items
        )
    ...
    def initialize_items(self, items: Optional[list[Item]] = None) -> None:
        ...
        for item in items:
            ...
            if (
                self.settings.shuffle_hideoutkeys in ("fortress", "regional")
                and item.type == "HideoutSmallKey"
            ) or (self.settings.shuffle_tcgkeys == "regional" and item.type == "TCGSmallKey"):
                item.priority = True

        for dungeon in self.dungeons:
            dungeon_items = [
                item for item_name in dungeon.get_item_names() for item in item_dict[item_name]
            ]
                ...
                if location.type == "Shop":
                    if location.name[-1:] in shop_item_indexes[:shop_item_count]:
                        if self.settings.shopsanity_prices == "random":
                            self.shop_prices[location.name] = (
                                int(random.betavariate(1.5, 2) * 60) * 5
                            )
    ...
    def set_scrub_prices(self) -> None:
        ...
        scrub_locations = [
            ...
            if location.type in ("Scrub", "GrottoScrub")
        ]
        scrub_dictionary = {}
    ...
    def set_empty_dungeon_rewards(self, empty_rewards: list[str] = []) -> None:
        empty_dungeon_bosses = list(
            map(lambda reward: self.find_items(reward)[0].name, empty_rewards)
        )
    ...
    def set_goals(self) -> None:
        ...
        dot = GoalCategory(
            "door_of_time",
            5,
            lock_entrances=["Temple of Time -> Beyond Door of Time"],
            minimum_goals=1,
        )
        b = GoalCategory(
            "rainbow_bridge", 10, lock_entrances=["Ganons Castle Grounds -> Ganons Castle Lobby"]
        )
        ...
        th = GoalCategory(
            "triforce_hunt",
            30,
            goal_count=round(self.settings.triforce_goal_per_world / 10),
            minimum_goals=1,
        )
        ganon = GoalCategory("ganon", 40, goal_count=1)
        trial_goal = Goal(
            self, "the Tower", "path to #the Tower#", "White", items=[], create_empty=True
        )
            ...
            th.add_goal(
                Goal(
                    ...
                    items=[
                        {
                            "name": "Triforce Piece",
                            "quantity": self.settings.triforce_count_per_world,
                            "minimum": self.settings.triforce_goal_per_world,
                            "hintable": False,
                        }
                    ],
                )
        ...
        if not self.settings.triforce_hunt:
            if self.settings.starting_age == "child":
                dot_items = [
                    {"name": "Temple of Time Access", "quantity": 1, "minimum": 1, "hintable": True}
                ]
                    ...
                    dot_items.append(
                        {
                            ...
                            "quantity": (
                                ...
                                and self.settings.item_pool_value == "plentiful"
                                else 1
                            ),
                            "minimum": 1,
                            "hintable": True,
                        ...
                        dot_items.append(
                            {
                                "name": "Ocarina",
                                "quantity": (
                                    3 if self.settings.item_pool_value == "plentiful" else 2
                                ),
                                "minimum": 1,
                                "hintable": True,
                            }
                ...
                dot.add_goal(
                    Goal(self, "Door of Time", "path of #time#", "Light Blue", items=dot_items)
                )
            ...
            if self.settings.bridge != "open" and not self.shuffle_special_dungeon_entrances:
                ...
                if (self.settings.bridge_stones > 0 and self.settings.bridge == "stones") or (
                    self.settings.bridge_rewards > 0 and self.settings.bridge == "dungeons"
                ):
                    b.add_goal(
                        Goal(
                            self,
                            "Kokiri Emerald",
                            {"replace": "Kokiri Emerald"},
                            "Green",
                            items=[
                                {
                                    "name": "Kokiri Emerald",
                    ...
                    b.minimum_goals = (
                        ...
                        if self.settings.bridge == "stones"
                        else self.settings.bridge_rewards
                    )
                if (
                    self.settings.bridge_medallions > 0 and self.settings.bridge == "medallions"
                ) or (self.settings.bridge_rewards > 0 and self.settings.bridge == "dungeons"):
                    b.add_goal(
                        Goal(
                            self,
                            "Forest Medallion",
                            {"replace": "Forest Medallion"},
                            "Green",
                            items=[
                                {
                    ...
                    b.minimum_goals = (
                        self.settings.bridge_medallions
                        if self.settings.bridge == "medallions"
                        else self.settings.bridge_rewards
                    )
                ...
                if (
                    self.settings.bridge_tokens > 0
                    and self.settings.bridge == "tokens"
                    and (
                        self.settings.shuffle_ganon_bosskey != "tokens"
                        or self.settings.bridge_tokens >= self.settings.ganon_bosskey_tokens
                    )
                    and (
                        self.settings.shuffle_ganon_bosskey != "on_lacs"
                            ...
                            items=[
                                {
                                    ...
                                    "quantity": 100,
                                    "minimum": self.settings.bridge_tokens,
                                    "hintable": False,
                                }
                            ],
                        )
                    ...
                    b.minimum_goals = 1
                if (
                    self.settings.bridge_hearts > self.settings.starting_hearts
                    and self.settings.bridge == "hearts"
                    and (
                        self.settings.shuffle_ganon_bosskey != "hearts"
                        or self.settings.bridge_hearts >= self.settings.ganon_bosskey_hearts
                    )
                    and (
                        self.settings.shuffle_ganon_bosskey != "on_lacs"
                        or self.settings.lacs_condition != "hearts"
                    ...
                    b.add_goal(
                        Goal(
                            ...
                            items=[
                                {
                                    "name": "Piece of Heart",
                                    "quantity": (20 - self.settings.starting_hearts) * 4,
                                    "minimum": (
                                        self.settings.bridge_hearts - self.settings.starting_hearts
                                    )
                                    * 4,
            ...
            if (
                (
                    ...
                    and self.settings.shuffle_ganon_bosskey == "stones"
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.ganon_bosskey_stones > self.settings.bridge_stones
                        or self.settings.bridge != "stones"
                    )
                )
                or (
                    self.settings.lacs_stones > 0
                    and self.settings.shuffle_ganon_bosskey == "on_lacs"
                    and self.settings.lacs_condition == "stones"
                    and (
                        self.shuffle_special_dungeon_entrances
                ...
                or (
                    self.settings.ganon_bosskey_rewards > 0
                    and self.settings.shuffle_ganon_bosskey == "dungeons"
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.ganon_bosskey_rewards > self.settings.bridge_medallions
                        or self.settings.bridge != "medallions"
                    )
                    and (
                        self.shuffle_special_dungeon_entrances
                    ...
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.ganon_bosskey_rewards > self.settings.bridge_rewards
                        or self.settings.bridge != "dungeons"
                    )
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.ganon_bosskey_rewards > 2
                        or self.settings.bridge != "vanilla"
                    )
                )
                or (
                    self.settings.lacs_rewards > 0
                    and self.settings.shuffle_ganon_bosskey == "on_lacs"
                    and self.settings.lacs_condition == "dungeons"
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.lacs_rewards > self.settings.bridge_medallions
                        or self.settings.bridge != "medallions"
                    )
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.lacs_rewards > self.settings.bridge_stones
                        or self.settings.bridge != "stones"
                    )
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.lacs_rewards > self.settings.bridge_rewards
                        or self.settings.bridge != "dungeons"
                    )
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.lacs_rewards > 2
                        or self.settings.bridge != "vanilla"
                    )
                )
            ):
                ...
                gbk.add_goal(
                    Goal(
                        ...
                        "Red",
                        items=[
                            {"name": "Goron Ruby", "quantity": 1, "minimum": 1, "hintable": False}
                        ],
                    )
                )
                gbk.add_goal(
                ...
                gbk.minimum_goals = (
                    self.settings.ganon_bosskey_stones
                    if self.settings.shuffle_ganon_bosskey == "stones"
                    else (
                        self.settings.lacs_stones
                        if self.settings.shuffle_ganon_bosskey == "on_lacs"
                        and self.settings.lacs_condition == "stones"
                        else (
                            self.settings.ganon_bosskey_rewards
            ...
            if (
                (
                    ...
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.ganon_bosskey_medallions > self.settings.bridge_medallions
                        or self.settings.bridge != "medallions"
                    )
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.ganon_bosskey_medallions > 2
                        or self.settings.bridge != "vanilla"
                    )
                )
                or (
                    self.settings.lacs_medallions > 0
                    and self.settings.shuffle_ganon_bosskey == "on_lacs"
                    and self.settings.lacs_condition == "medallions"
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.lacs_medallions > self.settings.bridge_medallions
                        or self.settings.bridge != "medallions"
                    )
                    and (
                        self.shuffle_special_dungeon_entrances
                        or self.settings.lacs_medallions > 2
                        or self.settings.bridge != "vanilla"
                    )
                )
                or (
                ...
                gbk.minimum_goals = (
                    self.settings.ganon_bosskey_medallions
                    if self.settings.shuffle_ganon_bosskey == "medallions"
                    else (
                        self.settings.lacs_medallions
                        if self.settings.shuffle_ganon_bosskey == "on_lacs"
                        and self.settings.lacs_condition == "medallions"
                        else (
                            self.settings.ganon_bosskey_rewards
            ...
            if (
                self.settings.shuffle_ganon_bosskey == "on_lacs"
                and self.settings.lacs_condition == "vanilla"
            ):
                gbk.add_goal(
                    Goal(
                        self,
            ...
            if (
                self.settings.ganon_bosskey_tokens > 0
                and self.settings.shuffle_ganon_bosskey == "tokens"
                and (
                    self.shuffle_special_dungeon_entrances
                        ...
                        items=[
                            {
                                "name": "Gold Skulltula Token",
                                "quantity": 100,
                                "minimum": self.settings.ganon_bosskey_tokens,
                                "hintable": False,
                            }
                        ],
                    )
            ...
            if (
                        ...
                        items=[
                            {
                                "name": "Gold Skulltula Token",
                                "quantity": 100,
                                "minimum": self.settings.lacs_tokens,
                                "hintable": False,
                            }
                        ],
                    )
                ...
                gbk.minimum_goals = 1
            if (
                self.settings.ganon_bosskey_hearts > self.settings.starting_hearts
                and self.settings.shuffle_ganon_bosskey == "hearts"
                and (
                    self.shuffle_special_dungeon_entrances
                        ...
                        items=[
                            {
                                ...
                                "quantity": (20 - self.settings.starting_hearts) * 4,
                                "minimum": (
                                    self.settings.ganon_bosskey_hearts
                                    - self.settings.starting_hearts
                                )
                                * 4,
                                "hintable": False,
                            }
                ...
                gbk.minimum_goals = 1
            if (
                self.settings.lacs_hearts > self.settings.starting_hearts
                and self.settings.shuffle_ganon_bosskey == "on_lacs"
                and self.settings.lacs_condition == "hearts"
                and (
                    self.shuffle_special_dungeon_entrances
                    or self.settings.bridge != "hearts"
                    or self.settings.bridge_hearts < self.settings.lacs_hearts
            ...
            if self.settings.shuffle_ganon_bosskey in (
                "dungeon",
                "overworld",
                "any_dungeon",
                "keysanity",
            ...
            if not self.skipped_trials["Forest"]:
                trial_goal.items.append(
                    {"name": "Forest Trial Clear", "quantity": 1, "minimum": 1, "hintable": True}
                )
                trials.goal_count += 1
            if not self.skipped_trials["Fire"]:
                trial_goal.items.append(
                    {"name": "Fire Trial Clear", "quantity": 1, "minimum": 1, "hintable": True}
                )
                trials.goal_count += 1
            if not self.skipped_trials["Water"]:
                trial_goal.items.append(
                    {"name": "Water Trial Clear", "quantity": 1, "minimum": 1, "hintable": True}
                )
                trials.goal_count += 1
            if not self.skipped_trials["Shadow"]:
                trial_goal.items.append(
                    {"name": "Shadow Trial Clear", "quantity": 1, "minimum": 1, "hintable": True}
                )
                trials.goal_count += 1
            if not self.skipped_trials["Spirit"]:
                trial_goal.items.append(
                    {"name": "Spirit Trial Clear", "quantity": 1, "minimum": 1, "hintable": True}
                )
                trials.goal_count += 1
            if not self.skipped_trials["Light"]:
                trial_goal.items.append(
                    {"name": "Light Trial Clear", "quantity": 1, "minimum": 1, "hintable": True}
                )
                trials.goal_count += 1
            ...
            ganon.add_goal(
                Goal(
                    ...
                    "path of #the hero#",
                    "White",
                    items=[{"name": "Triforce", "quantity": 1, "minimum": 1, "hintable": True}],
                )
            )
    ...
    def silver_rupee_puzzles(self) -> list[str]:
        return (
            ...
            + (
                []
                if self.dungeon_mq["Ice Cavern"]
                else ["Ice Cavern Spinning Scythe", "Ice Cavern Push Block"]
            )
            ...
            + (
                ["Spirit Temple Lobby and Lower Adult", "Spirit Temple Adult Climb"]
                if self.dungeon_mq["Spirit Temple"]
                else [
            ...
            + (
                ["Ganons Castle Shadow Trial", "Ganons Castle Water Trial"]
                if self.dungeon_mq["Ganons Castle"]
                else [
        ...
        if self.settings.logic_rules == "glitched":
            ...
            while jabu_reward_regions:
                locations = [
                    ...
                    and (
                        loc.type != "Shop" or loc.name in self.shop_prices
                    )  # ignore regular shop items (but keep special deals)
                ]
                if locations:
                    ...
                    best_type = max(
                        (location.type for location in locations),
                        key=lambda type: (
                            priority_types.index(type) if type in priority_types else -1
                        ),
                    )
                    location = random.choice(
                        list(filter(lambda loc: loc.type == best_type, locations))
                    )
                    break
                ...
                jabu_reward_regions = {
                    ...
                    and exit.connected_region.dungeon != "Jabu Jabus Belly"
    ...
    def get_shuffled_entrances(self, type=None, only_primary=False) -> list[Entrance]:
        return [
            entrance
            for entrance in self.get_shufflable_entrances(type=type, only_primary=only_primary)
    ...
    def keyring(self, dungeon_name: str) -> bool:
        if dungeon_name in (
            ...
            return (
                dungeon_name in self.settings.key_rings
                and self.settings.shuffle_smallkeys != "vanilla"
            )
        elif dungeon_name == "Thieves Hideout":
            return (
                dungeon_name in self.settings.key_rings
                and self.settings.gerudo_fortress != "fast"
                and self.settings.shuffle_hideoutkeys != "vanilla"
            )
        elif dungeon_name == "Treasure Chest Game":
            return (
                dungeon_name in self.settings.key_rings
                and self.settings.shuffle_tcgkeys != "vanilla"
            )
        else:
    ...
    def keyring_give_bk(self, dungeon_name: str) -> bool:
        return (
            dungeon_name
            in ("Forest Temple", "Fire Temple", "Water Temple", "Shadow Temple", "Spirit Temple")
    ...
    def update_useless_areas(self, spoiler: Spoiler) -> None:
        ...
        for location in self.get_locations():
            ...
            if (
                ...
                or (
                    location.item.type == "DungeonReward"
                    and location.item.world.settings.shuffle_dungeon_rewards
                    in ("vanilla", "reward", "dungeon")
                )
            ):
                continue
        ...
        if (
            self.settings.damage_multiplier != "ohko"
            and self.settings.damage_multiplier != "quadruple"
            and self.settings.shuffle_scrubs == "off"
            and not self.settings.shuffle_grotto_entrances
        ):
            # nayru's love may be required to prevent forced damage
            exclude_item_list.append("Nayrus Love")
        if (
            "logic_grottos_without_agony" in self.settings.allowed_tricks
            or self.settings.logic_rules != "glitchless"
        ) and self.settings.hints != "agony":
            # Stone of Agony skippable if not used for hints or grottos
            exclude_item_list.append("Stone of Agony")
        if (
            not self.shuffle_special_interior_entrances
            and not self.settings.shuffle_overworld_entrances
            and not self.settings.warp_songs
        ):
        ...
        if "logic_lens_botw" in self.settings.allowed_tricks or self.settings.shuffle_pots in (
            "off",
            "overworld",
        ):
        ...
        if (
            "logic_spirit_sun_chest_no_rupees" in self.settings.allowed_tricks
            and "logic_spirit_sun_chest_bow" not in self.settings.allowed_tricks
        ):
        ...
        if not any(
            setting in ("medallions", "dungeons")
            for setting in (
        ...
        woth_loc = [
            location
            for world_woth in spoiler.required_locations.values()
        ...
        for area, area_info in areas.items():
            ...
            for location in area_info["locations"]:
                ...
                if (
                    (not location.item.majoritem) or (location.item.name in exclude_item_list)
                ) and (location.item.name not in self.item_hint_type_overrides["barren"]):
                ...
                if item.name == "Rutos Letter" and item.name in duplicate_item_woth[world_id]:
                    ...
                    for dupe_location in dupe_locations:
                        if (
                            dupe_location.world.id == location.world.id
                            and dupe_location.name == location.name
                        ):
                            useless_area = False
                            break

...
class SettingInfos:
    ...
    sfx_link_unavailable_msg = Textbox(
        ...
        gui_params={"hide_when_disabled": True},
    )
    ...
    rom = Fileinput(
        ...
        gui_params={
            "file_types": [
                {"name": "ROM Files", "extensions": ["z64", "n64"]},
                {"name": "All Files", "extensions": ["*"]},
            ],
    ...
    patch_file = Fileinput(
        ...
        gui_params={
            "file_types": [
                {"name": "Patch File Archive", "extensions": ["zpfz", "zpf", "patch"]},
                {"name": "All Files", "extensions": ["*"]},
            ],
        },
    )
    ...
    shuffle_smallkeys = Combobox(
        ...
        """,
        disable={"any_dungeon": {"settings": ["one_item_per_dungeon"]}},
        shared=True,
        gui_params={
            "randomize_key": "randomize_settings",
        },
    ...
    open_forest = Combobox(
        ...
        """,
        shared=True,
        disable={"closed": {"settings": ["starting_age"]}},
        gui_params={
            "randomize_key": "randomize_settings",
            "distribution": [
                ("open", 1),
    ...
    gerudo_fortress = Combobox(
        ...
        """,
        shared=True,
        disable={"open": {"settings": ["shuffle_hideoutkeys"]}},
        gui_params={
            "randomize_key": "randomize_settings",
        },
    )
    ...
    disabled_locations = SearchBox(
        ...
        shared=True,
        choices=[
            location.name for location in LocationIterator(lambda loc: loc.filter_tags is not None)
        ],
        default=[],
        gui_tooltip="""
        ...
        gui_params={
            "filterdata": {
                location.name: location.filter_tags
                for location in LocationIterator(lambda loc: loc.filter_tags is not None)
            },
        },
    )
    ...
    allowed_tricks = SearchBox(
        ...
        shared=True,
        choices={val["name"]: gui_text for gui_text, val in logic_tricks.items()},
        default=[],
        gui_params={
            "choice_tooltip": {
                choice["name"]: choice["tooltip"] for choice in logic_tricks.values()
            },
    ...
    starting_equipment = SearchBox(
        ...
        shared=True,
        choices={key: value.gui_text for key, value in StartingItems.equipment.items()},
        default=[],
        gui_tooltip="""\
    ...
    starting_songs = SearchBox(
        ...
        shared=True,
        choices={key: value.gui_text for key, value in StartingItems.songs.items()},
        default=[],
        gui_tooltip="""\
    ...
    starting_inventory = SearchBox(
        ...
        shared=True,
        choices={key: value.gui_text for key, value in StartingItems.inventory.items()},
        default=[],
        gui_tooltip="""\
    ...
    correct_potcrate_appearances = Combobox(
        ...
        disable={
            "!textures_content": {
                "settings": ["potcrate_textures_specific", "soa_unlocks_potcrate_texture"]
            },
        },
    )
    ...
    item_hints = SettingInfoList(
        ...
        choices=[
            ...
            if item.type not in ("Drop", "Event", "Refill", "Shop")
        ],
    )
    ...
    item_pool_value = Combobox(
        ...
        """,
        shared=True,
        disable={"ludicrous": {"settings": ["one_item_per_dungeon"]}},
    )
    ...
    randomize_all_cosmetics = Checkbutton(
        ...
        disable={
            True: {
                "sections": ["equipment_color_section", "ui_color_section", "misc_color_section"]
            },
        },
    )
    ...
    model_adult_filepicker = Fileinput(
        ...
        gui_params={
            "file_types": [
                {"name": "Z64 Model Files", "extensions": ["zobj"]},
                {"name": "All Files", "extensions": ["*"]},
            ],
            "hide_when_disabled": True,
        },
    ...
    sword_trail_duration = ComboboxInt(
        ...
        gui_params={
            "randomize_key": "randomize_all_cosmetics",
            "distribution": [(4, 1), (10, 1), (15, 1), (20, 1)],
        },
    )
    ...
    randomize_all_sfx = Checkbutton(
        ...
        disable={
            True: {
                "sections": ["generalsfx_section", "UIsfx_section", "itemsfx_section"],
                "settings": [
                    "sfx_navi_overworld",
                    "sfx_navi_enemy",
                    "sfx_horse_neigh",
    ...
    custom_music_directorypicker = Directoryinput(
        ...
        """,
        gui_params={
            "file_types": [{"name": "OoTR Sequence Files", "extensions": ["ootrs"]}],
            "hide_when_disabled": True,
        },
    )
...
def build_close_match(
    name: str, value_type: str, source_list: Optional[list[str] | dict[str, list[Entrance]]] = None
) -> str:
    source = []
    if value_type == "item":
...
def validate_settings(settings_dict: dict[str, Any], *, check_conflicts: bool = True) -> None:
    for setting, choice in settings_dict.items():
        ...
        if setting not in SettingInfos.setting_infos:
            raise TypeError(
                "%r is not a valid setting. %s" % (setting, build_close_match(setting, "setting"))
            )
        info = SettingInfos.setting_infos[setting]
        # Ensure the type of the supplied choice is correct
        if type(choice) is not info.type:
            raise TypeError(
                "Supplied choice %r for setting %r is of type %r, expecting %r"
        ...
        if isinstance(choice, list):
            ...
            for element in choice:
                if element not in info.choice_list:
                    raise ValueError(
                        "%r is not a valid choice for setting %r. %s"
...
def validate_disabled_setting(
    settings_dict: dict[str, Any], setting: str, choice, other_setting: str
) -> None:
    if other_setting in settings_dict:
        if (
            settings_dict[other_setting]
            != SettingInfos.setting_infos[other_setting].disabled_default
        ):
            raise ValueError(
                f"The {other_setting!r} setting cannot be used since {setting!r} is set to {choice!r}"
            )
...
SettingInfos.setting_infos = {
    n: s for n, s in SettingInfos.__dict__.items() if isinstance(s, SettingInfo)
}
...
for info in SettingInfos.setting_infos.values():
    if (
        info.gui_text is not None
        and not info.gui_params.get("optional")
        and not is_mapped(info.name)
    ):
        ...
        for option, disabling in info.disable.items():
            ...
            for section in disabling.get("sections", []):
                for setting_name in get_settings_from_section(section):
                    SettingInfos.setting_infos[setting_name].create_dependency(
                        info, option, negative
                    )
            for tab in disabling.get("tabs", []):

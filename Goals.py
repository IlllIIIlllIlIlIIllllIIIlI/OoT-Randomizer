...
RequiredLocations: TypeAlias = (
    "dict[str, dict[str, dict[int, list[tuple[Location, int, int]]]] | list[Location]]"
)
...
class Goal:
    def __init__(
        ...
        self.required_locations: list[tuple[Location, int, int, list[int]]] = (
            required_locations or []
        )
        self.weight: int = 0
...
class GoalCategory:
    ...
    def is_beaten(self, search: Search) -> bool:
        ...
        return all(
            map(
                lambda s: len(starting_goals[self.name]["stateReverse"][s.world.id])
                >= self.minimum_goals,
                search.state_list,
            )
        )
            ...
            for goal in world_category.goals:
                if goal.items:
                    if all(map(full_search.state_list[index].has_item_goal, goal.items)):
                        for i in goal.items:
                            i["quantity"] = min(
                                full_search.state_list[index].item_name_count(i["name"]),
                                i["quantity"],
                            )
    ...
    for world in worlds:
        if world.settings.shuffle_dungeon_rewards in ("vanilla", "reward"):
            bosses = [
                ...
                or (
                    location.name == "ToT Reward from Rauru"
                    and not world.settings.skip_reward_from_rauru
                )
            ]
...
def update_goal_items(spoiler: Spoiler) -> None:
    ...
    item_locations = {
        location for location in all_locations if location.item.majoritem and not location.locked
    }

    # required_locations[category.name][goal.name][world_id] = [...]
    required_locations: RequiredLocations = defaultdict(
    ...
    always_locations = [
        location.name for world in worlds for location in get_hint_group("always", world)
    ]
        ...
        for cat_name, category in worlds[0].locked_goal_categories.items():
            for cat_world in worlds:
                ...
                for goal_name, world_location_lists in identified_locations[cat_name].items():
                    required_locations[cat_name][goal_name][cat_world.id] = list(
                        identified_locations[cat_name][goal_name][cat_world.id]
                    )
    ...
    for world in worlds:
        woth_locations_dict[world.id] = list(
            filter(lambda location: location.world.id == world.id, woth_locations)
        )
    ...
    if (
        not required_locations
        and "ganon" not in worlds[0].goal_categories
        and worlds[0].hint_dist_user["use_default_goals"]
        and worlds[0].enable_goal_hints
    ):
        for world in worlds:
            locations = [
                (location, 1, 1, [world.id]) for location in spoiler.required_locations[world.id]
            ]
            ...
            required_locations_dict[world.id]["ganon"]["the hero"][world.id] = [
        ...
        for world in worlds:
            for cat_name, category in world.goal_categories.items():
                ...
                for goal in category.goals:
                    ...
                    for goal_world, locations in required_locations[category.name][
                        goal.name
                    ].items():
        ...
        for world in worlds:
            spoiler.goal_categories[world.id] = {
                cat_name: category.copy() for cat_name, category in world.goal_categories.items()
            }
    spoiler.goal_locations = required_locations_dict


def lock_category_entrances(
    category: GoalCategory, state_list: Iterable[State]
) -> dict[int, dict[str, AccessRule]]:
...
def unlock_category_entrances(
    category_locks: dict[int, dict[str, AccessRule]], state_list: list[State]
) -> None:
...
def search_goals(
    categories: dict[str, GoalCategory],
    reachable_goals: ValidGoals,
    search: Search,
    priority_locations: dict[int, dict[str, str]],
    all_locations: list[Location],
    item_locations: Collection[Location],
    always_locations: Collection[str],
    search_woth: bool = False,
) -> RequiredLocations:
    # required_locations[category.name][goal.name][world_id] = [...]
    required_locations: RequiredLocations = defaultdict(
        lambda: defaultdict(lambda: defaultdict(list))
        ...
        if location in item_locations:
            ...
            for cat_name, category in categories.items():
                ...
                if category.name in reachable_goals and reachable_goals[category.name]:
                    for goal in category.goals:
                        if (
                            category.name in valid_goals
                            and goal.name in valid_goals[category.name]
                            and goal.name in reachable_goals[category.name]
                            and (
                                location.name not in priority_locations[location.world.id]
                                or priority_locations[location.world.id][location.name]
                                == category.name
                            )
                            and not goal.requires(old_item.name)
                        ):
                            invalid_states = set(world_ids) - set(
                                valid_goals[category.name][goal.name]
                            )
                            hintable_states = list(
                                invalid_states & set(reachable_goals[category.name][goal.name])
                            )
                            if hintable_states:
                                for world_id in hintable_states:
                                    ...
                                    if (
                                        location.name in always_locations
                                        or location.name in location.world.hint_exclusions
                                    ):
                                        location_weights = (location, 0, 0)
                                    else:
                                        location_weights = (location, 1, 1)
                                    required_locations[category.name][goal.name][world_id].append(
                                        location_weights
                                    )
                                goal.weight = 1
                                category.weight = 1

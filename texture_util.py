...
def rgba16_to_ci8(rgba16_texture: list[int]) -> tuple[list[int], list[int]]:
    ...
    if (
        len(palette) > 0x100
    ):  # Make sure there are <= 256 colors. Could probably do some fancy stuff to convert, but nah.
        raise (Exception("RGB Texture exceeds maximum of 256 colors"))
    if (
        len(palette) < 0x100
    ):  # Pad the palette with 0x0001 #Pad the palette with 0001s to take up the full 256 colors
...
def rgba16_from_file(
    rom: Rom, base_texture_address: int, base_palette_address: int, size: int, patchfile: str
) -> bytearray:
...
# patchfile - file path of a rgba16 binary texture to patch
# returns - bytearray of the new texture
def ci4_rgba16patch_to_ci8(
    rom: Rom, base_texture_address: int, base_palette_address: int, size: int, patchfile: str
) -> bytearray:
    palette = load_palette(rom, base_palette_address, 16)  # load the original palette from rom
    base_texture_rgba16 = ci4_to_rgba16(
        rom, base_texture_address, size, palette
    )  # load the original texture from rom and convert to ci8
    patch_rgba16 = None
    if patchfile:
        patch_rgba16 = load_rgba16_texture(patchfile, size)
    new_texture_rgba16 = apply_rgba16_patch(base_texture_rgba16, patch_rgba16)
...
def build_crate_ci8_patches() -> None:
    ...
    crate_texture_rgba16 = ci4_to_rgba16(
        rom, object_kibako2_addr + 0x20, SIZE_CI4_32X128, crate_palette
    )
...
def build_smallcrate_patches() -> None:
    ...
    # Load textures
    smallcrate_default_rgba16 = load_rgba16_texture_from_rom(
        rom, object_kibako_texture_addr, SIZE_32X64
    )
